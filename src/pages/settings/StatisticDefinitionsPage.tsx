...
export const StatisticDefinitionsPage: React.FC = () => {
  ...
  const handleFieldChange = (clientId: string, field: keyof StatisticRowState, val: any) => {
    ...
    if (
      field === "ciktiSatirSayisi" ||
      field === "f1Tusu" ||
      field === "muhHesapId" ||
      field === "efektifDepoHesapId" ||
      field === "efektifVaziyetHesapId"
    ) {
      sanitizedVal = val === "" || val === null || val === undefined ? "" : onlyDigits(String(val));
    } else if (
      field === "komisyonOrani" ||
      field === "bmvOrani" ||
      field === "kmvOrani"
    ) {
      sanitizedVal = onlyDecimal(val);
    }
