...
const durumRozet = (durum: string) => ebelgeGidenDurumRozet(['GONDERILDI', 'GONDERILMEDI', 'IPTAL', 'TASLAK'].includes(durum) ? durum : 'HATA');
export default function EBelgeKaynakPage() {
  ...
  const yukle = async (p = 1) => {
    ...
    setBusy(true); setHata(''); setListelendi(true);
    try { const data = await ebelgeService.kaynakListe({ sayfa: p, arama: arama.trim() || undefined, durum: durum || undefined,
  ...
  // Tek adım: her belge arka planda önce test edilir (mükellefiyet + doğrulama + parmak izi),
  ...
  const gonder = async () => {
    ...
    for (const k of kayitlar.filter(r => r.secilebilir && secili.includes(key(r)))) {
      ...
      } catch (e: any) { setSonuclar(o => ({ ...o, [key(k)]: { no: k.belgeNo, durum: 'Hatalı', mesaj: e.message || 'Gönderim sonucu alınamadı; giden kutusunu kontrol edin.' } })); }
    }
    await yukle(sayfa);
  };
  ...
  return <div className="w-100 pb-3 ebelge-kaynak">
    ...
    <Card><Card.Body>
      <div className="d-flex flex-wrap gap-2 align-items-center mb-2">
        <Button size="sm" disabled={busy || !secilebilir.length} onClick={() => { setSecili(secilebilir.every(k => secili.includes(key(k))) ? [] : secilebilir.map(key)); }}>Tümünü seç / kaldır</Button>
        <Button size="sm" disabled={busy || !secili.length} onClick={gonder}>Gönder</Button>
        <span className="small">{secili.length} seçili</span>
      </div>
      <Table responsive size="sm" hover className="kaynak-tablo"><thead><tr><th>Seç</th><th>Belge no</th><th>Tarih</th><th>Ünvan</th><th>Tutar</th><th>Tür</th><th>Durum / hata açıklaması</th></tr></thead>
        <tbody>{kayitlar.map(k => { const rozet=durumRozet(k.durum); return <tr key={key(k)}>
          <td title={k.engel || undefined}><Form.Check aria-label={`${k.belgeNo} seç`} disabled={busy || !k.secilebilir} checked={secili.includes(key(k))} onChange={e => {
            setSecili(o => e.target.checked ? [...o,key(k)] : o.filter(id => id!==key(k)));
        ...
        </tr>; })}{!kayitlar.length && <tr><td colSpan={7}>{busy ? 'Yükleniyor…' : listelendi ? 'Seçilen tarih ve filtrelerde kayıt bulunamadı.' : 'Filtreleri seçip Listele düğmesine basın.'}</td></tr>}</tbody>
      </Table>
