 ...
 */
let tasiyici = null;
export const mailYapilandirildiMi = () => !!env.SMTP_HOST && !!env.SMTP_FROM;
/** Panelde gösterilen gönderen adresi (şifre vb. asla dönmez). */
export const mailGonderenAdresi = () => (mailYapilandirildiMi() ? env.SMTP_FROM : null);
const tasiyiciAl = () => {
    if (!mailYapilandirildiMi()) {
        throw new ApiError(HttpStatus.SERVICE_UNAVAILABLE, "Mail ayarı yapılmamış. Sunucudaki Backend/.env.local dosyasına SMTP bilgilerini girin.");
    }
    if (!tasiyici) {
...
export const mailGonder = async (mail) => {
    const t = tasiyiciAl();
    try {
