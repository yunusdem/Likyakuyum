...
import { EbelgeKaynakRepository, dovizMi, kaynakAnahtar } from "../models/ebelgeKaynak.repository.js";
...
export function kaynakFaturaGirdisi(kaynak) {
    const b = kaynak.baslik;
    if (![0, 1].includes(Number(b.BELGE_TURU)))
        throw ApiError.badRequest("Bu kaynak fatura değil. e-Gider veya e-İrsaliye oluşturma ekranını kullanın.");
    ...
    if (!/^[A-Z]{3}\d{13}$/.test(belgeNo))
        throw ApiError.badRequest("Kaynak fatura numarası 3 harf ve 13 rakam olmalıdır; kaynak kayıt düzeltilmelidir.");
    const para = temiz(b.PARA_KODU);
    if (!["TL", "TRY"].includes(para))
        throw ApiError.badRequest("Kaynak dövizli fatura için kur eşlemesi doğrulanmadan otomatik gönderim yapılamaz.");
    if (!kaynak.satirlar.length)
        throw ApiError.badRequest("Kaynak faturanın satırları bulunamadı.");
    const satirlar = kaynak.satirlar.map((s, i) => {
        ...
        if (!/^[A-Z0-9]{2,5}$/.test(birimKodu))
            throw ApiError.badRequest(`${i + 1}. satırın birim kodu doğrulanamadı.`);
...
export class EbelgeKaynakService {
    ...
    static async dovizGiden(uuid, ctx) {
        const kayit = await EbelgeSqlRepository.getGiden(uuid, ctx);
        if (!kayit || kayit.belgeTuru !== "EDoviz")
            throw ApiError.notFound("e-Döviz belgesi bulunamadı.");
        return kayit;
    }
    static async dovizDurum(uuid, kullanici, ctx) {
        ...
        await EbelgeSqlRepository.writeLog({ metod: "Get_EDoviz_Status", yon: "GIDEN", basarili: !!sonuc,
            kullanici, ilgiliUuid: uuid, cevapOzet: sonuc ? `STATUS=${sonuc.STATUS || ""}` : "Kayıt dönmedi" }, ctx);
        return sonuc;
    }
    static async dovizPdf(uuid, kullanici, ctx) {
        await this.dovizGiden(uuid, ctx);
        const config = await EbelgeSqlRepository.getConnectionConfig(ctx);
        ...
        await EbelgeSqlRepository.writeLog({ metod: "GetEDoviz_XML_PDF", yon: "GIDEN", basarili: !!sonuc.pdf?.length,
            kullanici, ilgiliUuid: uuid, cevapOzet: `${sonuc.pdf?.length || 0} bayt` }, ctx);
        if (!sonuc.pdf?.length)
            throw ApiError.notFound("e-Döviz PDF çıktısı alınamadı.");
        return sonuc.pdf;
    }
    static async dovizIptal(uuid, iptalTarihi, kullanici, ctx) {
     ...
     */
    static async dovizGonder(k, parmakizi, kullanici, ctx) {
        const kaynak = await EbelgeKaynakRepository.dovizDetay(k, ctx);
        if (kaynakParmakizi(kaynak) !== parmakizi) {
        ...
        try {
            ...
            if (kaynakParmakizi(guncel) !== parmakizi)
                throw ApiError.conflict('Kaynak döviz fişi değişmiş; yeniden hazırlayın.');
            await EbelgeSqlRepository.insertGiden({
                ...
                aliciUnvan: girdi.musteri.unvan || [girdi.musteri.ad, girdi.musteri.soyad].filter(Boolean).join(" ") || null,
...
const zorunluSayi = (v, ad) => {
    const n = sayi(v);
    if (n === undefined)
        throw ApiError.badRequest(`${ad} kaynak fişte okunamadı; gönderim durduruldu.`);
    return n;
};
const isoTarih = (v, ad) => {
    const d = new Date(v);
    if (Number.isNaN(d.getTime()))
        throw ApiError.badRequest(`${ad} kaynak fişte geçersiz.`);
...
export function dovizGirdisi(kaynak, gonderici) {
    ...
    if (!/^[A-Z]{3}$/.test(dovizKodu))
        throw ApiError.badRequest("Döviz kodu okunamadı.");
    ...
    if (miktar <= 0)
        throw ApiError.badRequest("Döviz miktarı sıfır veya negatif olamaz.");
    const tlKarsilikKuru = zorunluSayi(b.TL_KARSILIK_KURU ?? b.KUR, "TL karşılık kuru");
    if (tlKarsilikKuru <= 0)
        throw ApiError.badRequest("TL karşılık kuru sıfır veya negatif olamaz.");
