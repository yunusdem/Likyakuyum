...
export class EBankaDenetimService {
    static async denetle(dbContext) {
        const sonuclar = [];
        const ekle = (grup, ad, durum, ayrinti) => sonuclar.push({ grup, ad, durum, ayrinti });
        /** Bir denetim patlarsa diğerleri yine çalışsın */
        const dene = async (grup, ad, is) => {
            try {
                await is();
            }
            catch (err) {
        ...
        const sorgu = async (q) => (await pool.request().query(q)).recordset;
        // ─── Veritabanı ──────────────────────────────────────────────────────────
        ...
        for (const [tablo, doldurulan] of Object.entries(FIS_KOLONLARI)) {
          ...
          FROM sys.columns c WHERE c.object_id = OBJECT_ID('dbo.${tablo}')`);
                if (!kolonlar.length)
                    return void ekle("Banka fişi", tablo, "hata", "Tablo yok. Banka Hesap Hareketleri ekranı bir kez açılınca oluşur.");
                ...
                if (eksik.length)
                    return void ekle("Banka fişi", tablo, "hata", `Fiş kaydının yazdığı kolonlar tabloda yok: ${eksik.join(", ")}`);
                if (engel.length)
                    return void ekle("Banka fişi", tablo, "hata", `Tabloda fiş kaydının doldurmadığı zorunlu kolonlar var: ${engel.join(", ")} — otomatik fiş kesilemez`);
        ...
        FROM dbo.TODVZ_KUR_TABLOSU T ORDER BY T.TARIH DESC, T.KUR_TABLOSU_ID DESC`))[0];
            if (!r)
                return void ekle("Tanımlar", "Kur tablosu", "uyari", "Kur tablosu boş; döviz hesaplarının hareketleri kur bulunamadığı için Bekleyenler'de kalır");
        ...
        await dene("Ayarlar", "Sanal POS banka hesabı", async () => {
            if (!ayar?.vposBankaId)
                return void ekle("Ayarlar", "Sanal POS banka hesabı", "uyari", "Seçilmemiş — Sanal POS tahsilatları için fiş kesilemez");
     ...
     */
    static async denemeFisi(bankaId, kullaniciId, dbContext) {
        if (!(Number(bankaId) > 0))
        ...
        const adimlar = [];
        const ekle = (ad, durum, ayrinti) => adimlar.push({ grup: "Deneme fişi", ad, durum, ayrinti });
        const pool = await EBankaVeriSqlRepository.pool(dbContext);
        ...
        if (!tl)
            throw ApiError.badRequest("TL para tanımı bulunamadı.");
        ...
        await EBankaSqlRepository.logYaz({ islem: "denetim-deneme-fisi", mod: (await EBankaSqlRepository.ayarGetir(dbContext))?.mod ?? "sahte", basarili, mesaj: adimlar.map((a) => `${a.ad}: ${a.durum}`).join(", ") + (fisId ? ` (#${fisId})` : ""), kullaniciId }, dbContext);
