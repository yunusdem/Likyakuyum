...
export const odendiMi = (durum) => {
    const s = kirp(durum).toLocaleLowerCase("tr");
    if (!s || s.includes("ödenme") || s.includes("odenme") || s.includes("unpaid") || s.includes("not paid"))
        return false;
...
const paraKodu = (paraBirimi) => (kirp(paraBirimi).toUpperCase() === "TRY" || !kirp(paraBirimi) ? "TL" : kirp(paraBirimi).toUpperCase());
export class EBankaVposService {
    static async ayar(dbContext) {
    ...
    static async fisOnKosullari(paraBirimi, gun, giris, dbContext) {
        const { mod, vposBankaId } = await this.ayar(dbContext);
        if (mod !== "canli")
            return { neden: "Test (örnek veri) modunda fiş kesilmez." };
        if (!vposBankaId)
            return { neden: "Ayarlar'da Sanal POS banka hesabı seçilmemiş." };
        try {
        ...
        if (!paraId)
            return { neden: `"${paraBirimi}" para birimi para tanımlarında yok.` };
        let kur = 1;
        if (paraId !== para.tlId) {
            kur = await EBankaAktarimSqlRepository.kurGetir(paraId, gun, giris, dbContext);
            if (!(kur > 0))
                return { neden: `${gun} için ${paraBirimi} kuru bulunamadı.` };
        }
        return { bankaId: vposBankaId, paraId, kur };
    }
    static async fisYaz(p, kullaniciId, dbContext) {
    ...
    // ─── Ödeme linkleri ────────────────────────────────────────────────────────
    static async linkOlustur(girdi, kullaniciId, dbContext) {
        ...
        if (sonGecerlilik && (!gunMu(sonGecerlilik) || sonGecerlilik < bugun()))
            throw ApiError.badRequest("Son geçerlilik tarihi geçersiz.");
        ...
        if (sms && !telefon)
            throw ApiError.badRequest("SMS bildirimi için telefon numarası gerekir.");
        if (mail && !eposta)
            throw ApiError.badRequest("E-posta bildirimi için e-posta adresi gerekir.");
        ...
        const yanit = await VomsisClient.istek("vpos", "/request-payment", {
            metod: "POST",
            govde: {
                title: baslik,
        ...
        if (yanit?.success === false)
            throw new ApiError(HttpStatus.BAD_GATEWAY, `Ödeme linki oluşturulamadı: ${yanit.message || "bilinmeyen hata"}`);
        ...
        if (!uid)
            throw new ApiError(HttpStatus.BAD_GATEWAY, "Ödeme linki oluşturuldu ama kimliği (uid) dönmedi. Servis panelinden kontrol edin.");
        ...
        await EBankaVposSqlRepository.linkEkle({ uid, cariKartId, baslik, tutar, paraBirimi, sonGecerlilik: sonGecerlilik || null, eposta: eposta || null, telefon: telefon || null, sms, mail, maxTaksit, aciklama, link, durum, odendi: false }, kullaniciId, dbContext);
        await EBankaSqlRepository.logYaz({ islem: "vpos-link", mod: (await this.ayar(dbContext)).mod, basarili: true, mesaj: `${cari.ad}: ${tutar.toFixed(2)} ${paraBirimi} ödeme linki`, kullaniciId }, dbContext);
        return (await EBankaVposSqlRepository.linkGetir(uid, dbContext));
    }
    /** Ödenmiş ve fişi olmayan link için tahsilat fişi. Kesilemezse nedenini döner (hata fırlatmaz: güncelleme yarıda kalmasın). */
    static async linkFisiKes(l, kullaniciId, dbContext) {
        ...
        if ("neden" in on)
            return on;
        if (!(await EBankaVposSqlRepository.linkFisTalebi(l.uid, dbContext)))
            return { neden: "Link için zaten fiş kesilmiş." };
        try {
            const bankaHareketId = await this.fisYaz({ islemTipi: HAVALE_ALMA, bankaId: on.bankaId, cariKartId: l.cariKartId, paraId: on.paraId, kur: on.kur, tutar: l.tutar, gun, belgeNo: l.uid.slice(0, 50), aciklama: `Sanal POS ödeme linki: ${l.baslik || ""}`.trim() }, kullaniciId, dbContext);
     ...
     */
    static async linkleriGuncelle(kullaniciId, dbContext) {
        ...
        for (const l of acik) {
            ...
            if (odendi) {
                ...
                if ("bankaHareketId" in sonuc)
                    kesilenFis++;
                else if (mod === "canli")
                    uyarilar.push(`${l.baslik || l.uid}: fiş kesilemedi — ${sonuc.neden}`);
            }
        }
        await EBankaSqlRepository.logYaz({ islem: "vpos-link-guncelle", mod, basarili: uyarilar.length === 0, adet: yeniOdenen, mesaj: `${acik.length} link soruldu, ${yeniOdenen} yeni ödeme, ${kesilenFis} tahsilat fişi${uyarilar.length ? ` — ${uyarilar[0]}` : ""}`, kullaniciId }, dbContext);
    ...
    static async cariIletisim(cariKartId, dbContext) {
        ...
        if (!c)
            throw ApiError.notFound("Cari bulunamadı.");
        return c;
    }
    // ─── İşlemler ──────────────────────────────────────────────────────────────
    /** Vomsis işlem listesini aynaya çeker. Dokümanda tarih süzgeci ve sayfalama yok; liste olduğu gibi alınır. */
    static async islemleriGuncelle(kullaniciId, dbContext) {
        const { mod } = await this.ayar(dbContext);
        try {
            const yanit = await VomsisClient.istek("vpos", "/transactions-list", { sorgu: { status: "" } }, dbContext);
            if (yanit?.success === false)
                throw new ApiError(HttpStatus.BAD_GATEWAY, `İşlem listesi alınamadı: ${yanit.message || "bilinmeyen hata"}`);
    ...
    static async islemDetayi(referansNo, dbContext) {
        ...
        const yanit = await VomsisClient.istek("vpos", "/transaction/find", { sorgu: { referanceNo: referansNo } }, dbContext).catch((err) => ({ success: false, message: err?.message }));
    ...
    static async iptalIade(referansNo, girdi, kullaniciId, dbContext) {
        const tur = girdi.tur === "cancel" ? "cancel" : girdi.tur === "refund" ? "refund" : null;
        if (!tur)
            throw ApiError.badRequest("İşlem türü iptal ya da iade olmalıdır.");
        const islem = await EBankaVposSqlRepository.islemGetir(referansNo, dbContext);
        if (!islem)
            throw ApiError.notFound("İşlem bulunamadı. Önce işlem listesini güncelleyin.");
        if (islem.tur && islem.tur !== "islem")
            throw ApiError.conflict("Bu işlem daha önce iptal ya da iade edilmiş.");
        const tutar = tur === "refund" ? Math.round(Number(String(girdi.tutar ?? "").replace(",", ".")) * 100) / 100 : islem.tutar;
        if (tur === "refund" && (!(tutar > 0) || tutar > islem.tutar))
            throw ApiError.badRequest("İade tutarı sıfırdan büyük olmalı ve işlem tutarını aşmamalıdır.");
        const { mod } = await this.ayar(dbContext);
        const yanit = await VomsisClient.istek("vpos", "/transaction", { metod: "POST", govde: { transactionType: tur, referanceNo: referansNo, ...(tur === "refund" ? { amount: tutar } : {}) } }, dbContext);
            ...
            if (mod !== "canli") {
                ...
                if (tur === "cancel") {
                    ...
                    if ("neden" in on) {
                        ...
                        const fisId = await this.fisYaz({ islemTipi: HAVALE_GONDERME, bankaId: on.bankaId, cariKartId: islem.cariKartId, paraId: on.paraId, kur: on.kur, tutar, gun, belgeNo: referansNo, aciklama: `Sanal POS iade: ${islem.aciklama || referansNo}` }, kullaniciId, dbContext);
        ...
        await EBankaSqlRepository.logYaz({ islem: `vpos-${tur}`, mod, basarili: true, mesaj: `${referansNo}: ${tutar.toFixed(2)} ${islem.paraBirimi || ""} ${tur === "cancel" ? "iptal" : "iade"} — ${muhasebe}`, kullaniciId }, dbContext);
        await this.islemleriGuncelle(kullaniciId, dbContext).catch(() => undefined);
        return { onaylandi: true, tutar, muhasebe, islem: (await EBankaVposSqlRepository.islemGetir(referansNo, dbContext)) };
    }
    /** Vomsis müşteri kayıtları — yalnızca okunur (E22). */
    static async vomsisMusterileri(dbContext) {
        const yanit = await VomsisClient.istek("vpos", "/customers", {}, dbContext);
        const liste = Array.isArray(yanit) ? yanit : Array.isArray(yanit?.data) ? yanit.data : Array.isArray(yanit?.customers) ? yanit.customers : [];
