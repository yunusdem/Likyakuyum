...
import { adetOzeti, aralikOzeti, filtreler, idFiltre, ozetEk, sinirla, tarihTr } from "../raporOrtak.js";
...
export const FIS_SORGULARI = {
    ...
    async FISLIS1(pool, p, t) {
        ...
        pids.forEach((id, i) => req.input(`fp${i}`, sql.Int, id));
      ...
      ORDER BY V.KOD, ISNULL(F.ZAMAN,F.TARIH), F.FIS_ID;`);
        const satirlar = res.recordset.map((r) => ({ ...r, tip: tipAdi(r.tipKod), toplamTutar: Number(r.toplamTutar), odemeTutari: Number(r.odemeTutari), satirSayisi: Number(r.satirSayisi),
            komisyon: Number(r.komisyon), bmv: Number(r.bmv), kmv: Number(r.kmv), kdv: Number(r.kdv), vezneBaslik: `${r.vezneKod} — ${r.vezneAd}` }));
        return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p) || " · Tüm vezneler"}${adetOzeti(p.istatistikIdler, "istatistik")}`, "Her fiş tek satırdır; komisyon ve vergiler fiş satırlarının toplamıdır. Para seçilirse, satırlarından en az birinde o para bulunan fişler listelenir. İptal fişler hariçtir.");
    },
    ...
    async ISTRAP1(pool, p, t) {
      ...
      ORDER BY I.KOD, F.TIP, ISNULL(P.SIRA_NO,99), P.KOD;`);
        const satirlar = res.recordset.map((r) => {
            const miktar = Number(r.miktar) || 0, tutar = Number(r.tutar) || 0;
        ...
        for (const s of satirlar) {
            const k = `${s.paraKod}|${s.tip}`;
            if (!oz.has(k))
                oz.set(k, { paraKod: s.paraKod, tip: s.tip, adet: 0, miktar: 0, tutar: 0, komisyon: 0, bmv: 0, kmv: 0, kdv: 0 });
            const o = oz.get(k);
            for (const a of ["adet", "miktar", "tutar", "komisyon", "bmv", "kmv", "kdv"])
                o[a] += Number(s[a]) || 0;
        }
        const ozet = [...oz.values()].map(o => ({ ...o, ortKur: o.miktar ? o.tutar / o.miktar : 0 }));
        return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p) || " · Tüm vezneler"}${adetOzeti(p.istatistikIdler, "istatistik")}`, "Fişler istatistik koduna göre gruplanır; ortalama kur = tutar / miktar. İptal fişler hariçtir.", ozet);
    },
    ...
    async ISTKMV1(pool, p, t) {
      ...
      ORDER BY I.KOD, ISNULL(P.SIRA_NO,99), P.KOD, S.KMV_ORANI;`);
        const satirlar = res.recordset.map((r) => ({ ...r, adet: Number(r.adet), miktar: Number(r.miktar), matrah: Number(r.matrah), kmvOrani: Number(r.kmvOrani), kmv: Number(r.kmv),
            istatistikBaslik: r.istatistikKod ? `${r.istatistikKod} — ${r.istatistikAd}` : "İstatistik kodu yok" }));
        return sinirla(satirlar, t, `${aralikOzeti(p)} · Satış${ozetEk(p) || " · Tüm vezneler"}${adetOzeti(p.istatistikIdler, "istatistik")}`, "Kambiyo muameleleri vergisi (KMV) yalnızca satış fişlerinde doğar; KMV'si veya KMV oranı olmayan satırlar listelenmez. Matrah = satır tutarı (TL). İptal fişler hariçtir.");
    },
    ...
    async VERNUM1(pool, p, t) {
      ...
      GROUP BY COALESCE(NULLIF(RTRIM(F.VERGI_KIMLIK_NO),''), NULLIF(RTRIM(F.PASAPORT_NO),''), ''),
        ...
        const satirlar = ham.sort((a, b) => (p.siralama === "tutar" ? (toplam.get(b.grupAnahtar) - toplam.get(a.grupAnahtar)) : p.siralama === "unvan" ? (unvan.get(a.grupAnahtar) || "").localeCompare(unvan.get(b.grupAnahtar) || "", "tr") : 0)
            || a.grupAnahtar.localeCompare(b.grupAnahtar) || a.tipKod - b.tipKod || a.siraNo - b.siraNo || a.paraKod.localeCompare(b.paraKod));
        return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p) || " · Tüm vezneler"}${p.arama?.trim() ? ` · Arama: "${p.arama.trim()}"` : ""}`, "Fişler müşterinin vergi / TC kimlik numarasına (yoksa pasaport numarasına) göre gruplanır. USD karşılığı her fişin kayıt anındaki gişe USD kuruyla hesaplanır (kuru kaydedilmemiş fişlerde 0). İptal fişler hariçtir.");
    },
};
