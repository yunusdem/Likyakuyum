...
import { adetOzeti, aralikOzeti, filtreler, idFiltre, ozetEk, sinirla } from "../raporOrtak.js";
...
async function masakFisleri(pool, p, ek = () => "") {
    ...
    return res.recordset.map((r) => ({ ...r, tip: Number(r.tipKod) === 1 ? "Satış" : "Alış", tutar: Number(r.tutar) || 0, usdKarsiligi: Number(r.usdKarsiligi) || 0,
...
export const MASAK_SORGULARI = {
    ...
    async KNSKLOG1(pool, p, t) {
      ...
      ORDER BY L.ZAMAN;`);
        const satirlar = res.recordset.map((r) => ({ ...r, basarili: !!r.basarili, karaListede: !!r.karaListede, kisilik: Number(r.kisilikKod) === 1 ? "Tüzel" : "Gerçek",
    ...
    async MSKYUK1(pool, p, t) {
        ...
        if (gunluk) {
            ...
            for (const f of fisler) {
                ...
                if (!m.has(k))
                    m.set(k, { ...f, fisAdedi: 0, tutar: 0, usdKarsiligi: 0, belgeler: [] });
                const o = m.get(k);
                o.fisAdedi++;
                o.tutar += f.tutar;
                o.usdKarsiligi += f.usdKarsiligi;
                o.belgeler.push(f.belgeNo);
            }
            ...
            satirlar = fisler.filter(f => f.tutar >= esik).map(f => ({ ...f, fisAdedi: 1 }));
        return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p)} · Eşik ${tl(esik)} TL · ${gunluk ? "aynı kimliğin günlük toplamı" : "fiş bazında"}`, gunluk ? "Aynı kimlik numarasıyla aynı gün yapılan fişler toplanır; toplamı eşiği aşanlar listelenir (kimlik numarası olmayan fişler tek başına değerlendirilir). \"Belge no\" ilk fişi ve ek fiş sayısını gösterir." : undefined);
    },
    ...
    async MSKKON1(pool, p, t) {
        ...
        return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p)} · Eşik ${esik > 0 ? tl(esik) + " TL" : "yok"} · Parça sayısı ≥ ${n}`, esik > 0 ? undefined : "Eşik tutarı girilmediği ve firma tanımında TL vergi sınırı olmadığı için yalnızca MASAK listesi eşleşmeleri kontrol edildi.");
    },
};
