 ...
 */
export function maliyetYurut(hareketler) {
    const stok = new Map();
...
const ISCILIK_SEKLI = { 0: "Adet başına", 1: "Toplam", 2: "Gram başına" };
export const ANALIZ_SORGULARI = {
    ...
    async KARLIL1(pool, p, t) {
      ...
      ORDER BY ISNULL(P.SIRA_NO,99), P.KOD, F.TARIH, F.FIS_ID, S.SATIR_NO;`);
        ...
        const yurutulen = maliyetYurut(res.recordset.map((r) => ({ ...r, paraId: Number(r.paraId), tip: Number(r.tip), miktar: Number(r.miktar), tutar: Number(r.tutar), kur: Number(r.kur) })));
        ...
        if (p.siralama === "kar") { // para grupları korunur, grup içinde net kâra göre büyükten küçüğe
            const sira = new Map();
            satirlar.forEach(s => { if (!sira.has(s.paraBaslik))
