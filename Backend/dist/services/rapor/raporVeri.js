...
import { tarihTr, HAREKET_TIPI, KISILIK, kurCoz, sinirla, aralikOzeti, filtreler, paraKumesi, ozetEk, vezneBakiyeleri, VEZNE_BAKIYE_DIPNOT, } from "./raporOrtak.js";
...
export const RAPOR_SORGULARI = {
    ...
    async CARBAK1(pool, p, t) {
      ...
      ORDER BY C.KOD, ISNULL(P.SIRA_NO,99), P.KOD;`);
        ...
        return sinirla(satirlar, t, `${tarihTr(bas)} – ${tarihTr(bit)}${ozetEk(p) || " · Tüm cariler"} · ${kur.aciklama}`, "Borç ve alacak seçilen tarih aralığındaki hareketlerin toplamıdır; bakiye bitiş tarihi itibarıyla tüm hareketlerden hesaplanır. Yön: Alacak = carinin bizden alacağı, Borç = carinin bize borcu. Has karşılığı = bakiye × para tanımındaki has oranı. Sıfır bakiyeli cariler de listelenir.");
    },
    ...
    async CAREKS1(pool, p, t) {
      ...
      ORDER BY C.KOD, ISNULL(P.SIRA_NO,99), P.KOD, H.TARIH, H.CARI_HAREKET_ID, S.SATIR_NO;`);
        ...
        return sinirla(satirlar, t, `${aralikOzeti(p)}${ozetEk(p)}`, "Yön: A = cari alacaklı (bizden alacağı var), B = cari borçlu. Her cari ve para birimi ayrı gruplanır; ilk satır başlangıç tarihinden önceki devirdir. Has karşılığı = yürüyen bakiye × para tanımındaki has oranı.");
    },
    ...
    async FIRVAR1(pool, p, t) {
      ...
      WHERE CAST(H.TARIH AS date)<=@t GROUP BY S.PARA_ID, P.KOD, P.SIRA_NO ORDER BY ISNULL(P.SIRA_NO,99), P.KOD;`);
        ...
        return sinirla(satirlar, t, `${tarihTr(p.tarih)} itibarıyla${ozetEk(p)} · ${kur.aciklama}`, `${VEZNE_BAKIYE_DIPNOT} Cari borçlarımız eksi işaretle düşülür; banka hesapları kapsam dışıdır (yönetici kararı). ${kur.aciklama}.`);
    },
    ...
    async VEZHAR1(pool, p, t) {
      ...
      ORDER BY V.KOD, ISNULL(F.ZAMAN,F.TARIH), F.FIS_ID, S.SATIR_NO;`);
        const satirlar = res.recordset.map((r) => {
            const secilenKur = kur.kurlar.get(Number(r.paraId)) ?? 0, ks = kur.satisKurlari.get(Number(r.paraId)) ?? 0;
            return { ...r, tip: Number(r.tipKod) === 1 ? "Satış" : "Alış", miktar: Number(r.miktar), kur: Number(r.kur), tutar: Number(r.tutar), komisyon: Number(r.komisyon), bmv: Number(r.bmv),
                secilenKur, secilenTl: Number(r.miktar) * secilenKur, kurSatis: ks, tlSatis: Number(r.miktar) * ks, vezneBaslik: `${r.vezneKod} — ${r.vezneAd}` };
        });
        return sinirla(satirlar, t, `${aralikOzeti(p)} · ${(p.baslangicSaat || "00:00").slice(0, 5)}–${(p.bitisSaat || "23:59").slice(0, 5)}${ozetEk(p) || " · Tüm vezneler"}${p.fisTipi === 0 ? " · Alış" : p.fisTipi === 1 ? " · Satış" : ""} · ${kur.aciklama}`, `İptal edilmiş fişler listelenmez. "Seçilen kur / TL" kolonları fiş kurundan bağımsız, parametrede seçilen kurla hesaplanır (${kur.aciklama}).`);
    },
};
