...
import { tarihTr, HAREKET_TIPI, KISILIK, kurCoz, hedefPara, gunOnce, kurTarihte, sinirla, TL_PARA_SQL, aralikOzeti, filtreler, paraKumesi, ozetEk, vezneBakiyeleri, VEZNE_BAKIYE_DIPNOT, } from "./raporOrtak.js";
...
export const RAPOR_SORGULARI = {
    ...
    async CARBAK1(pool, p, t) {
      ...
      ORDER BY C.KOD, ISNULL(P.SIRA_NO,99), P.KOD;`);
        ...
        for (const x of satirlar) {
            if (!pt.has(x.paraKod))
                pt.set(x.paraKod, { paraKod: x.paraKod, borcBakiye: 0, alacakBakiye: 0 });
            const o = pt.get(x.paraKod);
        ...
        return sinirla(satirlar, t, `${tarihTr(bas)} – ${tarihTr(bit)}${ozetEk(p) || " · Tüm cariler"} · ${kur.aciklama}`, "Borç bakiye / Alacak bakiye bitiş tarihi itibarıyla tüm hareketlerden hesaplanan net bakiyenin yönüdür (Borç = carinin bize borcu, Alacak = carinin bizden alacağı); sıfır bakiyeli satırlar listelenmez. Dönem borç ve dönem alacak seçilen tarih aralığındaki hareket toplamlarıdır. Has karşılığı = bakiye × para tanımındaki has oranı.", paraToplamlari);
    },
    ...
    async CAREKS1(pool, p, t) {
        if (!p.cariKartId && !p.cariIdler?.length && !p.cariSonId && !p.cariBaslangic && !p.cariBitis)
            throw ApiError.badRequest("En az bir cari seçilmelidir.");
      ...
      ORDER BY C.KOD, ISNULL(P.SIRA_NO,99), P.KOD, H.TARIH, H.CARI_HAREKET_ID, S.SATIR_NO;`);
        ...
        const ekle = (r) => {
            const k = anahtar(r);
            if (!gruplar.has(k))
                gruplar.set(k, { cariKod: r.cariKod, cariAd: r.cariAd, cariAdres: r.cariAdres || "", cariTelefon: r.cariTelefon || "", cariVergiNo: r.cariVergiNo || "",
                    paraKod: r.paraKod, siraNo: Number(r.siraNo), hasOrani: Number(r.hasOrani) || 0, devir: 0, hareketler: [] });
        ...
        const satirlar = [], paraToplamlari = [];
        const genel = new Map();
        const bakiyeOzeti = (b) => ({ bakiye: Math.abs(b), yon: b > 0 ? "A" : b < 0 ? "B" : "" });
        for (const g of [...gruplar.values()].sort((x, y) => x.cariKod.localeCompare(y.cariKod) || x.siraNo - y.siraNo || x.paraKod.localeCompare(y.paraKod))) {
            ...
            const satir = (ek) => ({ ...ortak, ...ek, bakiye: Math.abs(bakiye), yon: bakiye > 0 ? "A" : bakiye < 0 ? "B" : "", hasKarsiligi: Math.abs(bakiye) * g.hasOrani });
            ...
            const gp = genel.get(g.paraKod) || { borc: 0, alacak: 0 };
            gp.borc += tBorc;
            gp.alacak += tAlacak;
            genel.set(g.paraKod, gp);
        }
        // GENEL TOPLAM + genel bakiye + B/A — para birimleri karışmasın diye para başına bir satır
        if (gruplar.size > 1)
            for (const [paraKod, gp] of genel)
                paraToplamlari.push({ cari: "GENEL TOPLAM", paraKod, borc: gp.borc, alacak: gp.alacak, ...bakiyeOzeti(gp.alacak - gp.borc) });
        return sinirla(satirlar, t, `${aralikOzeti(p)}${ozetEk(p)}`, "Yön: A = cari alacaklı (bizden alacağı var), B = cari borçlu. Her cari ve para birimi ayrı gruplanır; ilk satır başlangıç tarihinden önceki devirdir. Has karşılığı = yürüyen bakiye × para tanımındaki has oranı.", paraToplamlari);
    },
    ...
    async CARHAR1(pool, p, t) {
      ...
      ORDER BY ISNULL(P.SIRA_NO,99), P.KOD, ${siraSql}H.TARIH, H.CARI_HAREKET_ID, S.SATIR_NO;`);
        const satirlar = res.recordset.map((r) => ({ ...r, fisNo: String(r.fisNo ?? ""), meblag: Number(r.meblag), hareketTipi: HAREKET_TIPI[Number(r.hareketTipiKod)] || "Diğer",
        ...
        const sayac = new Map(), net = new Map();
        for (const x of satirlar) {
            sayac.set(x.paraKod, (sayac.get(x.paraKod) || 0) + 1);
            x.siraNo = sayac.get(x.paraKod);
            const o = net.get(x.paraKod) || { paraKod: x.paraKod, borc: 0, alacak: 0 };
            o.borc += x.borc;
            o.alacak += x.alacak;
            net.set(x.paraKod, o);
        }
        ...
        return sinirla(satirlar, t, `${aralikOzeti(p)}${ozetEk(p)}${Number.isInteger(p.hareketTipi) ? ` · ${HAREKET_TIPI[p.hareketTipi] || "Diğer"}` : htler.length ? ` · ${htler.map(h => HAREKET_TIPI[h]).join(", ")}` : ""}`, undefined, netToplamlar);
    },
    ...
    async CARKRT1(pool, p, t) {
      ...
      ORDER BY C.KOD;`);
        const satirlar = res.recordset.map((r, i) => ({ ...r, siraNo: i + 1, kisilik: KISILIK[Number(r.kisilikKod)] || "-", karaListe: r.karaListe ? "EVET" : "" }));
    ...
    async FIRVAR1(pool, p, t) {
        ...
        const paraUygun = (id) => (!p.paraId || id === p.paraId) && (!paraSet || paraSet.has(id));
        const satirlar = vezneler.filter(v => Math.abs(v.miktar) > 0.000001 && paraUygun(Number(v.paraId))).map(v => ({
      ...
      WHERE CAST(H.TARIH AS date)<=@t GROUP BY S.PARA_ID, P.KOD, P.SIRA_NO ORDER BY ISNULL(P.SIRA_NO,99), P.KOD;`);
        for (const r of cari.recordset) {
            if (!paraUygun(Number(r.paraId)))
                continue;
            const k = kur.kurlar.get(Number(r.paraId)) ?? 0, ks = kur.satisKurlari.get(Number(r.paraId)) ?? 0;
            if (Number(r.borc))
                satirlar.push({ kaynak: "Cari alacaklarımız (carilerin bize borcu)", grup: "2-Cari alacaklar", paraKod: r.paraKod, miktar: Number(r.borc), kur: k, tlKarsiligi: Number(r.borc) * k, kurSatis: ks, tlSatis: Number(r.borc) * ks });
            if (Number(r.alacak))
                satirlar.push({ kaynak: "Cari borçlarımız (carilerin bizden alacağı)", grup: "3-Cari borçlar", paraKod: r.paraKod, miktar: -Number(r.alacak), kur: k, tlKarsiligi: -Number(r.alacak) * k, kurSatis: ks, tlSatis: -Number(r.alacak) * ks });
        }
        ...
        FROM dbo.TODVZ_CARI_HAREKET H JOIN dbo.TODVZ_CARI_HAREKET_SATIRI S ON S.CARI_HAREKET_ID=H.CARI_HAREKET_ID JOIN dbo.TODVZ_PARA P ON P.PARA_ID=S.PARA_ID WHERE CAST(H.TARIH AS date)<=@t GROUP BY S.PARA_ID, P.KOD`);
            for (const r of c.recordset) {
                if (!paraUygun(Number(r.paraId)))
                    continue;
                const o = m.get(r.paraKod) || { paraId: Number(r.paraId), paraKod: r.paraKod, miktar: 0 };
                o.miktar += Number(r.net) || 0;
                m.set(r.paraKod, o);
            }
            return m;
        };
        ...
        const hd = hedef.id === kur.tlId ? 1 : devirKur.get(hedef.id) ?? 0, bol = (x, y) => (y > 0 ? x / y : 0);
        const varlikOzeti = [];
        let dDeger = 0, dBugun = 0, vDeger = 0;
        for (const kod of new Set([...devirler.keys(), ...devredenler.keys()])) {
            const d0 = devirler.get(kod)?.miktar ?? 0, d1 = devredenler.get(kod)?.miktar ?? 0, id = (devredenler.get(kod) || devirler.get(kod)).paraId, h = d1 - d0;
            if (Math.abs(d0) < 0.000001 && Math.abs(d1) < 0.000001)
                continue;
            varlikOzeti.push({ kalem: kod, devir: d0, borc: h > 0 ? h : 0, alacak: h < 0 ? -h : 0, devreden: d1, deger: hedef.cevir(d1 * (kur.kurlar.get(id) ?? 0)) });
            dDeger += bol(d0 * (devirKur.get(id) ?? 0), hd);
            dBugun += hedef.cevir(d0 * (kur.kurlar.get(id) ?? 0));
            vDeger += hedef.cevir(d1 * (kur.kurlar.get(id) ?? 0));
        }
        const bos = { devir: null, borc: null, alacak: null, devreden: null };
        varlikOzeti.push({ kalem: `Devir değeri (${hedef.kod}, ${tarihTr(devirGun)} kuruyla)`, ...bos, deger: dDeger }, { kalem: `Kapanış / açılış evalüasyonu (${hedef.kod})`, ...bos, deger: dBugun - dDeger }, { kalem: `Devreden değeri (${hedef.kod})`, ...bos, deger: vDeger }, { kalem: `${vDeger - dDeger < 0 ? "Toplam Net Zarar" : "Toplam Net Kâr"} (${hedef.kod})`, ...bos, deger: Math.abs(vDeger - dDeger) });
        return sinirla(satirlar, t, `${tarihTr(p.tarih)} itibarıyla${ozetEk(p)} · ${kur.aciklama} · ${hedef.aciklama}`, `${VEZNE_BAKIYE_DIPNOT} Cari borçlarımız eksi işaretle düşülür; banka hesapları kapsam dışıdır (yönetici kararı). ${kur.aciklama}. Rapor altı: Devir = önceki günün kapanış varlığı, Borç / Alacak = seçilen günün net hareketi, Devreden = gün sonu varlığı; evalüasyon = devrin bugünkü kurla değeri − devir günü kuruyla değeri; toplam net kâr / zarar = devreden değeri − devir değeri.`, varlikOzeti);
    },
    ...
    async KARZAR1(pool, p, t) {
      ...
      ORDER BY ISNULL(P.SIRA_NO,99), P.KOD, F.TARIH, F.FIS_ID, S.SATIR_NO;`);
        ...
        for (const r of res.recordset) {
            ...
            if (!ozet.has(id))
                ozet.set(id, { paraId: id, paraKod: r.paraKod, paraAd: r.paraAd, devirMiktar: 0, alisMiktar: 0, alisTutar: 0, satisMiktar: 0, satisTutar: 0, satisMaliyeti: 0, brutKar: 0, komisyon: 0, vergiler: 0, netKar: 0, ortMaliyet: 0, kalanMiktar: 0 });
            ...
            if (!donemde)
                o.devirMiktar += Number(r.tip) === 0 ? miktar : -miktar; // devir = dönem öncesi alışlar − satışlar
        ...
        const satirlar = [...ozet.entries()].map(([id, o]) => {
            const s = stok.get(id);
            return { ...o, ortMaliyet: s.miktar > 0 ? s.maliyet / s.miktar : s.sonOrt, kalanMiktar: s.miktar, netKar: o.brutKar + o.komisyon - o.vergiler,
        ...
        const kmt = [];
        let tE = 0, tF = 0, tDevir = 0, tAlis = 0, tSatis = 0, tKapanis = 0;
        for (const o of satirlar) {
            const dk = devirKur.get(o.paraId) ?? 0, kk = kapanisKur.get(o.paraId) ?? 0, mevcut = o.devirMiktar + o.alisMiktar - o.satisMiktar;
            ...
            kmt.push({ para: o.paraKod, kmt: "K", devir: dk, alis: ortAlis, satis: ortSatis, kapanis: kk, eft: "E", sonuc: E }, { para: "", kmt: "M", devir: o.devirMiktar, alis: o.alisMiktar, satis: o.satisMiktar, kapanis: mevcut, eft: "F", sonuc: F }, { para: "", kmt: "T", devir: o.devirMiktar * dk, alis: o.alisTutar, satis: o.satisTutar, kapanis: mevcut * kk, eft: "T", sonuc: E + F });
            tE += E;
            tF += F;
            tDevir += o.devirMiktar * dk;
            tAlis += o.alisTutar;
            tSatis += o.satisTutar;
            tKapanis += mevcut * kk;
        }
        if (kmt.length)
            kmt.push({ para: "TOPLAM (TL)", kmt: "T", devir: tDevir, alis: tAlis, satis: tSatis, kapanis: tKapanis, eft: "E", sonuc: tE }, { para: "", kmt: "", devir: null, alis: null, satis: null, kapanis: null, eft: "F", sonuc: tF }, { para: "", kmt: "", devir: null, alis: null, satis: null, kapanis: null, eft: "T", sonuc: tE + tF });
        return sinirla(satirlar, t, `${aralikOzeti(p)}${ozetEk(p) || " · Tüm dövizler"}`, "Rapor altındaki tablo — KMT: Kur, Miktar, TL · EFT: Evalüasyon, Faaliyet, Toplam. Faaliyet = satış miktarı × (ortalama satış − ortalama alış); evalüasyon = devrin ve mevcudun kur farkı (alış yoksa devir × (kapanış − devir kuru)). Üstteki satırlar ağırlıklı ortalama maliyet yöntemini kullanır; iki yöntemin kârı farklı olabilir.", kmt);
    },
    /**
    ...
    async VERKOM1(pool, p, t) {
      ...
      ORDER BY ${sira}CAST(F.TARIH AS date), ISNULL(F.ZAMAN,F.TARIH), F.FIS_ID, S.SATIR_NO;`);
        const gunTr = (g) => tarihTr(new Date(g).toISOString().slice(0, 10));
        const ham = res.recordset.map((r) => {
            const bmv = Number(r.bmv) || 0, kmv = Number(r.kmv) || 0, kdv = Number(r.kdv) || 0;
        ...
        if (toplam) {
            const m = new Map();
            for (const x of ham) {
                const k = `${x.grupAnahtar}|${new Date(x.gun).toISOString().slice(0, 10)}|${x.paraKod}`;
                if (!m.has(k))
                    m.set(k, { ...x, zaman: x.gun, seriNo: "", belgeNo: "", tip: "", komisyonOrani: 0, fisler: new Set(), miktar: 0, tutar: 0, komisyon: 0, bmv: 0, kmv: 0, kdv: 0, toplamVergi: 0 });
                const o = m.get(k);
                o.fisler.add(x.fisId);
                for (const a_ of ["miktar", "tutar", "komisyon", "bmv", "kmv", "kdv", "toplamVergi"])
                    o[a_] += x[a_];
            }
    ...
    async VEZHAR1(pool, p, t) {
      ...
      ORDER BY V.KOD, X.zaman, X.belgeTipi, paraSira, X.aciklama;`);
        ...
        const satirlar = res.recordset.map((r) => {
            const id = Number(r.paraId), giris = Number(r.giris) || 0, cikis = Number(r.cikis) || 0, miktar = giris || cikis;
            ...
            if (odeme)
                return { ...r, belgeTipi: BELGE[0], yon: giris ? "<<" : ">>", tip: Number(r.fisTip) === 1 ? "Satış" : "Alış", giris, cikis, miktar: null, kur: null, tutar: 0, komisyon: 0, bmv: 0,
        ...
        const oz = new Map();
        for (const x of satirlar) {
            const k = `${x.vezneKod}|${x.paraKod}`;
            const o = oz.get(k) || { vezne: x.vezneBaslik, paraKod: x.paraKod, giris: 0, cikis: 0 };
            o.giris += x.giris;
            o.cikis += x.cikis;
            oz.set(k, o);
        }
        const ozet = [...oz.values()].map(o => ({ ...o, net: o.giris - o.cikis }));
        return sinirla(satirlar, t, `${aralikOzeti(p)} · ${(p.baslangicSaat || "00:00").slice(0, 5)}–${(p.bitisSaat || "23:59").slice(0, 5)}${ozetEk(p) || " · Tüm vezneler"}${p.fisTipi === 0 ? " · Yalnız alış fişleri" : p.fisTipi === 1 ? " · Yalnız satış fişleri" : ""} · ${kur.aciklama}`, `Vezne bakiyesini değiştiren tüm hareketler listelenir: fişler (döviz ayağı ve TL ödeme ayağı ayrı satır), vezne transferleri, nakit cari hareketler ve kasa hesap hareketleri (KDV TL'ye). << giriş, >> çıkış. İptal edilmiş fişler listelenmez. "Seçilen kur / TL" kolonları parametrede seçilen kurla hesaplanır (${kur.aciklama}).`, ozet);
    },
};
