    ...
    return new Promise((resolve, reject) => {
        ...
        if (!g.satirlar.length) {
            ...
            while (i < g.satirlar.length) {
                ...
                doc.fillColor("#000");
                hucre(doldur(grup.baslik, uyeler[0]), sol, y, genislik, "left", true, boyut + 0.5);
                y += satirH;
                for (const s of uyeler)
                    satirYaz(s);
                if (grup.altToplam !== false && kolonlar.some(k => k.toplam))
                    toplamSatiri(uyeler, `Ara toplam (${uyeler.length})`, "#f8f9fa");
            }
            if (kolonlar.some(k => k.toplam))
                toplamSatiri(g.satirlar, `GENEL TOPLAM (${g.satirlar.length} kayıt)`, "#e2e6ea");
        }
        else {
