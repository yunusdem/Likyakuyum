 ...
 */
export function raporTanimOku(kod) {
    if (!/^[A-Z0-9_]{1,20}$/.test(kod))
        throw ApiError.badRequest(`Rapor kodu geçersiz: ${kod}`);
    const tam = path.join(RAPOR_DIZINI, `${kod}.json`);
    if (!existsSync(tam))
        throw ApiError.notFound(`Rapor tanımı bulunamadı: rapor/${kod}.json`);
    let t;
    try {
        t = JSON.parse(readFileSync(tam, "utf8"));
    }
    catch (e) {
        throw ApiError.badRequest(`Rapor tanımı okunamadı (${kod}): ${e?.message || e}`);
    }
    if (!Array.isArray(t.kolonlar) || !t.kolonlar.length)
        throw ApiError.badRequest(`Rapor tanımında kolon yok: ${kod}`);
    t.kod = t.kod || kod;
    return t;
}
...
export async function raporPdf(g) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const genislikler = (ks) => { const tg = ks.reduce((t, k) => t + (k.g || 1), 0); return ks.map(k => ((k.g || 1) / tg) * genislik); };
        ...
        const hucre = (metin, x, yy, g, hiza, kalin, tabanBoyut = boyut) => {
            const f = kalin ? K() : N();
        ...
        const sayfaBasligi = () => {
            ...
            y += 6;
            zebra = 0;
            if (ozetSayfasi && g.tanim.ozet) {
                doc.fillColor("#111");
                hucre(g.tanim.ozet.baslik, sol, y, genislik, "left", true, boyut + 1.5);
                y += satirH;
            }
            kolonBasliklari();
        };
        ...
        const toplamSatiri = (satirlar, etiket, arka) => {
            const t = {};
            let etiketYazildi = false;
            for (const k of kolonlar) {
                if (k.toplam)
                    t[k.anahtar] = satirlar.reduce((s, r) => s + (Number(r[k.anahtar]) || 0), 0);
                else if (!etiketYazildi && !sayisal(k.bicim)) {
                    t[k.anahtar] = etiket;
                    etiketYazildi = true;
                }
            }
            if (!etiketYazildi)
                t[kolonlar[0].anahtar] = etiket;
        ...
        if (!g.satirlar.length) {
            ...
            while (i < g.satirlar.length) {
                ...
                const uyeler = [];
                while (i < g.satirlar.length && g.satirlar[i][grup.anahtar] === anahtar)
                    uyeler.push(g.satirlar[i++]);
                const altBaslik = grup.altBaslik ? doldur(grup.altBaslik, uyeler[0]).trim() : "";
                yeniSayfaGerekliyse(satirH * (altBaslik ? 4 : 3));
                y += 6;
                zebra = 0;
                doc.fillColor("#111");
                hucre(doldur(grup.baslik, uyeler[0]), sol, y, genislik, "left", true, boyut + 1);
                y += satirH;
                if (altBaslik) {
                    doc.fillColor("#555");
                    hucre(altBaslik, sol, y - 4, genislik, "left", false, boyut - 0.5);
                    y += satirH - 4;
                }
                doc.moveTo(sol, y - 2).lineTo(sol + genislik, y - 2).lineWidth(0.4).strokeColor("#999").stroke();
                for (const s of uyeler)
                    satirYaz(s);
                if (grup.altToplam !== false && kolonlar.some(k => k.toplam))
                    toplamSatiri(uyeler, "Ara toplam", "#eef0f3");
            }
            if (grup.genelToplam !== false && kolonlar.some(k => k.toplam)) {
                y += 6;
                toplamSatiri(g.satirlar, "Genel toplam", "#dfe3e8");
            }
        }
        else {
        ...
        if (g.tanim.ozet && g.ozetSatirlar?.length) {
            ...
            kolonlar = oz.kolonlar.filter(k => k.pdf !== false);
            kolonG = genislikler(kolonlar);
            if (y + satirH * 5 > altSinir) {
                doc.addPage();
                ozetSayfasi = true;
                sayfaBasligi();
            }
                ...
                y += 14;
                doc.fillColor("#111");
                hucre(oz.baslik, sol, y, genislik, "left", true, boyut + 1.5);
                y += satirH;
                zebra = 0;
                kolonBasliklari();
            }
            ozetSayfasi = true;
            for (const s of g.ozetSatirlar)
                satirYaz(s);
            if (kolonlar.some(k => k.toplam))
                toplamSatiri(g.ozetSatirlar, "Toplam", "#dfe3e8");
        }
