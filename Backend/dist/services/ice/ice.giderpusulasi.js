 ...
 */
export const sendGiderPusulasi = async (config, belgelerBase64) => {
 ...
 */
export const getGiderPusulasiCikti = async (config, ettn, secenekler = {}) => {
    const { data } = await callWithSession(config, {
        method: "Get_EGiderPusulasi_HTML_PDF",
        buildInnerXml: (loginHeaderXml) => `<eGiderPusulasi_HTML_PDF>` +
            loginHeaderXml +
