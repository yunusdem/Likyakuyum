...
export const IADE_BELGE_TIPLERI = ["EARSIV_FATURA", "BELGESIZ", "SATIS_FISI"];
/** `eArsiv.xsd` → `ContactTypeEnum` */
export const IADE_KANAL_TIPLERI = ["IADEKODU", "SMS"];
/* ==========================================================================
   Yardımcılar
   ========================================================================== */
...
/* ==========================================================================
   Doğrulama
   ========================================================================== */
export const dogrulaGiderPusulasi = (girdi) => {
    const belgeNo = girdi.belgeNo?.trim() || "";
    if (!/^[A-Z0-9]{3}\d{13}$/.test(belgeNo)) {
        throw ApiError.badRequest("Gider pusulası numarası 3 karakter seri + 13 hane biçiminde olmalıdır. Örnek: GIP2026000000001");
    }
    ...
    if (girdi.belgeTipi === "IADE") {
        ...
        if (!IADE_BELGE_TIPLERI.includes(dayanak.belgeTipi)) {
            throw ApiError.badRequest(`Dayanak belge tipi şunlardan biri olmalıdır: ${IADE_BELGE_TIPLERI.join(", ")}`);
        }
        if (dayanak.belgeTipi !== "BELGESIZ" && !dayanak.belgeNo?.trim()) {
            throw ApiError.badRequest("Belgeli iadelerde dayanak belge numarası zorunludur (BELGESIZ dışında).");
        }
        ...
        if (dayanak.belgeTipi === "BELGESIZ" && girdi.alici.vknTckn.trim().length !== 11) {
            throw ApiError.badRequest("Belgesiz iadede karşı taraf gerçek TCKN ile bildirilmelidir (GİB örneği).");
        }
        if (girdi.iadeKanali && !IADE_KANAL_TIPLERI.includes(girdi.iadeKanali.tip)) {
            throw ApiError.badRequest(`İade kanalı şunlardan biri olmalıdır: ${IADE_KANAL_TIPLERI.join(", ")}`);
        }
    }
};
/* ==========================================================================
   ...
   ========================================================================== */
export const hesaplaGiderPusulasi = (satirlar) => {
...
const billingReferenceXml = (dayanak) => `<cac:BillingReference><cac:InvoiceDocumentReference>` +
...
const deliveryXml = (kargo) => `<cac:Delivery><cac:DeliveryParty>` +
...
const taxSubtotalXml = (matrah, vergi, oran, paraBirimi, vergiKodu) => `<cac:TaxSubtotal>` +
    ...
    `</cac:TaxSubtotal>`;
const creditNoteLineXml = (satir, hesap, paraBirimi, vergiKodu) => `<cac:CreditNoteLine>` +
    ...
    `</cac:CreditNoteLine>`;
export const buildGiderPusulasiXml = (girdi) => {
