...
const para = (n) => yuvarla(n).toFixed(2);
...
const tag = (n, v) => v === undefined || v === null || String(v).trim() === "" ? "" : `<${n}>${escapeXml(v)}</${n}>`;
export function dogrulaMustahsil(g) {
    const no = (g.belgeNo || "").trim().toUpperCase();
    const tarih = g.tarih || bugun();
    if (!/^[A-Z0-9]{3}\d{13}$/.test(no))
        throw ApiError.badRequest("Müstahsil makbuzu numarası 3 karakter seri ve 13 rakam olmalıdır.");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(tarih) || no.slice(3, 7) !== tarih.slice(0, 4))
        throw ApiError.badRequest("Belge tarihi veya belge numarasındaki yıl geçersiz.");
    if ((g.paraBirimi || "TRY").toUpperCase() !== "TRY")
        throw ApiError.badRequest("e-Müstahsil bu sürümde yalnız TRY destekler.");
    for (const [ad, t] of [["Düzenleyen", g.gonderici], ["Üretici", g.uretici]]) {
        if (!/^\d{10}$|^\d{11}$/.test(t?.vknTckn?.trim() || ""))
            throw ApiError.badRequest(`${ad} VKN/TCKN geçersiz.`);
        if (!t.il?.trim() || !t.ilce?.trim())
            throw ApiError.badRequest(`${ad} il ve ilçe bilgisi zorunludur.`);
    }
    if (g.uretici.vknTckn.length === 11 && (!g.uretici.ad?.trim() || !g.uretici.soyad?.trim()))
        throw ApiError.badRequest("TCKN'li üreticinin adı ve soyadı zorunludur.");
    if (!g.uretici.telefon?.trim() || !g.smsKodu?.trim())
        throw ApiError.badRequest("Üretici telefonu ve SMS kodu zorunludur.");
    if (!g.smsSaglayiciAdi?.trim() || !/^\d{10}$/.test(g.smsSaglayiciVkn || ""))
        throw ApiError.badRequest("SMS sağlayıcı adı ve 10 haneli VKN'si zorunludur.");
    if (!g.satirlar?.length)
        throw ApiError.badRequest("En az bir ürün satırı zorunludur.");
    g.satirlar.forEach((s, i) => {
        if (!s.ad?.trim() || ![s.miktar, s.birimFiyat, s.stopajOrani].every(Number.isFinite) || s.miktar <= 0 || s.birimFiyat < 0 || s.stopajOrani < 0 || s.stopajOrani > 100)
            throw ApiError.badRequest(`${i + 1}. müstahsil satırı geçersiz.`);
        if (!/^\d{4}$/.test(s.stopajKodu || ""))
            throw ApiError.badRequest(`${i + 1}. satır stopaj kodu 4 rakam olmalıdır.`);
    });
}
export function hesaplaMustahsil(satirlar) {
...
const taxSub = (matrah, vergi, s) => `<cac:TaxSubtotal><cbc:TaxableAmount currencyID="TRY">${para(matrah)}</cbc:TaxableAmount><cbc:TaxAmount currencyID="TRY">${para(vergi)}</cbc:TaxAmount><cbc:CalculationSequenceNumeric>1</cbc:CalculationSequenceNumeric><cbc:Percent>${s.stopajOrani}</cbc:Percent><cac:TaxCategory><cac:TaxScheme><cbc:Name>${escapeXml(s.stopajAdi || "GELİR VERGİSİ STOPAJI")}</cbc:Name><cbc:TaxTypeCode>${s.stopajKodu}</cbc:TaxTypeCode></cac:TaxScheme></cac:TaxCategory></cac:TaxSubtotal>`;
export function buildMustahsilXml(g) {
    dogrulaMustahsil(g);
    const h = hesaplaMustahsil(g.satirlar), uuid = g.uuid?.trim() || randomUUID(), tarih = g.tarih || bugun(), saat = g.saat || simdi();
