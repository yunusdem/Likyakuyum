 ...
 */
export const assertAllowedServiceUrl = (rawUrl) => {
    let url;
    try {
        url = new URL(rawUrl);
    }
    ...
    if (url.protocol !== "https:") {
        throw ApiError.badRequest("Entegratör servis adresi https:// ile başlamalıdır. Kimlik bilgileri şifresiz bağlantıda gönderilemez.");
    }
    const host = url.hostname.toLowerCase();
    if (!host.endsWith(ALLOWED_HOST_SUFFIX)) {
        throw ApiError.badRequest(`Entegratör servis adresi yalnızca *${ALLOWED_HOST_SUFFIX} alan adında olabilir.`);
    }
    return url;
};
/**
 ...
 */
export const maskSensitive = (xml, maxLength = 2000) => {
    if (!xml)
        return "";
    let masked = xml
 ...
 */
export const buildEnvelope = (method, innerXml) => `<?xml version="1.0" encoding="utf-8"?>` +
 ...
 */
export const mapFaultToApiError = (faultCode, faultMessage) => {
    const code = (faultCode || "").toUpperCase().trim();
...
const extractFault = (body) => {
    const fault = body?.Fault;
    if (!fault)
        return null;
    const code = (typeof fault.faultcode === "string" ? fault.faultcode : fault.faultcode?.["#text"]) ||
        fault.Code?.Value ||
        "";
    const message = (typeof fault.faultstring === "string" ? fault.faultstring : fault.faultstring?.["#text"]) ||
        fault.Reason?.Text ||
        "";
 ...
 */
export const callSoap = async (options) => {
    ...
    try {
        ...
        throw new ApiError(HttpStatus.BAD_GATEWAY, `Entegratör cevabı okunamadı (${method}).`);
    }
    const body = parsed?.Envelope?.Body;
    if (!body) {
        if (!response.ok) {
            throw new ApiError(HttpStatus.BAD_GATEWAY, `Entegratör servisi HTTP ${response.status} döndü (${method}).`);
        }
        throw new ApiError(HttpStatus.BAD_GATEWAY, `Entegratör cevabında SOAP gövdesi bulunamadı (${method}).`);
    }
    ...
    return {
        data: (result !== undefined ? result : responseNode),
 ...
 */
export const buildLoginHeaderXml = (header) => `<Login_Request_Header>` +
 ...
 */
export const optionalField = (name, value, formatter) => {
...
export const toIceDateTime = (value) => {
