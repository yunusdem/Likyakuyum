...
export class OturumService {
     ...
     */
    static async dogrula(oturum, istekYolu) {
        if (!this.aktifMi())
            return;
        // Merkez açılmadan önce verilmiş (sid'siz) token'lar geçersizdir: herkes bir kez yeniden giriş yapar
        if (!oturum.sid)
            throw ApiError.unauthorized("Oturumunuzun yenilenmesi gerekiyor. Lütfen tekrar giriş yapınız.");
        ...
        const { durum } = kayit;
        if (!durum.gecerli)
            throw ApiError.unauthorized(NEDEN_MESAJI[durum.neden]);
