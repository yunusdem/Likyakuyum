...
const hesap = (id, bankId, doviz, no, iban, bakiye, sube, subeKodu, urun) => ({
    id,
    bank_id: bankId,
    branch_name: sube,
    fec_name: doviz,
...
const iban = (id) => HESAPLAR.find((h) => h.id === id).iban;
const SABLONLAR = [
...
const gunMetni = (d) => `${d.getUTCFullYear()}-${iki(d.getUTCMonth() + 1)}-${iki(d.getUTCDate())}`;
/** "GG-AA-YYYY[ SS:DD:ss]" → UTC ms (duvar saati UTC gibi işlenir; yalnızca karşılaştırma içindir) */
const vomsisTarihi = (metin, gunSonu) => {
...
const hareketUret = (s, sira, gunNo) => {
...
const hareketler = (sorgu, hesapId) => {
    const bas = vomsisTarihi(sorgu.beginDate, false);
    const bit = vomsisTarihi(sorgu.endDate, true);
    if (bas === null || bit === null)
        return { status: "error", message: "beginDate ve endDate zorunludur (GG-AA-YYYY SS:DD:ss)." };
    ...
    for (let gunNo = Math.floor(bas / GUN_MS); gunNo <= sonGun; gunNo++) {
        ...
        SABLONLAR.forEach((s, i) => {
            if (s.herGun && gunNo % s.herGun !== 0)
                return;
            if (hesapId && s.hesapId !== hesapId)
                return;
            if (tipler && !tipler.includes(s.tip))
                return;
            const h = hareketUret(s, i + 1, gunNo);
            const an = Date.parse(h.system_date.replace(" ", "T") + "Z");
            if (an < bas || an > bit)
                return;
            if (sorgu.lastId && h.id <= Number(sorgu.lastId))
                return;
            if (sorgu.type && h.type !== sorgu.type)
                return;
            if (sorgu.bankName && h.account.bank.bank_name !== sorgu.bankName)
                return;
            liste.push(h);
        });
    }
...
const noktaliGun = (d) => `${iki(d.getUTCDate())}.${iki(d.getUTCMonth() + 1)}.${d.getUTCFullYear()}`;
/** Vomsis POS sorgusunda tarih aralığı en fazla 14 gündür. */
const posHareketleri = (terminalId, sorgu) => {
    const terminal = TERMINALLER.find((t) => t.id === terminalId);
    if (!terminal)
        return { status: "error", message: "Terminal bulunamadı." };
    const bas = vomsisTarihi(sorgu.beginDate, false);
    const bit = vomsisTarihi(sorgu.endDate, true);
    if (bas === null || bit === null)
        return { status: "error", message: "beginDate ve endDate zorunludur (GG-AA-YYYY)." };
    if (bit - bas > 14 * GUN_MS)
        return { status: "error", message: "Tarih aralığı en fazla 14 gün olabilir." };
    const sonGun = Math.min(Math.floor(bit / GUN_MS), Math.floor(Date.now() / GUN_MS));
    ...
    for (let gunNo = Math.floor(bas / GUN_MS); gunNo <= sonGun; gunNo++) {
        ...
        for (let i = 1; i <= adet; i++) {
            ...
            if (sorgu.lastId && id <= Number(sorgu.lastId))
                continue;
...
const sahteLinkler = new Map();
const sahteIadeler = new Map();
let sahteSayac = 0;
...
const linkOdendiMi = (l) => Date.now() - l.olusturma >= SAHTE_ODEME_GECIKMESI_MS;
// uid'nin değişen kısmı sonundadır; referans ondan türetilir ki linkler arasında çakışmasın
const linkReferansi = (l) => `68VL${l.uid.replace(/-/g, "").slice(-16).toUpperCase()}`;
const tamZaman = (ms) => new Date(ms).toISOString().slice(0, 19).replace("T", " ");
const linkDetayi = (l) => ({
    uid: l.uid,
    title: l.baslik,
...
const sahteKartOdemeleri = new Map();
const taksitSecenekleri = (oranlar) => oranlar.map((rate, i) => ({ installment: i === 0 ? 1 : i + 1, title: i === 0 ? "Tek Çekim" : `${i + 1} Taksit`, status: "true", rate, extra_installment: 0, discount_rate: 0 }));
const kartIslemi = (k, sira) => {
...
const linkIslemleri = () => [...sahteLinkler.values()].filter(linkOdendiMi).map((l, i) => {
...
const vposYanitlari = {
    ...
    "POST /bin-check": (_s, govde) => {
        const bin = String(govde?.cc_number ?? "");
        if (!/^\d{6}$/.test(bin))
            return { success: false, message: "BIN 6 hane olmalıdır." };
    ...
    "POST /payment": (_s, govde) => {
        const g = (govde || {});
        if (!g.referanceNo || !(Number(g.amount) > 0) || !g.returnUrl)
            return { success: false, message: "referanceNo, amount ve returnUrl zorunludur." };
        ...
        return {
            htmlContent: `<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>Örnek 3D Secure</title></head>` +
    ...
    "POST /request-payment": (_s, govde) => {
        ...
        if (!(Number(g.amount) > 0))
            return { success: false, message: "Tutar 0'dan büyük olmalı." };
        sahteSayac++;
    ...
    "POST /transaction": (_s, govde) => {
        ...
        if (!i)
            return { transId: null, response: "Error", proc_return_code: "99", error_code: "NOT_FOUND", error_message: "İşlem bulunamadı." };
        if (sahteIadeler.has(i.referans_kodu))
            return { transId: null, response: "Declined", proc_return_code: "05", error_code: "ALREADY", error_message: "İşlem daha önce iptal/iade edilmiş." };
        const tutar = g.transactionType === "refund" ? Number(g.amount) : Number(i.tutar);
        if (!(tutar > 0) || tutar > Number(i.tutar))
            return { transId: null, response: "Declined", proc_return_code: "05", error_code: "AMOUNT", error_message: "İade tutarı geçersiz." };
...
export const sahteYanit = (servis, metod, yol, sorgu = {}, govde) => {
    ...
    if (yanit)
        return yanit(sorgu, govde);
    // Yol parametreli uçlar
    const pos = /^\/pos-rapor\/stations\/(\d+)\/transactions$/.exec(temizYol);
    if (servis === "banka" && metod.toUpperCase() === "GET" && pos)
        return posHareketleri(Number(pos[1]), sorgu);
