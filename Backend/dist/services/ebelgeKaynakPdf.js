...
const trSayi = (n, basamak = 2) => n.toLocaleString('tr-TR', { minimumFractionDigits: basamak, maximumFractionDigits: basamak });
/** Tutarı ICE belgesindeki gibi yazıyla verir: "YÜZKIRKBEŞ TL ELLİİKİ KR." */
export function tutarYaziyla(tutar, paraAdi = 'TL', kurusAdi = 'KR.') {
 ...
 */
export function eDovizBelgePdf(g, ek = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (ek.onizleme) {
            y += 12;
            doc.fillColor('#8a1c1c').text('ÖNİZLEME — ICE onaylı belge değildir', sol, y, { width: genislik, align: 'center' });
            doc.fillColor('#000');
        }
        y += 20;
        ...
        y += 10;
        N().fontSize(8).text(`Yalnız: ${tutarYaziyla(t.taxInclusiveAmount)}`, sol, y, { width: genislik });
        y += 12;
        doc.text(`VEZNE : ${g.ekBilgiler?.vezne || ''}`, sol, y, { width: genislik });
        y += 12;
        doc.text(`Uyruk : ${g.musteri.ulke || ''}`, sol, y, { width: genislik });
        y += 16;
        if (ek.hesapVkn && g.uuid) {
            doc.fillColor('#333').text(`e-Döviz ve Kıymetli Maden belgesine http://ebelge.iceteknoloji.com.tr/edoviz/ettn/${ek.hesapVkn}/${g.uuid} adresinden erişebilirsiniz.`, sol, y, { width: genislik });
        }
        doc.end();
    });
}
