...
export const hataMetni = (err) => {
    ...
    if (err?.code === "ETIMEOUT" || msg.includes("timeout"))
    ...
    if (msg.includes("Cannot open database"))
        return "Veritabanı bulunamadı veya kullanıcının erişimi yok.";
 ...
 */
export const baglantiSina = async (dbServer, dbName, dbUser, dbSifre) => {
    const { host, port } = parseServerAndPort(dbServer, 1433);
 ...
 */
export const firmaDbIleCalistir = async (firmaId, is) => {
    const b = await FirmaSqlRepository.baglantiBilgisi(firmaId);
    if (!b)
        throw ApiError.notFound("Firma bulunamadı.");
    if (!b.dbUser)
        throw new Error("Veritabanı kullanıcı adı tanımlı değil.");
    ...
    if (!sifre)
        throw new Error("Veritabanı şifresi tanımlı değil.");
 ...
 */
export const firmaDbContextHazirla = async (firmaId) => {
    try {
...
export class FirmaBaglantiService {
    ...
    static async masakKontrol(firmaId) {
        let sonuc;
        try {
            sonuc = await firmaDbIleCalistir(firmaId, async (pool) => {
                const res = await pool.request().query(`
        ...
        `);
                const r = res.recordset[0];
                const parcalar = [];
                parcalar.push(r.HESAP === null ? "MASAK hesap alanı yok" : r.HESAP > 0 ? `MASAK hesabı tanımlı kullanıcı: ${r.HESAP}` : "MASAK hesabı tanımlı değil");
                if (r.KAYIT === null)
                    parcalar.push("liste tablosu yok");
                else {
                    const tarih = r.SON_GUNCELLEME ? r.SON_GUNCELLEME.toISOString().slice(0, 10) : "hiç";
