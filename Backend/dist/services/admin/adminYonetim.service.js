...
export class AdminYonetimService {
    ...
    static async ekle(yapan, girdi) {
        ...
        try {
            ...
            if (benzersizIhlalMi(err))
                throw ApiError.conflict("Bu kullanıcı adıyla bir admin zaten var.");
            throw err;
        }
        if (adminId === null) {
            throw ApiError.conflict(`En fazla ${AZAMI_AKTIF_ADMIN} aktif admin olabilir. Yeni admin açmak için önce birini pasife alın.`);
        }
    ...
    static async guncelle(yapan, adminId, girdi) {
        ...
        if (girdi.durum !== undefined && girdi.durum !== hedef.durum) {
            ...
            if (sonuc === "SON_AKTIF")
                throw ApiError.conflict("Son aktif admin pasife alınamaz.");
    ...
    static async sifreSifirla(yapan, adminId) {
        if (adminId === yapan.adminId) {
