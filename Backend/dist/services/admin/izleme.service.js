...
export class IzlemeService {
    static async cevrimici() {
        return { dakika: CEVRIMICI_DAKIKA, oturumlar: await IzlemeSqlRepository.cevrimici(CEVRIMICI_DAKIKA) };
    }
    static girisLoglari = IzlemeSqlRepository.girisLoglari;
    static islemLoglari = IzlemeSqlRepository.islemLoglari;
    /** Tek bir oturumu kapatır; kullanıcı bir sonraki isteğinde giriş ekranına düşer. */
    static async oturumuKapat(yapan, sid) {
        if (sid.toLowerCase() === yapan.sid.toLowerCase())
            throw ApiError.badRequest("Kendi oturumunuzu 'Çıkış' ile kapatın.");
        let kapanan = await IzlemeSqlRepository.oturumlariIptalEt({ sid }, yapan.adminId);
        if (kapanan === 0)
            kapanan = await AdminOturumSqlRepository.sidIleIptalEt(sid, yapan.adminId);
        if (kapanan === 0)
