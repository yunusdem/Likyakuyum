...
export const MASAK_KAYNAKLAR = {
    A: {
        ...
        varsayilanUrl: "https://ms.hmb.gov.tr/uploads/sites/12/2026/08/A-BIRLESMIS-MILLETLER-GUVENLIK-KONSEYI-KARARINA-ISTINADEN-MALVARLIKLARI-DONDURULANLAR-6415-SAYILI-KANUN-5.-MADDE-27.08-40e72586a1281a0e.xlsx",
    },
    B: {
        listeKod: "B",
        listeAdi: "Yabancı Ülke Taleplerine İstinaden Malvarlıkları Dondurulanlar (6415 S.K. m.6)",
        varsayilanUrl: "https://ms.hmb.gov.tr/uploads/sites/12/2026/01/B-YABANCI-ULKE-TALEPLERINE-ISTINADEN-MALVARLIKLARI-DONDURULANLAR-6415-SAYILI-KANUN-6.-MADDE-0cb2484e46e910b1.xlsx",
    },
    C: {
        listeKod: "C",
        listeAdi: "İç Dondurma Kararı ile Malvarlıkları Dondurulanlar (6415 S.K. m.7)",
        varsayilanUrl: "https://ms.hmb.gov.tr/uploads/sites/12/2026/07/C-IC-DONDURMA-KARARI-ILE-MALVARLIKLARI-DONDURULANLAR-6415-SAYILI-KANUN-7.-MADDE-G-6e6f23b75ed73de9.xlsx",
    },
    "3AB": {
        listeKod: "3AB",
        listeAdi: "7262 Sayılı Kanun 3.A ve 3.B Maddeleri Kapsamında Dondurulanlar",
        varsayilanUrl: "https://ms.hmb.gov.tr/uploads/sites/12/2026/07/D-7262-SAYILI-KANUN-3.A-VE-3.B-MADDELERI-EXCEL-29.07.2026-7065dd0684b9962c.xlsx",
    },
};
/* ============================================================================
   Metin yardımcıları
   ========================================================================== */
const TR_HARF_ESLEME = {
    İ: "I",
    I: "I",
    Ş: "S",
    Ğ: "G",
 ...
 */
export const normalizeMetin = (value) => {
...
export const tcknGecerliMi = (value) => {
 ...
 */
export const tarihAyikla = (deger) => {
...
const kayitTipiTahmin = (adNorm, tckn) => {
    if (tckn)
        return "GERCEK";
    const ad = ` ${adNorm} `;
...
export const adresGecerliMi = (url) => {
    if (!url)
        return false;
    const u = url.trim();
    if (!u.toLowerCase().startsWith(MASAK_IZINLI_ONEK))
        return false;
 ...
 */
export const indirDosya = async (url, opts = {}) => {
    if (!adresGecerliMi(url)) {
        throw ApiError.badRequest("Geçersiz kaynak adresi. Yalnızca https://ms.hmb.gov.tr/ ile başlayan .xlsx adresleri kabul edilir.");
    }
    ...
    const mesaj = sonHata?.name === "TimeoutError"
        ? "Dosya indirme zaman aşımına uğradı. İnternet bağlantınızı kontrol ediniz."
        : sonHata?.message || "Dosya indirilemedi.";
...
const BASLIK_ESLEME = [
    ...
    {
        alan: "kimlikNo",
        kalip: (b) => b.includes("TCKN") || b.includes("PASAPORT") || b.includes("KIMLIK NO") || b.includes("VKN"),
    },
    {
        alan: "adUnvan",
        kalip: (b) => b.includes("AD SOYAD") ||
...
const hucreMetni = (deger) => {
    ...
    if (typeof deger === "object") {
        ...
        if (Array.isArray(nesne.richText)) {
            return temizle(nesne.richText.map((p) => p.text ?? "").join(""));
        }
        if (nesne.text !== undefined)
            return temizle(nesne.text);
        if (nesne.result !== undefined)
            return temizle(nesne.result);
        if (nesne.hyperlink !== undefined)
            return temizle(nesne.hyperlink);
        if (nesne.error !== undefined)
            return null;
 ...
 */
export const parseMasakExcel = async (buffer, listeKod, listeAdi) => {
    const workbook = new ExcelJS.Workbook();
    try {
        await workbook.xlsx.load(buffer);
    }
    catch (error) {
        throw ApiError.badRequest(`Excel dosyası okunamadı: ${error?.message || "biçim tanınmadı"}. Adresin doğru dosyayı gösterdiğinden emin olunuz.`);
    }
    ...
    if (baslikIndex < 0 || enIyiSkor < 3) {
        throw ApiError.badRequest("Excel dosyasında beklenen başlık satırı bulunamadı. Dosya biçimi değişmiş olabilir.");
    }
    // --- 3) Kolon eşlemesi
    const basliklar = satirlar[baslikIndex].metin;
    const alanKolon = new Map();
    const eslesmeyen = [];
    ...
    for (let i = baslikIndex + 1; i < satirlar.length; i++) {
        ...
        const dogumTarihiDt = dogumTarihiIdx !== undefined
        ...
        kayitlar.push({
            ...
            yaptirimTuru: alanDeger(satir, "yaptirimTuru") ??
    ...
    if (kayitlar.length === 0) {
        throw ApiError.badRequest("Excel dosyasında kayıt bulunamadı. Dosya biçimi değişmiş olabilir.");
    }
    return {
        kayitlar,
