...
export class EtiketLogoSqlRepository {
    ...
    static async listLogos(tip = ETIKET_LOGO_URUN_TIPI, dbContext) {
    ...
    static async saveLogo(data, kullaniciId, tip = ETIKET_LOGO_URUN_TIPI, urunId = 0, dbContext) {
        if (!data.base64) {
    ...
    static async deleteLogo(fotografId, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTables(pool);
        try {
