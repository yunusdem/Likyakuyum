...
export class EbelgeKnskRepository {
    ...
    static satir = (k) => ({ ...k, kalanGun: Number(k.kalanGun) });
    static async get(vknTckn, ctx) {
    ...
    static async list(yalnizYaklasan, ctx) {
        const pool = await this.pool(ctx);
     ...
     */
    static async onayla(k, kullanici, ctx) {
        const pool = await this.pool(ctx);
    ...
    static async kaldir(vknTckn, kullanici, ctx) {
        const pool = await this.pool(ctx);
