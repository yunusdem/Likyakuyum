...
export class KasaHesapSqlRepository {
    ...
    `;
        ...
        if (!savedId)
            throw ApiError.internal("Hesap kartı kaydedildi ancak kimlik bilgisi alınamadı.");
        const saved = await this.getHesapById(savedId, dbContext);
        if (!saved)
            throw ApiError.internal("Hesap kartı kaydedildi ancak okunamadı.");
        return saved;
    }
    ...
    static async saveHareket(dto, kullaniciId = 1, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTables(pool);
        await this.ensureProcedures(pool);
    ...
    `;
        ...
        if (!savedId)
            throw ApiError.internal("Hesap hareketi kaydedildi ancak kimlik bilgisi alınamadı.");
        const saved = await this.getHareketById(savedId, dbContext);
        if (!saved)
            throw ApiError.internal("Hesap hareketi kaydedildi ancak okunamadı.");
        return saved;
    }
    static async deleteHareket(hesapHareketiId, kullaniciId = 1, degisiklikTakipVar = false, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTables(pool);
        await this.ensureProcedures(pool);
        try {
