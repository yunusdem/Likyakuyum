...
export class ModulSqlRepository {
     ...
     */
    static async katalogEsitle(moduller) {
        const pool = await getAdminPool();
        const tx = new sql.Transaction(pool);
        await tx.begin();
        try {
            const mevcut = new Set((await new sql.Request(tx).query(`SELECT MODUL_KODU FROM dbo.ADM_MODUL WITH (UPDLOCK, HOLDLOCK)`)).recordset.map((r) => r.MODUL_KODU));
            let eklenen = 0;
     ...
     */
    static async firmaAcikModulleri(firmaId) {
        const pool = await getAdminPool();
        ...
        if (res.recordset.length === 0)
            return null;
        return res.recordset.filter((r) => !!r.ACIK).map((r) => r.MODUL_KODU);
    }
    /** Firmanın modül ayarını katalogdaki tüm kodlar için yazar: acikKodlar içindekiler açık, diğerleri kapalı. */
    static async firmaModulleriniYaz(firmaId, acikKodlar, adminId) {
        ...
        if (kodlar.length > 2000)
            throw new Error("Modül sayısı tek istekte yazılabilecek sınırı aşıyor.");
        const pool = await getAdminPool();
    ...
    static async firmaModulleriniSifirla(firmaId) {
        const pool = await getAdminPool();
