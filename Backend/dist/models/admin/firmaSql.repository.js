...
export const gunYaz = (d) => d.toISOString().slice(0, 10);
const lisansDurumu = (lisansId, kalanGun) => {
    if (lisansId === null || kalanGun === null)
        return "YOK";
    if (kalanGun < 0)
        return "BITMIS"; // bitiş günü dahil geçerlidir
...
const yazimGirdileri = (req, v) => req
...
export class FirmaSqlRepository {
    ...
    static async anahtarIleBul(dbAnahtar) {
        const pool = await getAdminPool();
    ...
    static async guncelle(firmaId, v, dbSifreEnc) {
        const pool = await getAdminPool();
        const req = yazimGirdileri(pool.request(), v).input("id", sql.Int, firmaId);
        if (dbSifreEnc !== undefined)
            req.input("dbSifreEnc", sql.VarChar(600), dbSifreEnc);
        await req.query(`
    ...
    static async durumDegistir(firmaId, durum, not) {
        const pool = await getAdminPool();
        await pool
            .request()
    ...
    static async dogrulamaYaz(firmaId, dogrulandi, adminId, not) {
        const pool = await getAdminPool();
        await pool
            .request()
            .input("id", sql.Int, firmaId)
    ...
    static async baglantiBilgisi(firmaId) {
        const pool = await getAdminPool();
    ...
    // ------------------------------------------------------------ e-posta doğrulaması
    static async epostaSonGonderimdenBeriSn(firmaId) {
        const pool = await getAdminPool();
        const res = await pool
            .request()
            .input("id", sql.Int, firmaId)
    ...
    static async epostaAnahtariYaz(firmaId, anahtarOzeti, gecerlilikSaat) {
        const pool = await getAdminPool();
        await pool
            .request()
            .input("id", sql.Int, firmaId)
     ...
     */
    static async epostaAnahtariylaDogrula(anahtarOzeti) {
        const pool = await getAdminPool();
    ...
    static async epostaDogrulamaElleYaz(firmaId, dogrulandi) {
        const pool = await getAdminPool();
    ...
    static async dbTestSonucuYaz(firmaId, sonuc) {
        const pool = await getAdminPool();
        await pool
            .request()
            .input("id", sql.Int, firmaId)
    ...
    static async masakDurumuYaz(firmaId, durum) {
        const pool = await getAdminPool();
        await pool
            .request()
            .input("id", sql.Int, firmaId)
