...
export class KullaniciSqlRepository {
    ...
    static async idIleBul(kullaniciId) {
        const pool = await getAdminPool();
    ...
    static async adIleBul(firmaId, kullaniciAdi) {
        const pool = await getAdminPool();
        const res = await pool
            .request()
            .input("firmaId", sql.Int, firmaId)
     ...
     */
    static async ekle(veri, limitUygula) {
        const pool = await getAdminPool();
        const res = await pool
            .request()
    ...
    static async durumDegistir(kullaniciId, durum) {
        const pool = await getAdminPool();
    ...
    static async bilgiGuncelle(kullaniciId, veri) {
        const pool = await getAdminPool();
        await pool
            .request()
    ...
    static async kullaniciAdiGuncelle(kullaniciId, kullaniciAdi) {
        const pool = await getAdminPool();
        await pool
            .request()
            .input("id", sql.Int, kullaniciId)
    ...
    static async sifreGuncelle(kullaniciId, sifreHash, sifreDegismeli) {
        const pool = await getAdminPool();
        await pool
            .request()
            .input("id", sql.Int, kullaniciId)
    ...
    static async girisSonucuYaz(kullaniciId, basarili) {
        const pool = await getAdminPool();
