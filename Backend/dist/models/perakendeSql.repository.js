...
export class PerakendeSqlRepository {
     ...
     */
    static async ensureTablesAndProcedures(pool) {
        try {
    ...
    `;
        ...
        if (row.SATILDI === true || row.SATILDI === 1) {
            throw ApiError.badRequest(`'${cleanBarcode}' barkodlu ürün (${row.MODEL || row.GRUP_KODU || "Altın Ürün"}) daha önce satılmıştır ve stokta mevcut değildir.`);
        }
        return {
    ...
    static async createInvoice(dto, userId, dbContext) {
      ...
      `);
            ...
            if (!result)
                throw new Error("Oluşturulan fatura getirilemedi.");
            return result;
        }
        catch (err) {
            await transaction.rollback().catch(() => { });
            const detailedMessage = err.originalError?.info?.message || err.originalError?.message || err.message || "Fatura kaydedilemedi.";
     ...
     */
    static async getInvoiceById(faturaId, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTablesAndProcedures(pool);
    ...
    `);
        let satirlar = (linesRes.recordset || []).map((s) => ({
     ...
     */
    static async listInvoices(filter, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTablesAndProcedures(pool);
        const req = pool.request();
        let query = `
     ...
     */
    static async deleteInvoice(faturaId, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTablesAndProcedures(pool);
        try {
            const req = pool.request();
            ...
            const detailedMessage = err.originalError?.info?.message || err.originalError?.message || err.message || "Fatura silinemedi.";
