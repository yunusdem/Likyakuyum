...
export class AltinUrunSqlRepository {
    ...
    static async list(filter, dbContext) {
        ...
        if (items.length > 0) {
            try {
                ...
                if (ids.length > 0) {
                    const resimRes = await pool.request().query(`SELECT ISLEM_ID, DOSYA_YOLU, RESIM_DATA FROM TODVZ_URUN_RESIM WHERE TIP = 0 AND ISLEM_ID IN (${ids.join(",")}) ORDER BY RESIM_ID ASC`);
    ...
    static async remove(altinUrunId, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTables(pool);
        await this.ensureProcedures(pool);
        try {
