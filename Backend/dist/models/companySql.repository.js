      ...
      `);
            ...
            for (const col of columnMapping) {
                ...
                if (col.len && (col.type === sql.VarChar || col.type === sql.Char)) {
                    request.input(col.name, col.type(col.len), val !== undefined ? val : null);
                }
                else {
                    request.input(col.name, col.type, val !== undefined ? val : null);
                }
            }
        ...
        `;
                ...
                // INSERT row
                const colNames = columnMapping.map((col) => `[${col.name}]`).join(", ");
                const paramNames = columnMapping.map((col) => `@${col.name}`).join(", ");
                const insertQuery = `
