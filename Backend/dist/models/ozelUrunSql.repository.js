    ...
    static async list(filter, dbContext) {
        ...
        if (items.length > 0) {
            try {
                ...
                if (ids.length > 0) {
                    const resimRes = await pool.request().query(`SELECT ISLEM_ID, DOSYA_YOLU, RESIM_DATA FROM TODVZ_URUN_RESIM WHERE TIP = 1 AND ISLEM_ID IN (${ids.join(",")}) ORDER BY RESIM_ID ASC`);
