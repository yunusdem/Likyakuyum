...
export class MasakSqlRepository {
     ...
     */
    static async knskLogYaz(kayit, dbContext) {
        try {
            const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        ...
        END TRY BEGIN CATCH IF ERROR_NUMBER() <> 2714 THROW; END CATCH;`);
            ...
            logger.warn("MASAK: KNSK sorgulama logu yazılamadı (sorgu sonucu etkilenmedi):", e);
        }
    }
    static async sorgula(params, dbContext) {
        ...
        req.input("kimlikLike", sql.NVarChar(1000), kimlik ? `%${MasakSqlRepository.likeKacir(kimlik)}%` : null);
