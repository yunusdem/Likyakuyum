...
export class VezneTransferiSqlRepository {
    ...
    static async ensureProcedures(pool) {
      ...
      `);
            const existingNames = new Set((checkRes.recordset || []).map((r) => r.name));
     ...
     */
    static async deleteViaProcedure(id, kullaniciId = 1, degisiklikTakipVar = false, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await VezneTransferiSqlRepository.ensureProcedures(pool);
        try {
            const request = pool.request();
    ...
    static async findById(id, dbContext) {
      ...
      `);
            return {
                ...
                satirlar: (linesRes.recordset || []).map((r) => ({
                    satirNo: r.satirNo,
                    paraId: r.paraId,
