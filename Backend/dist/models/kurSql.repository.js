...
export class KurSqlRepository {
     ...
     */
    static async findTablo(params, dbContext) {
        ...
        if (id) {
            const res = await pool
                .request()
                .input("id", sql.Int, id)
                .query("SELECT TOP 1 * FROM [dbo].[TODVZ_KUR_TABLOSU] WHERE [KUR_TABLOSU_ID] = @id");
            ...
            const res = await pool
                ...
                .query("SELECT TOP 1 * FROM [dbo].[TODVZ_KUR_TABLOSU] WHERE [TUR] = @tur ORDER BY [KUR_TABLOSU_ID] DESC");
            if (res.recordset.length > 0) {
                header = res.recordset[0];
            }
            else {
                ...
                const createdRes = await pool
                    ...
                    .query("SELECT TOP 1 * FROM [dbo].[TODVZ_KUR_TABLOSU] WHERE [KUR_TABLOSU_ID] = @id");
                if (createdRes.recordset.length > 0)
                    header = createdRes.recordset[0];
            }
        }
        else {
    ...
    `;
        const ratesRes = await pool
            ...
            .query(ratesQuery);
        const satirlar = ratesRes.recordset.map((r) => ({
            paraId: r.PARA_ID,
            kod: r.PARA_KOD || "",
            ad: r.PARA_AD || "",
        ...
        let kapanisId = null;
        if (tur === 0) {
            const kapRes = await pool.request().query(`
    ...
    static async saveViaProcedure(dto, dbContext) {
        ...
        if (satirlar && satirlar.length > 0) {
            ...
            const uniqueSatirlar = Array.from(new Map(satirlar.filter((s) => s && s.paraId).map((s) => [s.paraId, s])).values());
     ...
     */
    static async sakla(kaynakKurTablosuId, targetTur = 2, zaman, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await KurSqlRepository.ensureTablesAndProceduresExist(pool);
        ...
        procReq.output("KAPANIS_KUR_TABLOSU_ID", sql.Int);
        await procReq.execute("SODVZ_KUR_TABLOSU_KAYDET");
        const newId = procReq.parameters.KUR_TABLOSU_ID.value || existingId;
     ...
     */
    static async deleteKurTablosu(id, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await KurSqlRepository.ensureTablesAndProceduresExist(pool);
