...
export class SarrafFisSqlRepository {
    ...
    static async getUrunler(dbContext) {
        try {
            const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    ...
    static async getFisList(filters, dbContext) {
        try {
            const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
            let query = `
        SELECT TOP (@limit)
        ...
        WHERE 1=1`;
            ...
            if (filters.vezneId && filters.vezneId > 0) {
                query += " AND SF.VEZNE_ID = @vezneId";
                req.input("vezneId", sql.Int, filters.vezneId);
            }
            if (filters.tip !== undefined && filters.tip !== null) {
                query += " AND SF.TIP = @tip";
                req.input("tip", sql.TinyInt, filters.tip);
            }
            if (filters.search) {
                query += " AND (SF.FIS_NO LIKE @search OR SF.UNVAN LIKE @search)";
                req.input("search", sql.VarChar(200), `%${filters.search}%`);
            }
    ...
    static async getFisById(sarrafFisiId, dbContext) {
        try {
            const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
            ...
            return {
                ...
                pasaportNo: h.PASAPORT_NO ? h.PASAPORT_NO.trim() : null,
                hukukiYapiId: h.HUKUKI_YAPI_ID ?? null, vergiDairesiId: h.VERGI_DAIRESI_ID ?? null,
                vergiKimlikNo: h.VERGI_KIMLIK_NO ? h.VERGI_KIMLIK_NO.trim() : null,
                ...
                vekilAdi: h.VEKIL_ADI || null, vekilKimlikNo: h.VEKIL_KIMLIK_NO ? h.VEKIL_KIMLIK_NO.trim() : null,
                ...
                dogumYeri: h.DOGUM_YERI || null, kimlikSeriNo: h.KIMLIK_SERI_NO ? h.KIMLIK_SERI_NO.trim() : null,
    ...
    static async saveDetay(dto, dbContext) {
        try {
            const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
            const req = pool.request();
