...
export class BankoSqlRepository {
    ...
    static async getById(bankoId, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTables(pool);
        await this.ensureProcedures(pool);
    ...
    static async save(dto, kullaniciId, dbContext) {
        ...
        if (!savedId)
            throw ApiError.internal("Banko kaydedildi ancak ID alınamadı.");
        const saved = await this.getById(savedId, dbContext);
        if (!saved)
            throw ApiError.internal("Banko kaydedildi ancak okunamadı.");
        return saved;
    }
    static async remove(bankoId, dbContext) {
        const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
        await this.ensureTables(pool);
        await this.ensureProcedures(pool);
        try {
