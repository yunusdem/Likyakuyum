...
import { EBankaSqlRepository } from "./ebankaSql.repository.js";
...
const varsayilanKural = (tipKodu) => (AKTARMA_TIPLERI.includes(tipKodu) ? 2 : ELLE_TIPLERI.includes(tipKodu) ? 1 : 0);
const sayi = (v) => {
...
const etiketMetni = (tags) => {
    if (!Array.isArray(tags))
        return null;
    const adlar = tags.map((t) => (typeof t === "string" ? t : t?.tag?.name || t?.name)).filter(Boolean);
...
export class EBankaVeriSqlRepository {
    ...
    // ─── Eşitleme yazımları ────────────────────────────────────────────────────
    static async bankalariYaz(bankalar, dbContext) {
    ...
    static async hesaplariYaz(hesaplar, dbContext) {
        const pool = await this.pool(dbContext);
     ...
     */
    static async ibanIleEsle(dbContext) {
        const pool = await this.pool(dbContext);
    ...
    static async hareketTipleriniYaz(tipler, dbContext) {
        const pool = await this.pool(dbContext);
    ...
    static async hareketleriYaz(hareketler, dbContext) {
        const pool = await this.pool(dbContext);
        let yeni = 0;
        let guncellenen = 0;
    ...
    static async sonEsitlemedenBeriSaniye(dbContext) {
        const pool = await this.pool(dbContext);
    ...
    static async hesapEsle(vomsisHesapId, bankaId, dbContext) {
        ...
        if (bankaId !== null) {
            ...
            if (!(await kart.query(`SELECT 1 AS X FROM TODVZ_BANKA WHERE BANKA_ID = @BANKA_ID`)).recordset.length)
                return "kart-yok";
            ...
            if ((await dolu.query(`SELECT 1 AS X FROM TODVZ_EBANKA_HESAP WHERE BANKA_ID = @BANKA_ID AND VOMSIS_HESAP_ID <> @ID`)).recordset.length)
                return "kart-dolu";
        }
        const req = pool.request();
    ...
    static async hareketTipleriniListele(dbContext) {
        const pool = await this.pool(dbContext);
        const rows = (await pool.request().query(`SELECT TIP_KODU, TIP_ADI FROM TODVZ_EBANKA_HAREKET_TIPI ORDER BY TIP_ADI`)).recordset;
        return rows.map((r) => ({ tipKodu: r.TIP_KODU, tipAdi: r.TIP_ADI }));
    }
    static hareketSatiri(r) {
        return {
  ...
  `;
    static async hareketleriListele(f, dbContext) {
      ...
      `)).recordset;
        return {
            satirlar: rows.map((r) => this.hareketSatiri(r)),
            toplam: ozet.reduce((t, r) => t + Number(r.ADET), 0),
            toplamlar: ozet.map((r) => ({ doviz: r.DOVIZ, giris: Number(r.GIRIS) || 0, cikis: Number(r.CIKIS) || 0, adet: Number(r.ADET) })),
        };
    }
    ...
    static async ozetSayilari(dbContext) {
        const pool = await this.pool(dbContext);
        const r = (await pool.request().query(`
