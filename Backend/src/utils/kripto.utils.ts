...
const anahtar = (): Buffer => crypto.createHash("sha256").update(env.ADMIN_DB_ENC_KEY, "utf8").digest();

export const sifrele = (duzMetin: string): string => {
...
export const sifreCoz = (saklanan: string | null | undefined): string | null => {
  if (!saklanan) return null;
  const [surum, iv, etiket, sifreli] = saklanan.split(":");
  if (surum !== SURUM || !iv || !etiket || sifreli === undefined) return null;
  try {
