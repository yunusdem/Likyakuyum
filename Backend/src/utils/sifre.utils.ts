...
export const sifreKuralHatasi = (sifre: string): string | null => {
...
export const sifreHashle = (sifre: string): Promise<string> => bcrypt.hash(sifre, BCRYPT_TUR);

export const sifreDogrula = async (sifre: string, hash: string): Promise<boolean> => {
  if (!sifre || !hash) return false;
  try {
...
export const sahteSifreDogrula = async (sifre: string): Promise<void> => {
