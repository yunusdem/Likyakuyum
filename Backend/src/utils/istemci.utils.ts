...
const portuAt = (deger: string): string => {
  const v = deger.trim();
 ...
 */
export const istemciIp = (req: Request): string => {
  ...
  if (LOOPBACK.has(soket)) {
    ...
    if (son && son.trim()) return portuAt(son).slice(0, 64);
  }
  return soket.replace(/^::ffff:/, "").slice(0, 64);
};

export const istemciTarayici = (req: Request): string =>
