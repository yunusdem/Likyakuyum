 ...
 */
export const isEncryptionConfigured = (): boolean => readKey() !== null;

const requireKey = (): Buffer => {
  const key = readKey();
  if (!key) {
    throw ApiError.internal(
      `${KEY_ENV_NAME} ortam değişkeni tanımlı değil veya geçersiz. ` +
 ...
 */
export const decryptSecret = (cipher: Buffer, iv: Buffer, tag: Buffer): string => {
  const key = requireKey();
  try {
    ...
    throw ApiError.internal(
      "Kayıtlı entegratör şifresi çözülemedi. Şifreleme anahtarı değişmiş olabilir; " +
