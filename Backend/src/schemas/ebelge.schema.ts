...
export const ebelgeTarihSchema = z.string().trim().regex(/^\d{4}-\d{2}-\d{2}$/)
  .refine((v) => { const d = new Date(v); return Number.isFinite(d.getTime()) && d.toISOString().slice(0, 10) === v; },
    "Geçerli bir takvim tarihi giriniz (YYYY-AA-GG).");
...
export type EbelgeAyarInput = z.infer<typeof ebelgeAyarSchema>;
...
export type EbelgeDogrulaInput = z.infer<typeof ebelgeDogrulaSchema>;
...
export type EbelgeTaslakInput = z.infer<typeof ebelgeTaslakSchema>;
...
export const ebelgeArsivSenkronizeSchema = z.object({
  ...
  new Date(v.bitis).getTime() - new Date(v.baslangic).getTime() <= 366 * 86400000,
  "Tarih aralığı sıralı ve en fazla 366 gün olmalıdır.");
...
export type EbelgeIrsaliyeInput = z.infer<typeof ebelgeIrsaliyeSchema>;
...
export type EbelgeMustahsilInput = z.infer<typeof ebelgeMustahsilSchema>;
