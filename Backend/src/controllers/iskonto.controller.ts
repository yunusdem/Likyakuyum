...
export class IskontoController {
   ...
   * GET /api/v1/iskonto
   */
  public static listIskontolar = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = IskontoController.getDbContext(req);
    const search = req.query.search as string | undefined;
    const aktifQuery = req.query.aktif;
    let aktif: boolean | undefined = undefined;
    if (aktifQuery === "true" || aktifQuery === "1") aktif = true;
    else if (aktifQuery === "false" || aktifQuery === "0") aktif = false;
   ...
   */
  public static getIskontoById = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = IskontoController.getDbContext(req);
   ...
   * POST /api/v1/iskonto
   */
  public static saveIskonto = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = IskontoController.getDbContext(req);
    const {
      iskontoId,
      tanim,
   ...
   */
  public static deleteIskonto = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = IskontoController.getDbContext(req);
    const id = Number(req.params.id);
    if (!id || isNaN(id)) {
      throw ApiError.badRequest("Geçersiz iskonto ID.");
