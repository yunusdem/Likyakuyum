...
export class BanknotController {
   ...
   */
  public static createCurrency = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = BanknotController.getDbContext(req);
   ...
   */
  public static getBanknotlarByParaId = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = BanknotController.getDbContext(req);
   ...
   */
  public static saveBanknotlar = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = BanknotController.getDbContext(req);
    const paraId = Number(req.params.paraId);
    if (!paraId || isNaN(paraId)) {
      throw ApiError.badRequest("Geçerli bir Para ID parametresi girilmelidir.");
   ...
   */
  public static deleteBanknotlar = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = BanknotController.getDbContext(req);
    const paraId = Number(req.params.paraId);
    if (!paraId || isNaN(paraId)) {
      throw ApiError.badRequest("Geçerli bir Para ID parametresi girilmelidir.");
   ...
   */
  public static deleteCurrency = asyncHandler(async (req: Request, res: Response) => {
    const dbContext = BanknotController.getDbContext(req);
    const paraId = Number(req.params.paraId);
    if (!paraId || isNaN(paraId)) {
      throw ApiError.badRequest("Geçerli bir Para ID parametresi girilmelidir.");
