...
const sayiOpt = (min: number, max: number) => z.preprocess(v => (v === "" || v === undefined || v === null ? undefined : String(v).replace(",", ".")), z.coerce.number().min(min).max(max).optional());
...
const parametreSema = z.object({
  ...
  hareketTipleri: z.preprocess(v => (v === "" || v === undefined ? undefined : String(v).split(",").map(x => Number(x.trim())).filter(n => Number.isInteger(n) && n >= 0 && n <= 5)),
    z.array(z.number().int().min(0).max(5)).max(6).optional()),
...
export class RaporController {
  ...
  private static parametreler(req: Request) {
    ...
    if (p.data.vadeBaslangic && p.data.vadeBitis && p.data.vadeBaslangic > p.data.vadeBitis) throw ApiError.badRequest("Vade başlangıcı bitişten sonra olamaz.");
  ...
  public static veri = asyncHandler(async (req: Request, res: Response) => {
    ...
    return ApiResponse.ok(res, v.sinirAsildi ? `Rapor ${v.toplamKayit} satır üretiyor; üst sınır aşıldı. Tarih aralığını daraltın.` : "Rapor verisi.",
      { satirlar: v.satirlar, filtreOzeti: v.filtreOzeti, ekDipnot: v.ekDipnot, sinirAsildi: !!v.sinirAsildi, toplamKayit: v.toplamKayit, tanim: v.tanim, ozetSatirlar: v.ozetSatirlar || [] });
  });

  /** GET /api/v1/rapor/secim/:kaynak — dürbün listeleri (hesap | istatistik | meslek | sektor | kullanici | banka) */
  public static secim = asyncHandler(async (req: Request, res: Response) => {
    const k = z.enum(["hesap", "istatistik", "meslek", "sektor", "kullanici", "banka"]).safeParse(req.params.kaynak);
    if (!k.success) throw ApiError.badRequest("Seçim listesi geçersiz.");
