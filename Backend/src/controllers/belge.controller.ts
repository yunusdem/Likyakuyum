...
export class BelgeController {
  ...
  private static kullanici(req: Request) { return req.user?.username || "belge"; }
  ...
  public static fisler = asyncHandler(async (req: Request, res: Response) => {
    const p = listeSema.safeParse(req.query);
    if (!p.success) throw ApiError.badRequest("Fiş listesi filtresi geçersiz.", p.error.format());
  ...
  public static arsivle = asyncHandler(async (req: Request, res: Response) => {
    const p = istekSema.safeParse(req.body || {});
    if (!p.success) throw ApiError.badRequest(p.error.issues[0]?.message || "Arşiv isteği geçersiz.");
