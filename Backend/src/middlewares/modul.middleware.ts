 ...
 */
export const modulKapisi = (apiOnek: keyof typeof API_MODULLERI | string) => {
  const kodlar = API_MODULLERI[apiOnek] || [];
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (env.MERKEZ_GIRIS !== "zorunlu" || kodlar.length === 0) return next();
      const { dbServer, dbName } = req.user || {};
      if (!dbServer || !dbName) return next();

      const acik = await ModulService.oturumModulleri(dbServer, dbName);
      if (acik === null || kodlar.some((k) => acik.includes(k))) return next();
