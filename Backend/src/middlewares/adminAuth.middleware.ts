 ...
 */
export const adminKapisi = (req: Request, res: Response, next: NextFunction) => {
  if (!adminYapilandirildiMi()) return next(adminYapilandirmaHatasi());
...
export const adminAuthenticate = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!oturum) throw ApiError.unauthorized("Oturumunuz sonlandırılmış. Lütfen tekrar giriş yapınız.");
...
export const sifreBelirlenmisOlmali = (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const adminHataIsleyici = (err: any, req: Request, res: Response, _next: NextFunction) => {
  if (err instanceof ApiError && err.isOperational) {
    if (err.statusCode >= 500) logger.error(`[ADMIN ${err.statusCode}] ${req.method} ${req.originalUrl} - ${err.message}`);
    else logger.warn(`[ADMIN ${err.statusCode}] ${req.method} ${req.originalUrl} - ${err.message}`);
