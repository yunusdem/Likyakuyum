...
import { EbelgeAyarView, IceConnectionConfig } from "../services/ice/ice.types.js";
...
export interface EarsivArsivKaydi {
  ...
  tutar: number | null; paraBirimi: string | null; profil: string | null;
  iceStatuKodu: string | null; iceStatuAciklama: string | null;
}

export interface EbelgeAyarKayit {
  id: number;
  ortam: "CANLI" | "TEST";
  servisUrl: string;
  kullaniciAdi: string;
  sifreSifreli: Buffer | null;
  sifreIv: Buffer | null;
  sifreTag: Buffer | null;
  uygulamaAdi: string;
...
export interface SaveEbelgeAyarDto {
  ortam: "CANLI" | "TEST";
  servisUrl: string;
  kullaniciAdi: string;
...
export interface GelenBelgeUpsert {
  uuid: string;
  belgeNo?: string | null;
...
export interface GelenBelgeSatiri extends Required<Pick<GelenBelgeUpsert, "uuid">> {
  belgeNo: string | null;
  belgeTuru: string;
  profil: string | null;
  sender: string | null;
...
export interface GelenListeFiltre {
  sayfa?: number;
  boyut?: number;
  baslangicTarihi?: string;
  bitisTarihi?: string;
  arama?: string;
  /** "BEKLEYEN" → henüz cevap verilmemiş belgeler */
  redKabul?: "BEKLEYEN" | "Kabul" | "Red" | "TUMU";
}

export interface GidenBelgeKaydi {
  uuid: string;
  belgeNo: string;
  belgeTuru?: string;
  profil?: string | null;
  ...
  kaynakFisId?: string | null;
  xmlIcerik?: string | null;
...
export class EbelgeSqlRepository {
  private static ensuredKeys = new Set<string>();
  ...
  public static async ensureTablesExist(pool: sql.ConnectionPool): Promise<void> {
    ...
    if (this.ensuredKeys.has(key)) return;
   ...
   */
  public static async getAyar(dbContext?: DbContext): Promise<EbelgeAyarKayit | null> {
    ...
    return {
      ...row,
      ortam: (row.ortam === "TEST" ? "TEST" : "CANLI") as "CANLI" | "TEST",
   ...
   */
  public static async getFirmaVknFromTanim(dbContext?: DbContext): Promise<string> {
    const pool = await this.getPool(dbContext);
    try {
      const res = await pool.request().query(`
   ...
   */
  public static toAyarView(kayit: EbelgeAyarKayit): EbelgeAyarView {
    return {
      id: kayit.id,
      ortam: kayit.ortam,
    ...
    if (dto.sifre && dto.sifre.trim() !== "") {
      if (!isEncryptionConfigured()) {
        throw ApiError.internal(
          "EBELGE_ENC_KEY ortam değişkeni tanımlı değil. Şifre düz metin olarak saklanamayacağı için kayıt iptal edildi."
        );
      }
   ...
   */
  public static async getConnectionConfig(dbContext?: DbContext): Promise<IceConnectionConfig> {
    const kayit = await this.getAyar(dbContext);

    if (!kayit || !kayit.servisUrl || !kayit.kullaniciAdi) {
      throw ApiError.badRequest(
        "e-Belge entegratör ayarları tanımlı değil. Ayarlar ekranından servis adresi ve kullanıcı bilgilerini giriniz."
      );
    }
    if (!kayit.aktif) {
   ...
   */
  public static async writeLog(kayit: EbelgeLogKaydi, dbContext?: DbContext): Promise<void> {
    try {
      const pool = await this.getPool(dbContext);
      await pool
        .request()
   ...
   */
  public static async upsertGelen(kayit: GelenBelgeUpsert, dbContext?: DbContext): Promise<void> {
    const pool = await this.getPool(dbContext);
   ...
   */
  public static async listGelen(
    filtre: GelenListeFiltre,
    dbContext?: DbContext
  ): Promise<{ kayitlar: GelenBelgeSatiri[]; toplam: number }> {
    const pool = await this.getPool(dbContext);
    ...
    if (filtre.arama) {
      kosullar.push(
        "([BELGE_NO] LIKE @arama OR [SUPPLIER] LIKE @arama OR [SENDER] LIKE @arama OR [UUID] LIKE @arama)"
      );
    ...
    const sayimRes = await request.query(
      `SELECT COUNT(*) AS toplam FROM [dbo].[TODVZ_EBELGE_GELEN] ${where}`
    );
  ...
  public static async getGelen(uuid: string, dbContext?: DbContext): Promise<GelenBelgeSatiri | null> {
    const pool = await this.getPool(dbContext);
    ...
    const row = res.recordset[0];
    if (!row) return null;
    return { ...row, okunduMu: Boolean(row.okunduMu), islendiMi: Boolean(row.islendiMi) } as GelenBelgeSatiri;
  }
   ...
   */
  public static async setGelenRedKabul(
    uuid: string,
    redKabul: "Red" | "Kabul",
    aciklama: string,
    kullanici: string,
   ...
   */
  public static async clearGelenRedKabul(uuid: string, dbContext?: DbContext): Promise<void> {
    const pool = await this.getPool(dbContext);
  ...
  public static async setGelenOkunmaDurumu(
    uuid: string,
    durum: { okunduMu?: boolean; islendiMi?: boolean },
    dbContext?: DbContext
  ): Promise<void> {
    const pool = await this.getPool(dbContext);
    await pool
  ...
  public static async updateGelenStatu(
    uuid: string,
    statu: { statu?: string | null; statuAciklama?: string | null; gibStatuKodu?: number | null },
    dbContext?: DbContext
  ): Promise<void> {
    const pool = await this.getPool(dbContext);
    await pool
   ...
   */
  public static async insertGiden(kayit: GidenBelgeKaydi, dbContext?: DbContext): Promise<void> {
    const pool = await this.getPool(dbContext);
    try {
      await pool
        .request()
      ...
      if (err?.number === 2601 || err?.number === 2627) {
        throw ApiError.conflict(
          `${kayit.belgeNo} numaralı belge bu tarihte zaten oluşturulmuş. Aynı numara ikinci kez gönderilemez.`
        );
      }
      throw err;
    }
  ...
  public static async upsertEarsivArsiv(r: EarsivArsivKaydi, dbContext?: DbContext): Promise<void> {
    const pool = await this.getPool(dbContext);
    await pool.request()
  ...
  public static async listEarsivArsiv(sayfa: number, arama: string, dbContext?: DbContext) {
    const pool = await this.getPool(dbContext);
  ...
  public static async earsivArsivVarMi(uuid: string, dbContext?: DbContext): Promise<boolean> {
    const pool = await this.getPool(dbContext);
  ...
  public static async setEarsivArsivIsaret(uuid: string, statu: string, kullanici: string, dbContext?: DbContext) {
    const pool = await this.getPool(dbContext);
  ...
  public static async earsivDurumGecir(
    uuid: string,
    beklenen: string,
    durum: string,
  ...
  public static async gidenBelgeNoVarMi(belgeNo: string, dbContext?: DbContext): Promise<boolean> {
    const pool = await this.getPool(dbContext);
    const res = await pool
      .request()
  ...
  public static async listGiden(
    ...
    const sayim = await request.query(
      `SELECT COUNT(*) AS toplam FROM [dbo].[TODVZ_EBELGE_GIDEN] ${where}`
    );
  ...
  /** Tek giden belge */
  public static async getGiden(uuid: string, dbContext?: DbContext): Promise<any | null> {
    const pool = await this.getPool(dbContext);
    const res = await pool.request().input("uuid", sql.VarChar(60), uuid).query(`
  ...
  public static async setGidenIptal(
    uuid: string,
    kullanici: string,
    dbContext?: DbContext
  ): Promise<boolean> {
  ...
  public static async setGidenEarsivIptal(
    uuid: string,
    kullanici: string,
    dbContext?: DbContext
  ): Promise<boolean> {
   ...
   */
  public static async getLogs(limit = 50, dbContext?: DbContext): Promise<any[]> {
    const pool = await this.getPool(dbContext);
