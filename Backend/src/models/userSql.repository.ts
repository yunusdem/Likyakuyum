...
import { TodvzKullaniciEntity, UserModel, UserResponseDto } from "../types/user.types.js";
 ...
 */
export const colorToDb = (color?: string, defaultInt = 16777215): number => {
  if (!color) return defaultInt;
 ...
 */
export const colorFromDb = (val: any, defaultHex = "#ffffff"): string => {
...
export const parseMenuFont = (raw?: string | null): { font: string; selectedBgColor: string } => {
  if (!raw) return { font: "Segoe UI, sans-serif", selectedBgColor: "#ff80ff" };
  if (raw.includes("|")) {
...
export const parseMenuHeaderFont = (raw?: string | null): { font: string; headerBgColor: string } => {
  if (!raw) return { font: "Segoe UI, sans-serif", headerBgColor: "#000080" };
  if (raw.includes("|")) {
    const parts = raw.split("|");
    return {
      font: parts[0] || "Segoe UI, sans-serif",
...
export const encodeMenuFont = (font?: string, selectedBgColor?: string): string => {
  const f = font || "Segoe UI, sans-serif";
  const c = selectedBgColor || "#ff80ff";
  return `${f}|${c}`;
};

export const encodeMenuHeaderFont = (font?: string, headerBgColor?: string): string => {
  const f = font || "Segoe UI, sans-serif";
...
const toBit = (val?: boolean | number | string): number => {
...
const toFloat = (val: any, defaultVal = 0.0): number => {
  if (val === undefined || val === null || val === "") return defaultVal;
  const parsed = parseFloat(String(val));
  return isNaN(parsed) ? defaultVal : parsed;
};

const toStatId = (val: any): number | null => {
...
const kurYetkisiToDb = (val: any): number => {
  if (typeof val === "number") return val;
  const str = String(val || "").trim();
  if (str === "Yok" || str === "0") return 0;
  if (str === "Limitli" || str === "2") return 2;
  return 1; // "Var" -> 1
};

const kurYetkisiFromDb = (val: any): string => {
  if (val === 0 || val === "0") return "Yok";
  if (val === 2 || val === "2") return "Limitli";
  return "Var";
};
...
export class UserSqlRepository {
   ...
   */
  public static mapEntityToModel(entity: TodvzKullaniciEntity): UserModel {
    ...
    return {
      ...
      buyStatCode: (entity as any).ALIS_ISTATISTIK_KOD ? String((entity as any).ALIS_ISTATISTIK_KOD).trim() : (entity.ALIS_ISTATISTIK_ID != null ? String(entity.ALIS_ISTATISTIK_ID) : ""),
      sellStatCode: (entity as any).SATIS_ISTATISTIK_KOD ? String((entity as any).SATIS_ISTATISTIK_KOD).trim() : (entity.SATIS_ISTATISTIK_ID != null ? String(entity.SATIS_ISTATISTIK_ID) : ""),
      arbitrageBuyStatCode: (entity as any).ARBITRAJ_ALIS_ISTATISTIK_KOD ? String((entity as any).ARBITRAJ_ALIS_ISTATISTIK_KOD).trim() : (entity.ARBITRAJ_ALIS_ISTATISTIK_ID != null ? String(entity.ARBITRAJ_ALIS_ISTATISTIK_ID) : ""),
      arbitrageSellStatCode: (entity as any).ARBITRAJ_SATIS_ISTATISTIK_KOD ? String((entity as any).ARBITRAJ_SATIS_ISTATISTIK_KOD).trim() : (entity.ARBITRAJ_SATIS_ISTATISTIK_ID != null ? String(entity.ARBITRAJ_SATIS_ISTATISTIK_ID) : ""),
   ...
   */
  public static async resolveValidStatId(pool: sql.ConnectionPool, inputStat: any): Promise<number | null> {
   ...
   */
  public static async findAll(dbContext?: { dbServer?: string; dbName?: string }): Promise<UserModel[]> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      const result = await pool.request().query<TodvzKullaniciEntity>(`
   ...
   */
  public static async findById(id: string | number, dbContext?: { dbServer?: string; dbName?: string }): Promise<UserModel | null> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      const request = pool.request();
      request.input("userId", sql.Int, toInt(id));

      const result = await request.query<TodvzKullaniciEntity>(`
        ${UserSqlRepository.BASE_SELECT}
   ...
   */
  public static async findByUsername(username: string, dbContext?: { dbServer?: string; dbName?: string }): Promise<UserModel | null> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      const request = pool.request();
   ...
   */
  public static async getCashiers(dbContext?: { dbServer?: string; dbName?: string }): Promise<{ id: number; kod?: string; name: string }[]> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
   ...
   */
  public static async create(user: Partial<UserModel>, dbContext?: { dbServer?: string; dbName?: string }): Promise<UserModel> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      const request = pool.request();
      ...
      const result = await request.query<TodvzKullaniciEntity>(insertQuery);
      if (!result.recordset || result.recordset.length === 0) {
   ...
   */
  public static async update(id: string | number, user: Partial<UserModel>, dbContext?: { dbServer?: string; dbName?: string }): Promise<UserModel | null> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      const request = pool.request();
      ...
      const result = await request.query<TodvzKullaniciEntity>(updateQuery);
      if (!result.recordset || result.recordset.length === 0) {
        return null;
      }
  ...
  public static async updateAppearance(
    ...
    try {
      ...
      const result = await request.query<TodvzKullaniciEntity>(query);
      if (!result.recordset || result.recordset.length === 0) {
        return null;
      }
      return UserSqlRepository.mapEntityToModel(result.recordset[0]);
   ...
   */
  public static async delete(id: string | number, dbContext?: { dbServer?: string; dbName?: string }): Promise<boolean> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      const request = pool.request();
      request.input("userId", sql.Int, toInt(id));
   ...
   */
  public static toDto(user: UserModel): UserResponseDto {
