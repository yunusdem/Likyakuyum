...
// ─── Interfaces: TODVZ_BANKA ─────────────────────────────────────────────────
export interface BankaHesapModel {
  bankaId: number;
  hesapNo: string;
  hesapAdi: string;
  iban?: string | null;
...
export interface SaveBankaHesapDto {
  bankaId?: number | null;
  hesapNo: string;
  hesapAdi: string;
  iban?: string | null;
...
// ─── Interfaces: TODVZ_BANKA_HAREKET & SATIRI ────────────────────────────────
export interface BankaHareketSatiriModel {
...
export interface BankaHareketModel {
  bankaHareketId: number;
  islemTipi: number; // 0: Havale Alma, 1: Havale Gönderme, 2: Kasadan Bankaya Nakit, 3: Bankadan Kasaya Nakit, 4: Virman
  bankaId: number;
  bankaHesapNo?: string;
  ...
  guncelleyenId?: number | null;
  guncellemeZamani?: string | null;
  satirlar: BankaHareketSatiriModel[];
}

export interface SaveBankaHareketSatiriDto {
  satirNo?: number;
  paraId: number;
  meblag: number;
  kur?: number;
...
export interface SaveBankaHareketDto {
  ...
  belgeNo?: string | null;
  aciklama?: string | null;
  satirlar: SaveBankaHareketSatiriDto[];
}
...
export class BankaSqlRepository {
  ...
  public static async getBankaById(
    bankaId: number,
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<BankaHesapModel | null> {
    const list = await this.listBankalar(undefined, dbContext);
    const item = list.find((b) => b.bankaId === bankaId);
    return item || null;
  }

  public static async getNextHesapNo(dbContext?: { dbServer?: string; dbName?: string }): Promise<string> {
    const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    await this.ensureTables(pool);
  ...
  public static async saveBanka(
    ...
    if (!saved) throw ApiError.internal("Banka hesabı kaydedildi ancak okunamadı.");
    return saved;
  }
  ...
  public static async listHareketler(
    ...
    const hareketIds = headers.map((h: any) => h.BANKA_HAREKET_ID);
    ...
    const satirlarMap = new Map<number, BankaHareketSatiriModel[]>();
  ...
  public static async getHareketById(
    bankaHareketId: number,
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<BankaHareketModel | null> {
    ...
    `);
    if (!exactQuery.recordset?.length) return null;
    ...
    const satirlar: BankaHareketSatiriModel[] = (satirlarRes.recordset || []).map((s: any) => ({
      bankaHareketId: s.BANKA_HAREKET_ID,
      satirNo: s.SATIR_NO,
      paraId: s.PARA_ID,
      paraKodu: (s.PARA_KODU || "TL").trim(),
  ...
  public static async saveHareket(
    ...
    try {
      ...
      if (!checkBankaRes.recordset?.[0]) {
        let foundExistingId: number | null = null;
        ...
        if (foundExistingId) {
          ...
          if (!yeniHesapNo) yeniHesapNo = "102.01.001";
          if (!yeniHesapAdi) yeniHesapAdi = "Banka Ticari TL Hesabı";
      ...
      if (satirlarToSave.length === 0 && (dto as any).meblag !== undefined && Number((dto as any).meblag) > 0) {
        satirlarToSave = [{
          paraId: (dto as any).paraId || 1,
          meblag: Number((dto as any).meblag),
          kur: 1,
          giseKuru: 1,
          tutarTl: Number((dto as any).meblag),
      ...
      if (!saved) throw ApiError.internal("Banka hareketi kaydedildi ancak okunamadı.");
      return saved;
    } catch (err: any) {
