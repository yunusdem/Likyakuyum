...
// Faz 0: ayar + çağrı günlüğü. Diğer TODVZ_EBANKA_* tabloları kendi fazlarında eklenir.

export type DbContext = { dbServer?: string; dbName?: string };

export type EBankaMod = "sahte" | "canli";
export type VomsisServis = "banka" | "vpos";

/** Ham ayar satırı. Gizli alanlar şifreli haldedir; çözme işi servistedir. */
export interface EBankaAyarSatiri {
  mod: EBankaMod;
  appKey: string | null;
  appSecretSifreli: string | null;
  vposAppKey: string | null;
  vposAppSecretSifreli: string | null;
...
export interface EBankaLogSatiri {
  logId: number;
  zaman: string;
  islem: string;
  mod: EBankaMod;
...
const TOKEN_KOLONLARI: Record<VomsisServis, { token: string; bitis: string }> = {
...
export class EBankaSqlRepository {
  ...
  public static async ayarGetir(dbContext?: DbContext): Promise<EBankaAyarSatiri | null> {
  ...
  public static async ayarKaydet(dto: EBankaAyarKaydet, kullaniciId?: number, dbContext?: DbContext): Promise<void> {
    const pool = await this.pool(dbContext);
  ...
  public static async tokenYaz(servis: VomsisServis, sifreli: string | null, bitis: Date | null, dbContext?: DbContext): Promise<void> {
    const pool = await this.pool(dbContext);
    const k = TOKEN_KOLONLARI[servis];
    const req = pool.request();
  ...
  public static async logYaz(
    kayit: { islem: string; mod: EBankaMod; basarili: boolean; adet?: number | null; mesaj?: string | null; kullaniciId?: number | null },
    dbContext?: DbContext
  ): Promise<void> {
    try {
      const pool = await this.pool(dbContext);
