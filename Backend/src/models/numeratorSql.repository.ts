...
export class NumeratorSqlRepository {
  ...
  public static async saveViaProcedure(
    ...
    try {
      ...
      const yaziciOrtakAlan =
        data.yaziciOrtakAlan !== undefined
          ? (data.yaziciOrtakAlan ? 1 : 0)
          : (yaziciId === null ? 1 : 0);

      logger.info(
        `[SODVZ_NUMERATOR_KAYDET] Çağrılıyor: TUR=${tur}, YAZICI_ID=${yaziciId}, ONEK='${onek}', BASLANGIC=${baslangic}, BITIS=${bitis}, UZUNLUK=${uzunluk}, ONUNE_SIFIR_KOY=${onuneSifirKoy ? 1 : 0}, YAZICI_ORTAK_ALAN=${yaziciOrtakAlan}`
      );
