...
export interface AdminOturumBaglami {
  sid: string;
  adminId: number;
  kullaniciAdi: string;
  adSoyad: string;
...
export class AdminOturumSqlRepository {
  ...
  public static async ac(adminId: number, ip: string, tarayici: string): Promise<string> {
   ...
   */
  public static async gecerliOturum(sid: string, adminId: number): Promise<AdminOturumBaglami | null> {
    const pool = await getAdminPool();
  ...
  public static async kapat(sid: string): Promise<void> {
    const pool = await getAdminPool();
    await pool
      .request()
      .input("sid", sql.UniqueIdentifier, sid)
  ...
  public static async sidIleIptalEt(sid: string, iptalEdenAdminId: number): Promise<number> {
    const pool = await getAdminPool();
    const res = await pool
      .request()
      .input("sid", sql.UniqueIdentifier, sid)
  ...
  public static async adminOturumlariniIptalEt(
    adminId: number,
    iptalEdenAdminId: number | null,
    haricSid?: string
  ): Promise<void> {
    const pool = await getAdminPool();
    await pool
