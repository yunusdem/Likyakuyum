...
export interface OturumDurumu {
  gecerli: boolean;
  /** Geçersizse kullanıcıya gösterilecek neden */
  neden: "OTURUM_KAPALI" | "KULLANICI_PASIF" | "FIRMA_DONDURULDU" | "FIRMA_PASIF" | "LISANS_BITTI" | null;
  sifreDegismeli: boolean;
}

export interface SayfaIstegi {
  sayfa: number;
  boyut: number;
}

const sayfala = (s: SayfaIstegi) => ({ atla: (Math.max(1, s.sayfa) - 1) * s.boyut, al: s.boyut });

/** ADM_OTURUM'un kullanıcı (TUR='KULLANICI') tarafı ve yönetim panelinin izleme listeleri. */
export class IzlemeSqlRepository {
  public static async kullaniciOturumuAc(
    kullaniciId: number,
    firmaId: number,
    ip: string,
    tarayici: string
    ...
    await pool
      ...
      .query(
        `INSERT INTO dbo.ADM_OTURUM (OTURUM_ID, TUR, KULLANICI_ID, FIRMA_ID, IP, TARAYICI) VALUES (@sid, 'KULLANICI', @kullaniciId, @firmaId, @ip, @tarayici)`
      );
    return sid;
  }
   ...
   */
  public static async oturumDurumu(sid: string): Promise<OturumDurumu> {
    const pool = await getAdminPool();
    ...
    const sonuc = (neden: OturumDurumu["neden"]): OturumDurumu => ({
      gecerli: neden === null,
      neden,
      sifreDegismeli: !!r?.SIFRE_DEGISMELI,
    });
    if (!r || r.BITIS !== null || r.IPTAL_EDILDI) return sonuc("OTURUM_KAPALI");
    if (r.FIRMA_DURUM === "PASIF") return sonuc("FIRMA_PASIF");
    if (r.FIRMA_DURUM === "DONDURULMUS") return sonuc("FIRMA_DONDURULDU");
    if (r.LISANS_BITTI) return sonuc("LISANS_BITTI");
    if (r.KULLANICI_DURUM !== "AKTIF") return sonuc("KULLANICI_PASIF");
    return sonuc(null);
  }

  /** Kullanıcının kendi çıkışı */
  public static async oturumuBitir(sid: string): Promise<void> {
    const pool = await getAdminPool();
    await pool
      .request()
      .input("sid", sql.UniqueIdentifier, sid)
  ...
  public static async oturumlariIptalEt(
    olcut: { sid?: string; kullaniciId?: number; firmaId?: number },
    adminId: number | null
  ): Promise<number> {
    const pool = await getAdminPool();
  ...
  public static async cevrimici(dakika: number): Promise<any[]> {
    const pool = await getAdminPool();
  ...
  public static async girisLoglari(
    filtre: { firmaId?: number; tur?: "ADMIN" | "KULLANICI"; basarili?: boolean; arama?: string } & SayfaIstegi
  ): Promise<{ satirlar: any[]; toplam: number }> {
    const pool = await getAdminPool();
    ...
    const res = await pool
      ...
      `);
    const kumeler = res.recordsets as any[];
    return { toplam: kumeler[0][0].TOPLAM, satirlar: kumeler[1] };
  }

  public static async islemLoglari(filtre: { arama?: string } & SayfaIstegi): Promise<{ satirlar: any[]; toplam: number }> {
    const pool = await getAdminPool();
    const { atla, al } = sayfala(filtre);
    const res = await pool
      .request()
