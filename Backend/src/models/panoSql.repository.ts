 ...
 */
export function delphiColorToHex(colorVal: any): string {
  if (colorVal === undefined || colorVal === null || colorVal === "") return "#000000";
 ...
 */
export function hexToDelphiColor(hex?: string | null): number {
...
export interface TodvzPanoEntity {
  PANO_ID: number;
  PANO_NO: string | null;
  YENILEME_ARALIGI: number | null;
  FIRMA_ADI: string | null;
  PARA_BASLIGI: string | null;
  ALIS_KURU_BASLIGI: string | null;
  SATIS_KURU_BASLIGI: string | null;
  FIRMA_ADI_OZELLIKLERI: string | null;
  TARIH_SAAT_OZELLIKLERI: string | null;
  BASLIK_OZELLIKLERI: string | null;
  SATIR_OZELLIKLERI: string | null;
  ZEMIN_RENGI: string | number | null;
  BOSLUK_SAYISI: number | null;
  HTML_DOSYA_ADI: string | null;
  KOD_ALANI_GENISLIGI: number | null;
  KUR_ALANI_GENISLIGI: number | null;
}

export interface TodvzPanoSatiriEntity {
  PANO_ID: number;
  PARA_ID: number;
  GORUNECEK_AD: string | null;
  SIRA_NO: number;
  GORUNUR: boolean | number;
  CARPAN: number;
  PARA_KOD?: string;
  PARA_AD?: string;
}
...
export class PanoSqlRepository {
  ...
  public static async findAll(dbContext?: { dbServer?: string; dbName?: string }): Promise<PanoModel[]> {
    ...
    const res = await pool
      .request()
      .query<TodvzPanoEntity>("SELECT * FROM [dbo].[TODVZ_PANO] ORDER BY [PANO_ID] ASC");
   ...
   */
  public static async findById(id: number, dbContext?: { dbServer?: string; dbName?: string }): Promise<PanoModel | null> {
    const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    await PanoSqlRepository.ensureTablesAndProceduresExist(pool);
    ...
    const headerRes = await pool
      .request()
      .input("id", sql.Int, id)
      .query<TodvzPanoEntity>("SELECT TOP 1 * FROM [dbo].[TODVZ_PANO] WHERE [PANO_ID] = @id");
   ...
   */
  public static async saveViaProcedure(dto: SavePanoDto, dbContext?: { dbServer?: string; dbName?: string }): Promise<PanoModel> {
    const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    await PanoSqlRepository.ensureTablesAndProceduresExist(pool);
   ...
   */
  public static async deleteViaProcedure(id: number, dbContext?: { dbServer?: string; dbName?: string }): Promise<boolean> {
    const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    await PanoSqlRepository.ensureTablesAndProceduresExist(pool);
   ...
   */
  public static async getLiveBoardData(panoId: number, dbContext?: { dbServer?: string; dbName?: string }): Promise<PanoModel | null> {
    let targetId = panoId;
    if (!targetId || targetId <= 0) {
      const allPanos = await PanoSqlRepository.findAll(dbContext);
      if (allPanos.length > 0) targetId = allPanos[0].panoId;
      else return null;
    }
    ...
    const ratesMap = new Map<number, { alis: number | null; satis: number | null }>();
