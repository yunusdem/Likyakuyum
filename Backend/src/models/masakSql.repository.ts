 ...
 * Tablolar: TODVZ_MASAK_LISTE (veri), TODVZ_MASAK_GUNCELLEME (güncelleme geçmişi)
 */

/**
 * Liste kodu. Standart dört liste (A, B, C, 3AB) sabittir; kullanıcı ekrandan
 * ek liste tanımlayabildiği için tip serbest metindir (en fazla 10 karakter).
 */
...
export const MASAK_LISTE_KODLARI: MasakStandartListeKod[] = ["A", "B", "C", "3AB"];

/** Büyük harf, rakam ve . _ - karakterleri; 1-10 karakter (kolon VARCHAR(10)) */
export const MASAK_LISTE_KOD_DESENI = /^[A-Z0-9][A-Z0-9._-]{0,9}$/;

export const masakListeKodGecerliMi = (kod: unknown): kod is MasakListeKod =>
  typeof kod === "string" && MASAK_LISTE_KOD_DESENI.test(kod);

export const masakStandartListeMi = (kod: string): kod is MasakStandartListeKod =>
  (MASAK_LISTE_KODLARI as string[]).includes(kod);

/**
 * Ayrıştırılmış tek bir MASAK satırı (parser çıktısı, tabloya yazılmaya hazır)
 */
export interface MasakKayitDto {
...
/**
 * Bir listenin güncel durumu (kayıt sayısı + son güncelleme + son kullanılan adres)
 */
export interface MasakListeDurumModel {
  listeKod: string;
  listeAdi: string | null;
  kayitSayisi: number;
  ...
  /** true: kullanıcı tanımlı liste (standart A/B/C/3AB dışı); silinebilir */
  ozel?: boolean;
}

export interface MasakGuncellemeLogDto {
  listeKod: string;
  baslamaZamani: Date;
  bitisZamani?: Date | null;
  sureMs?: number | null;
  durum: "BASARILI" | "HATA";
...
export interface MasakGuncellemeLogModel extends MasakGuncellemeLogDto {
  guncellemeId: number;
}

interface MasakDurumEntity {
  LISTE_KOD: string;
  LISTE_ADI: string | null;
  KAYIT_SAYISI: number;
  SON_GUNCELLEME: Date | null;
  KAYNAK_URL: string | null;
  KAYNAK_HASH: string | null;
}

interface MasakLogEntity {
  GUNCELLEME_ID: number;
  LISTE_KOD: string;
  BASLAMA_ZAMANI: Date;
  BITIS_ZAMANI: Date | null;
  SURE_MS: number | null;
  DURUM: string;
  KAYIT_SAYISI: number | null;
  ONCEKI_KAYIT_SAYISI: number | null;
  KAYNAK_URL: string | null;
  KAYNAK_HASH: string | null;
...
export interface MasakKayitModel {
  masakId: number;
  listeKod: string;
  listeAdi: string | null;
  siraNo: number | null;
...
export type MasakEslesmeTipi = "KIMLIK_TAM" | "AD_TAM" | "ALIAS_TAM" | "AD_KELIME";

export interface MasakEslesmeModel extends MasakKayitModel {
  skor: number;
  eslesmeTipi: MasakEslesmeTipi;
  dogumUyumlu: boolean | null;
}

export interface MasakListeSorgusu {
  listeKod?: string;
  q?: string;
  kimlikNo?: string;
  page?: number;
...
export class MasakSqlRepository {
   ...
   */
  private static kes(value: string | null | undefined, len: number): string | null {
  ...
  private static formatIso(d: Date | null | undefined): string | null {
    if (!d) return null;
    try {
   ...
   */
  public static async getKayitSayisi(
    listeKod: string,
    ...
    const res = await pool
      ...
      .query<{ ADET: number }>(
        "SELECT COUNT(*) AS [ADET] FROM [dbo].[TODVZ_MASAK_LISTE] WHERE [LISTE_KOD] = @listeKod"
      );
   ...
   */
  public static async getDurum(
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<MasakListeDurumModel[]> {
    const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    await MasakSqlRepository.ensureTablesExist(pool);

    const dataRes = await pool.request().query<MasakDurumEntity>(`
      SELECT
        [LISTE_KOD],
    ...
    const logRes = await pool.request().query<{ LISTE_KOD: string; KAYNAK_URL: string | null }>(`
    ...
    const logUrlMap = new Map<string, string | null>();
    logRes.recordset.forEach((r) => logUrlMap.set(r.LISTE_KOD, r.KAYNAK_URL));

    const dataMap = new Map<string, MasakDurumEntity>();
    ...
    const kodlar: string[] = [...MASAK_LISTE_KODLARI, ...ozelKodlar];
   ...
   */
  public static async deleteListe(
    listeKod: string,
    dbContext?: { dbServer?: string; dbName?: string }
   ...
   */
  public static async replaceListe(
    listeKod: MasakListeKod,
    kayitlar: MasakKayitDto[],
    meta: { listeAdi?: string | null; kaynakUrl?: string | null; kaynakHash?: string | null },
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<number> {
    const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    await MasakSqlRepository.ensureTablesExist(pool);
    ...
    try {
      ...
      if (kayitlar.length > 0) {
        ...
        for (const k of kayitlar) {
          table.rows.add(
            listeKod,
            kes(k.listeAdi, 200) ?? listeAdi,
            k.siraNo ?? null,
            kes(k.adUnvan, 500) ?? "",
            kes(k.adUnvanNorm, 500),
            kes(k.kayitTipi, 10),
            kes(k.kimlikNo, 1000),
            kes(k.tckn, 11),
            kes(k.vkn, 10),
            k.digerIsimler || null,
            k.digerIsimlerNorm || null,
            kes(k.orijinalAd, 500),
            kes(k.eskiAdi, 500),
            kes(k.gorevi, 500),
            k.adres || null,
            kes(k.uyruk, 200),
            kes(k.digerUyruk, 300),
            kes(k.yaptirimTuru, 200),
            kes(k.anneAdi, 150),
            kes(k.babaAdi, 150),
            kes(k.dogumTarihi, 300),
            k.dogumTarihiDt ?? null,
            kes(k.dogumYeri, 300),
            kes(k.orgut, 300),
            kes(k.kurulusYapisi, 300),
            kes(k.listeyeAlinma, 300),
            kes(k.kararBilgi, 300),
            kes(k.resmiGazete, 300),
            k.digerBilgiler || null,
            k.ekBilgi || null,
            kaynakUrl,
            kaynakHash,
            zaman,
   ...
   */
  public static async logGuncelleme(
    dto: MasakGuncellemeLogDto,
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<number> {
    const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
    await MasakSqlRepository.ensureTablesExist(pool);
  ...
  public static async getGecmis(
    ...
    const res = await req.query<MasakLogEntity>(query);
    ...
    return res.recordset.map((r) => ({
      ...
      sureMs: r.SURE_MS,
      durum: (r.DURUM === "BASARILI" ? "BASARILI" : "HATA") as "BASARILI" | "HATA",
  ...
  private static likeKacir(value: string): string {
  ...
  public static async listele(
    ...
    const ekle = (isim: string, tip: any, deger: any) => {
    ...
    if (params.qNorm && params.qNorm.trim()) {
      ...
      kosullar.push(
        "([AD_UNVAN_NORM] LIKE @q ESCAPE '[' OR [DIGER_ISIMLER_NORM] LIKE @q ESCAPE '[')"
      );
    }
    ...
    if (params.kimlikNo && params.kimlikNo.trim()) {
      ...
      kosullar.push(
        "([TCKN] = @kimlikTam OR [VKN] = @kimlikTam OR [KIMLIK_NO] LIKE @kimlikLike ESCAPE '[')"
      );
    }

    const where = kosullar.length ? `WHERE ${kosullar.join(" AND ")}` : "";

    const sayimRes = await sayimReq.query<{ TOPLAM: number }>(
      `SELECT COUNT(*) AS [TOPLAM] FROM [dbo].[TODVZ_MASAK_LISTE] ${where};`
    );
    ...
    return {
      kayitlar: res.recordset.map((r: any) => MasakSqlRepository.mapKayit(r)),
      toplam,
      page,
      pageSize,
    };
  ...
  public static async sorgula(
    ...
    req.input(
      "adLike",
      sql.NVarChar(500),
      adNorm ? `%${MasakSqlRepository.likeKacir(adNorm)}%` : null
    );
