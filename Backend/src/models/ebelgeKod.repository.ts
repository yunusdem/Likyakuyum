...
/**
 * e-Belge kod listeleri (istisna / tevkifat / özel matrah / ihraç kayıtlı) — docs/ebelge-revizyon.md K8.
 * GİB kod listesi tabloya bir kez ekilir (SISTEM=1); kullanıcı formdaki dürbünden yeni kod ekleyebilir (SISTEM=0).
 * Liste yalnızca seçim kolaylığıdır: hangi kodun kullanılacağı mali müşavirle teyit edilir, uygulama kod önermez.
 */
...
type Tohum = [kod: string, ad: string, oran?: number];
...
export class EbelgeKodRepository {
  private static async pool(ctx?: DbContext) {
    ...
    const tohum = (Object.entries(TOHUM) as [EbelgeKodTuru, Tohum[]][])
      .flatMap(([tur, liste]) => liste.map(([kod, ad, oran]) => ({ tur, kod, ad, oran: oran ?? null })));
    const mevcut = new Set((await pool.request().query(`SELECT RTRIM(TUR)+':'+RTRIM(KOD) a FROM dbo.TODVZ_EBELGE_KOD`)).recordset.map((x: any) => x.a));
  ...
  static async list(tur: EbelgeKodTuru | undefined, ctx?: DbContext): Promise<EbelgeKod[]> {
    ...
    `);
    return r.recordset.map((k: any) => ({ ...k, oran: k.oran == null ? null : Number(k.oran), sistem: !!k.sistem }));
  }

  static async ekle(k: { tur: EbelgeKodTuru; kod: string; ad: string; oran?: number | null }, kullanici: string, ctx?: DbContext): Promise<EbelgeKod> {
    const pool = await this.pool(ctx);
    try {
      ...
      if (e?.number === 2627 || e?.number === 2601) throw ApiError.badRequest(`${k.kod} kodu listede zaten var.`);
      throw e;
    }
