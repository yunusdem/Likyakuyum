...
export class CompanySqlRepository {
  ...
  private static async sanitizeFkIds(
    data: Partial<TodvzTanimEntity>,
    pool: sql.ConnectionPool
  ): Promise<Partial<TodvzTanimEntity>> {
    const copy = { ...data };
    ...
    if (copy.POSTA_KODU_ID !== undefined && copy.POSTA_KODU_ID !== null) {
      ...
      if (pkId) {
        try {
          const pkCheck = await pool.request().query<{ TABLO_MADDESI_ID: number }>(`
    ...
    const tmFields: (keyof TodvzTanimEntity)[] = ["VERGI_DAIRESI_ID", "ILCE_ID", "IL_ID", "POSTA_KODU_ID"];
    const tmIdsToCheck = tmFields
      .map((f) => ({ field: f, id: toValidId(copy[f]) }))
      .filter((x) => x.id !== null);

    if (tmIdsToCheck.length > 0) {
      const ids = tmIdsToCheck.map((x) => x.id as number);
      try {
    ...
    if (copy.ULKE_ID !== undefined && copy.ULKE_ID !== null) {
      ...
      if (uId) {
        try {
          ...
          `);
          if (!res.recordset || res.recordset.length === 0) copy.ULKE_ID = null;
        } catch {
          copy.ULKE_ID = null;
        }
      } else {
    ...
    const paraFields: (keyof TodvzTanimEntity)[] = [
      "USD_PARA_ID",
      "EUR_PARA_ID",
      "RAPOR_PARA_ID",
    ...
    if (paraIdsToCheck.length > 0) {
      const ids = paraIdsToCheck.map((x) => x.id as number);
      try {
    ...
    const statFields: (keyof TodvzTanimEntity)[] = [
      "ALIS_ISTATISTIK_ID",
      "SATIS_ISTATISTIK_ID",
    ...
    if (statIdsToCheck.length > 0) {
      const ids = statIdsToCheck.map((x) => x.id as number);
      try {
    ...
    if (copy.FOREKS_KUR_VEZNE_ID !== undefined && copy.FOREKS_KUR_VEZNE_ID !== null) {
      ...
      if (vId) {
        try {
          const res = await pool.request().query<{ VEZNE_ID: number }>(`
            SELECT TOP 1 [VEZNE_ID] FROM [dbo].[TODVZ_VEZNE] WHERE [VEZNE_ID] = ${vId}
          `);
          if (!res.recordset || res.recordset.length === 0) copy.FOREKS_KUR_VEZNE_ID = null;
        } catch {
    ...
    const hesapFields: (keyof TodvzTanimEntity)[] = ["SERMAYE_HESABI_ID", "URETIM_HESABI_ID"];
      ...
      if (hId) {
        ...
        try {
          ...
          if (muhRes.recordset && muhRes.recordset.length > 0) {
            ...
            `);
            if (res.recordset && res.recordset.length > 0) valid = true;
          }
          if (!valid) {
  ...
  public static async updateDefinitions(
    ...
    try {
      ...
      try {
        const colsRes = await pool.request().query<{ COLUMN_NAME: string }>(`
      ...
      const allColumnMappings: { name: keyof TodvzTanimEntity; type: any; len?: number }[] = [
        // 1. Genel
