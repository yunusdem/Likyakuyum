...
export class AyarSqlRepository {
   ...
   */
  public static async listAyarlar(
    sadeceAktif: boolean = false,
   ...
   */
  public static async getAyarById(
    ayarId: number,
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<AyarModel | null> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      await this.ensureSchema(pool);
  ...
  public static async saveAyar(
    dto: SaveAyarDto,
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<{ ayarId: number; yeniKayit: boolean }> {
    try {
      const pool = await getDbPool(dbContext?.dbServer, dbContext?.dbName);
      await this.ensureSchema(pool);
   ...
   */
  public static async deleteAyar(
    ayarId: number,
    dbContext?: { dbServer?: string; dbName?: string }
  ): Promise<boolean> {
    try {
