...
export interface SayimSatirDto {
  satirId?: number;
  satirNo: number;
...
export interface SaveSayimFisiPayload {
  sayimFisiId?: number | null;
  fisNo: string;
  tarih?: string | null;
  saat?: string | null;
  sistemAdet?: number;
  ...
  satirlar?: SayimSatirDto[];
}

export class SayimSqlRepository {
  static async getNextFisNo(): Promise<string> {
   ...
   */
  static async generateNextFisNo(): Promise<string> {
   ...
   */
  static async saveSayimFisi(payload: SaveSayimFisiPayload): Promise<{ sayimFisiId: number; fisNo: string }> {
    const pool = await getDbPool();
    ...
    try {
      ...
      if (!effectiveSayimFisiId && effectiveFisNo) {
        ...
        const checkRes = await checkReq.query(
          `SELECT TOP 1 SAYIM_FISI_ID FROM dbo.TODVZ_SAYIM_FISI WHERE FIS_NO = @checkFisNo;`
        );
        if (checkRes.recordset && checkRes.recordset.length > 0) {
      ...
      if (!effectiveSayimFisiId) {
        if (!effectiveFisNo) {
          ...
          const dupRes = await dupReq.query(
            `SELECT TOP 1 SAYIM_FISI_ID FROM dbo.TODVZ_SAYIM_FISI WHERE FIS_NO = @dupFisNo;`
          );
      ...
      headerReq.input(
        "TOPLAM_KALEM",
        sql.Int,
        Number(payload.toplamKalem || (payload.satirlar ? payload.satirlar.length : 0))
      );
      ...
      if (Array.isArray(payload.satirlar) && payload.satirlar.length > 0) {
        ...
        for (let i = 0; i < payload.satirlar.length; i++) {
          ...
          satirReq.input(
            "SAYILAN_ADET",
            sql.Float,
            s.sayilanAdet !== null && s.sayilanAdet !== undefined ? Number(s.sayilanAdet) : null
          );
          satirReq.input(
            "SAYILAN_GRAM",
            sql.Float,
            s.sayilanGram !== null && s.sayilanGram !== undefined ? Number(s.sayilanGram) : null
          );
      ...
      logger.info(
        `Sayım Fişi Kaydedildi: ID=${sayimFisiId}, FişNo=${effectiveFisNo}, Kalem=${payload.satirlar?.length || 0}`
      );
   ...
   */
  static async deleteSayimFisi(sayimFisiId: number): Promise<boolean> {
    const pool = await getDbPool();
    try {
      const req = pool.request();
   ...
   */
  static async listSayimFisleri(limit = 100): Promise<any[]> {
    const pool = await getDbPool();
    try {
      const req = pool.request();
   ...
   */
  static async getSayimFisiById(idOrFisNo: string | number): Promise<any | null> {
    const pool = await getDbPool();
    try {
      ...
      const mappedSatirlar = (satirlarRes.recordset || []).map((s: any) => ({
