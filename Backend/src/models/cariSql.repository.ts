...
export interface TodvzCariKartEntity {
  CARI_KART_ID: number;
  KOD: string;
  AD: string;
  KISILIK_TIPI: number;
  ...
  IL_ID: number | null;
  TELEFON: string | null;
  UYRUK_ID: number | null;
  ULKE_ID: number | null;
  HUKUKI_YAPI_ID: number | null;
  VEKIL_TURU: number | null;
  VEKIL_KISILIK_TIPI: number | null;
  VEKIL_ADI: string | null;
  VEKIL_KIMLIK_NO: string | null;
  ...
  EPOSTA: string | null;
  BANKA_HESABI_ID: number | null;
  ANNE_ADI: string | null;
  KIMLIK_SERI_NO: string | null;
  DOGUM_TARIHI: Date | null;
  DOGUM_YERI: string | null;
  KARA_LISTEDE: boolean | null;
  SEKTOR_ID: number | null;
  MESLEK_ID: number | null;
  KIMLIK_GECERLILIK_TARIHI: Date | null;
  FAALIYET_BELGESI_ALINDI: boolean | null;
  VERGI_LEVHASI_ALINDI: boolean | null;
  IMZA_SIRKULERI_ALINDI: boolean | null;
  IMZA_SIRKU_GECERLILIK_TARIHI: Date | null;
  YETKILI_KIMLIK_NO: string | null;
  YETKILI_KMLK_GECERLIK_TARIH: Date | null;
  FILTRE: string | null;
  CARI_BAKIYE_SINIRI: number | null;
  SIRKET_TURU: number | null;
  KIMLIK_BELGE_TURU: number | null;
  DERNEK_AMACI: string | null;
  YETKILI_KISI_ID: number | null;
  FAVORI_PARA_ID: number | null;
  WHATSAPP_ADI: string | null;
...
export interface CariKartInputDto {
  kod: string;
  ad: string;
  kisilikTipi?: number;
  yetkiliKisi?: string | null;
...
const formatDateStr = (d: Date | null | undefined): string | null => {
  if (!d) return null;
  try {
...
export class CariSqlRepository {
  private static mapEntityToModel(entity: TodvzCariKartEntity): CariKartModel {
    return {
  ...
  public static async findAll(dbContext?: { dbServer?: string; dbName?: string }): Promise<CariKartModel[]> {
    try {
      ...
      `;
      const result = await pool.request().query<TodvzCariKartEntity>(query);
  ...
  public static async findById(
    ...
    try {
      ...
      `;
      const result = await request.query<TodvzCariKartEntity>(query);
  ...
  public static async getLookups(dbContext?: { dbServer?: string; dbName?: string }) {
    try {
      ...
      const ilceList = items.filter((x: any) => x.tur === 2);
      ...
      const postaKoduList = items.filter((x: any) => x.tur === 4);
      ...
      return {
        ...
        sektorList: items.filter((x: any) => x.tur === 8),
        meslekList,
        bankaList,
  ...
  public static async ensureTabloMaddesi(
    tur: number,
    ad: string,
    kod: string | null = null,
  ...
  private static async sanitizeFkIds(data: CariKartInputDto, pool: sql.ConnectionPool): Promise<CariKartInputDto> {
    const copy = { ...data };
    ...
    if (copy.ilAdi && (!copy.ilId || isNaN(Number(copy.ilId)))) {
      ...
      if (resolvedIlId) copy.ilId = resolvedIlId;
    }
    if (copy.ilceAdi && (!copy.ilceId || isNaN(Number(copy.ilceId)))) {
      ...
      if (resolvedIlceId) copy.ilceId = resolvedIlceId;
    }
    ...
    if (tmIdsToCheck.length > 0) {
      const res = await pool.request().query<{ TABLO_MADDESI_ID: number }>(`
      ...
      if (copy.vergiDairesiId && !validTmIds.has(Number(copy.vergiDairesiId))) copy.vergiDairesiId = null;
      if (copy.postaKoduId && !validTmIds.has(Number(copy.postaKoduId))) copy.postaKoduId = null;
      if (copy.ilceId && !validTmIds.has(Number(copy.ilceId))) copy.ilceId = null;
      if (copy.ilId && !validTmIds.has(Number(copy.ilId))) copy.ilId = null;
      if (copy.hukukiYapiId && !validTmIds.has(Number(copy.hukukiYapiId))) copy.hukukiYapiId = null;
      if (copy.sektorId && !validTmIds.has(Number(copy.sektorId))) copy.sektorId = null;
      if (copy.meslekId && !validTmIds.has(Number(copy.meslekId))) copy.meslekId = null;
    }

    // 2. Check ULKE ids
    const ulkeIdsToCheck = [copy.ulkeId, copy.uyrukId].filter((id) => id !== null && id !== undefined);
    if (ulkeIdsToCheck.length > 0) {
      const res = await pool.request().query<{ ULKE_ID: number }>(`
      ...
      if (copy.ulkeId && !validUlkeIds.has(Number(copy.ulkeId))) copy.ulkeId = null;
      if (copy.uyrukId && !validUlkeIds.has(Number(copy.uyrukId))) copy.uyrukId = null;
    }

    // 3. Check PARA id
    if (copy.favoriParaId) {
      const res = await pool.request().query<{ PARA_ID: number }>(`
        SELECT [PARA_ID] FROM [dbo].[TODVZ_PARA] WHERE [PARA_ID] = ${Number(copy.favoriParaId)}
      `);
      if (res.recordset.length === 0) copy.favoriParaId = null;
    }
    ...
    if (cariIdsToCheck.length > 0) {
      const res = await pool.request().query<{ CARI_KART_ID: number }>(`
      ...
      if (copy.bankaHesabiId && !validCariIds.has(Number(copy.bankaHesabiId))) copy.bankaHesabiId = null;
      if (copy.yetkiliKisiId && !validCariIds.has(Number(copy.yetkiliKisiId))) copy.yetkiliKisiId = null;
    }
    ...
    if (istIdsToCheck.length > 0) {
      const res = await pool.request().query<{ ISTATISTIK_ID: number }>(`
      ...
      if (copy.alisIstatistikId && !validIstIds.has(Number(copy.alisIstatistikId))) copy.alisIstatistikId = null;
      if (copy.satisIstatistikId && !validIstIds.has(Number(copy.satisIstatistikId))) copy.satisIstatistikId = null;
      if (copy.arbitrajAlisIstatistikId && !validIstIds.has(Number(copy.arbitrajAlisIstatistikId))) copy.arbitrajAlisIstatistikId = null;
      if (copy.arbitrajSatisIstatistikId && !validIstIds.has(Number(copy.arbitrajSatisIstatistikId))) copy.arbitrajSatisIstatistikId = null;
    }

    return copy;
  }

  private static bindInputs(request: sql.Request, data: CariKartInputDto) {
  ...
  public static async saveViaProcedure(
    data: CariKartInputDto,
    cariKartId?: number | string | null,
    dbContext?: { dbServer?: string; dbName?: string }
    ...
    try {
      ...
      logger.info(
        `[SODVZ_CARI_KART_KAYDET] Çağrılıyor: CARI_KART_ID=${idNum ?? "NULL (INSERT)"}, KOD='${(sanitized.kod || "").trim()}', AD='${(sanitized.ad || "").trim()}'`
      );
      ...
      const result = await request.query<{ CARI_KART_ID: number }>(execQuery);
