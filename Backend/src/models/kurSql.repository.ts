  ...
  public static async saveViaProcedure(
    ...
    if (!targetId && (tur === 0 || tur === 1)) {
      ...
      const checkRes = await pool
        ...
        .query<{ KUR_TABLOSU_ID: number }>(
          "SELECT TOP 1 [KUR_TABLOSU_ID] FROM [dbo].[TODVZ_KUR_TABLOSU] WHERE [TUR] = @tur AND CAST([TARIH] AS DATE) = CAST(@tarih AS DATE) ORDER BY [KUR_TABLOSU_ID] DESC"
        );
      if (checkRes.recordset.length > 0) {
        targetId = checkRes.recordset[0].KUR_TABLOSU_ID;
      }
    ...
    let finalKurTablosuId: number = targetId || 0;
    let kapanisId: number | null = null;
    ...
    try {
      ...
      if (targetId) {
        await pool
          ...
          .query(
            "UPDATE [dbo].[TODVZ_KUR_TABLOSU] SET [ZAMAN] = @zaman, [TARIH] = CAST(@zaman AS DATE) WHERE [KUR_TABLOSU_ID] = @id"
          );
