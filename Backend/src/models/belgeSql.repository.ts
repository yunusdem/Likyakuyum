...
export interface BelgeSablonModel {
...
export class BelgeSqlRepository {
  ...
  private static async pool(ctx?: DbContext): Promise<sql.ConnectionPool> {
    const pool = await getDbPool(ctx?.dbServer, ctx?.dbName);
    const anahtar = `${ctx?.dbServer || ""}|${ctx?.dbName || ""}`;
    if (!this.hazirlanan.has(anahtar)) { await this.ensureTablesExist(pool); this.hazirlanan.add(anahtar); }
    return pool;
  }
  ...
  public static async ensureTablesExist(pool: sql.ConnectionPool): Promise<void> {
    try {
      ...
      `);
    } catch (e) { logger.error("BelgeSqlRepository.ensureTablesExist hatası:", e); throw e; }
  }

  private static sablonMap(r: any): BelgeSablonModel {
  ...
  static async sablonlar(tur: "BELGE" | "RAPOR" | undefined, ctx?: DbContext): Promise<BelgeSablonModel[]> {
  ...
  static async sablonBul(kod: string, ctx?: DbContext): Promise<BelgeSablonModel> {
    const pool = await this.pool(ctx);
    ...
    if (!res.recordset[0]) throw ApiError.notFound(`Şablon bulunamadı: ${kod}`);
    const s = this.sablonMap(res.recordset[0]);
    if (!s.aktif) throw ApiError.badRequest(`Şablon pasif: ${kod}`);
    return s;
  }

  static async varsayilanSablon(fisTipi: number, ctx?: DbContext): Promise<BelgeSablonModel> {
    const pool = await this.pool(ctx);
    ...
    if (!res.recordset[0]) throw ApiError.notFound(`${fisTipi === 1 ? "Satış" : "Alış"} fişi için aktif şablon yok.`);
   ...
   */
  static async fisDetay(p: { fisId?: number; belgeNo?: string }, ctx?: DbContext): Promise<Record<string, any>> {
    if (!p.fisId && !p.belgeNo) throw ApiError.badRequest("fisId veya belgeNo verilmelidir.");
    const pool = await this.pool(ctx);
    ...
    if (basliklar.length === 0) throw ApiError.notFound(`Döviz fişi bulunamadı (${p.belgeNo || "fiş " + p.fisId}).`);
    if (basliklar.length > 1) throw ApiError.conflict("Birden fazla fiş eşleşti; belge numarasıyla arayın.");
    const baslik = basliklar[0];
    let detay: Record<string, any> = {};
    try {
      ...
      detay = x.recordset[0] || {};
    } catch (e) { logger.warn("Belge: XSLT görünümü okunamadı, yalnızca başlık kullanılıyor.", e); }
    return { ...detay, ...baslik };
  }
  ...
  static async firmaBelgeDizini(ctx?: DbContext): Promise<string | null> {
    try {
      const pool = await this.pool(ctx);
    ...
    } catch (e) { logger.warn("Belge: TODVZ_TANIM.BELGE_DIZINI okunamadı.", e); return null; }
  }
}
