...
const sifreyiYaz = (kullaniciAdi: string, geciciSifre: string) => {
  console.log("");
...
const olustur = async (kullaniciAdi: string, adSoyad: string) => {
  const { toplam } = await AdminSqlRepository.sayilar();
  if (toplam > 0) {
    throw new Error(
      "Sistemde zaten admin var. Yeni adminler panelden eklenir; kurtarma için: --sifirla <kullaniciAdi>"
    );
  }
  ...
  if (adminId === null) throw new Error("Admin oluşturulamadı.");
...
const sifirla = async (kullaniciAdi: string) => {
  const admin = await AdminSqlRepository.kullaniciAdiIleBul(kullaniciAdi);
  if (!admin) throw new Error(`'${kullaniciAdi}' adında bir admin yok.`);
...
const calistir = async () => {
  if (!adminYapilandirildiMi()) {
    throw new Error(
      "Backend/.env içinde ADMIN_DB_USER, ADMIN_DB_PASSWORD ve en az 32 karakterlik ADMIN_JWT_SECRET ile ADMIN_DB_ENC_KEY tanımlı olmalı."
    );
  }
  const args = process.argv.slice(2);

  if (args[0] === "--sifirla") {
    if (!args[1]) throw new Error("Kullanım: npm run ilk-admin -- --sifirla <kullaniciAdi>");
