...
export class BelgeService {
  static sablonlar(tur: "BELGE" | "RAPOR" | undefined, ctx?: DbContext) { return BelgeSqlRepository.sablonlar(tur, ctx); }
  static fisler(f: Parameters<typeof BelgeSqlRepository.fisler>[0], ctx?: DbContext) { return BelgeSqlRepository.fisler(f, ctx); }
   ...
   */
  static async pdf(istek: BelgeIstek, kullanici: string, ctx?: DbContext): Promise<BelgeSonuc> {
    ...
    if (sablon.tur !== "BELGE") throw ApiError.badRequest(`${sablon.kod} bir belge şablonu değil.`);
  ...
  static async arsivle(istek: BelgeIstek, kullanici: string, ctx?: DbContext) {
