...
const ozet = (anahtar: string): string => crypto.createHash("sha256").update(anahtar, "utf8").digest("hex");

const htmlKacis = (s: string): string =>
  s.replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c]!);

const mailIcerigi = (firma: FirmaDto, baglanti: string) => {
...
export class EpostaDogrulamaService {
  ...
  public static mailDurumu(): { yapilandirildi: boolean; gonderen: string | null; gecerlilikSaat: number } {
  ...
  public static async gonder(yapan: AdminBaglam, firmaId: number): Promise<FirmaDto> {
    const firma = await FirmaSqlRepository.idIleBul(firmaId);
    if (!firma) throw ApiError.notFound("Firma bulunamadı.");
    if (!firma.eposta) throw ApiError.badRequest("Firmanın e-posta adresi tanımlı değil. Önce Bilgi & Bağlantı sekmesinden girin.");
    if (firma.epostaDogrulandi) throw ApiError.badRequest("Bu e-posta adresi zaten doğrulanmış.");
   ...
   */
  public static async onayla(anahtar: string): Promise<{ unvan: string; eposta: string }> {
    const sonuc = await FirmaSqlRepository.epostaAnahtariylaDogrula(ozet(anahtar));
    if (!sonuc) throw ApiError.badRequest("Bağlantı geçersiz ya da süresi dolmuş. Hizmet sağlayıcınızdan yeni bir doğrulama maili isteyin.");
  ...
  public static async elleAyarla(yapan: AdminBaglam, firmaId: number, dogrulandi: boolean): Promise<FirmaDto> {
    const firma = await FirmaSqlRepository.idIleBul(firmaId);
    if (!firma) throw ApiError.notFound("Firma bulunamadı.");
    if (dogrulandi && !firma.eposta) throw ApiError.badRequest("Firmanın e-posta adresi tanımlı değil.");
