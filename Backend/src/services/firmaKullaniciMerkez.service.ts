...
import { MerkezBaglam, MerkezGirisService } from "./merkezGiris.service.js";
...
export class FirmaKullaniciMerkezService {
  public static baglam(oturum: JwtPayload | undefined): Promise<MerkezBaglam | null> {
  ...
  public static limitMesaji(b: MerkezBaglam): string {
  ...
  public static olusturmaOnDenetimi(b: MerkezBaglam, girdi: { username?: string; password?: string }): void {
    MerkezGirisService.yoneticiOlmali(b);
    if (!girdi.password) throw ApiError.badRequest("Yeni kullanıcı için bir başlangıç şifresi girilmelidir.");
    MerkezGirisService.sifreKuraliniDenetle(girdi.password);
    const limit = b.firma.aktifLisans?.kullaniciLimiti;
    if (limit === undefined || b.firma.kullaniciSayisi >= limit) throw ApiError.forbidden(this.limitMesaji(b));
  }
   ...
   */
  public static async olusturmaSonrasi(
    b: MerkezBaglam,
    yeni: UserResponseDto,
    sifre: string,
    dbContext: DbContext
  ): Promise<void> {
    const geriAl = async () => {
    ...
    try {
      ...
      await geriAl();
      if (benzersizIhlalMi(err)) throw ApiError.conflict(`"${yeni.username}" kullanıcı adı merkezde zaten kayıtlı.`);
      throw err;
    }
  }
   ...
   */
  public static guncellemeOnDenetimi(b: MerkezBaglam, mevcutKullaniciAdi: string, girdi: { password?: string }): void {
    const kendisi = mevcutKullaniciAdi === b.kullanici.kullaniciAdi;
    if (!kendisi) MerkezGirisService.yoneticiOlmali(b);
    if (girdi.password) {
      if (kendisi) throw ApiError.badRequest("Kendi şifrenizi 'Şifre Değiştir' ekranından değiştirin.");
      MerkezGirisService.sifreKuraliniDenetle(girdi.password);
    }
  }

  /** Güncellemeden sonra: kullanıcı adı / ad / yönetici bilgisi ve (verildiyse) şifre merkeze işlenir. */
  public static async guncellemeSonrasi(
    b: MerkezBaglam,
    eskiKullaniciAdi: string,
    guncel: UserResponseDto,
    yeniSifre: string | undefined
  ): Promise<void> {
    const hedef = await KullaniciSqlRepository.adIleBul(b.firma.firmaId, eskiKullaniciAdi);
    if (!hedef) return; // henüz içe aktarılmamış kullanıcı: merkezde karşılığı yok
    ...
    if (guncel.username && guncel.username !== hedef.kullaniciAdi) {
      try {
        ...
        if (benzersizIhlalMi(err)) throw ApiError.conflict(`"${guncel.username}" kullanıcı adı merkezde zaten kayıtlı.`);
        throw err;
      }
    }
  ...
  public static silmeEngeli(): never {
  ...
  public static async sifreSifirla(
    oturum: JwtPayload | undefined,
    hedefDbKullaniciId: string,
    dbContext: DbContext
  ): Promise<{ geciciSifre: string }> {
    const b = await this.baglam(oturum);
    if (!b) throw ApiError.badRequest("Şifre sıfırlama bu kurulumda etkin değil.");
    MerkezGirisService.yoneticiOlmali(b);

    const hedefDb = await UserSqlRepository.findById(hedefDbKullaniciId, dbContext);
    if (!hedefDb) throw ApiError.notFound("Kullanıcı bulunamadı.");
    const hedef = await KullaniciSqlRepository.adIleBul(b.firma.firmaId, hedefDb.username);
    if (!hedef) throw ApiError.notFound("Kullanıcı merkezde tanımlı değil. Lütfen hizmet sağlayıcınızla iletişime geçiniz.");
