...
/**
 * UBL-TR e-İrsaliye (`DespatchAdvice`) üreteci — Faz 9.
 *
 ...
 *  IssueDate → IssueTime → DespatchAdviceTypeCode → Note[] → LineCountNumeric →
 ...
 *
 * ## Faturadan en önemli farkı
 *
 * **İrsaliyede tutar yoktur.** `DespatchLine` içinde `Price`, `LineExtensionAmount`
 * veya `TaxTotal` bulunmaz; yalnızca sevk edilen miktar ve mal bilgisi taşınır.
 * Bu yüzden bu üreteçte hiçbir parasal hesap yapılmaz.
 */
...
/** UBL-TR e-İrsaliye senaryoları */
export type IrsaliyeSenaryo = "TEMELIRSALIYE";
...
export interface IrsaliyeSatiri {
  ...
  /** Sevk edilen miktar */
  miktar: number;
  /** UN/ECE birim kodu — adet C62, gram GRM, kilogram KGM */
  birimKodu?: string;
  /** Satıcı stok kodu */
  stokKodu?: string;
  marka?: string;
...
export interface IrsaliyeGirdi {
  ...
  tarih?: string;
  saat?: string;
  senaryo?: IrsaliyeSenaryo;
  irsaliyeTipi: IrsaliyeTipi;
  notlar?: string[];
  /** Malı gönderen — bizim firmamız */
  gonderici: UblTaraf;
  /** Malın teslim edileceği taraf */
  alici: UblTaraf;
...
const gecerliTarih = (t: string): boolean => {
...
export const dogrulaIrsaliye = (girdi: IrsaliyeGirdi): void => {
  const belgeNo = girdi.belgeNo?.trim() || "";
  if (!/^[A-Z0-9]{3}\d{13}$/.test(belgeNo)) {
    throw ApiError.badRequest(
      "İrsaliye numarası 3 karakter seri + 13 hane biçiminde olmalıdır. Örnek: ABC2026000000001"
    );
  }
  ...
  girdi.satirlar.forEach((satir, i) => {
    const no = i + 1;
    if (!satir.ad?.trim()) throw ApiError.badRequest(`${no}. satırda mal adı zorunludur.`);
  ...
  if (!sevk) throw ApiError.badRequest("Sevkiyat bilgisi zorunludur.");
...
export interface IrsaliyeUretimSonucu {
  xml: string;
  uuid: string;
  satirSayisi: number;
}

export const buildDespatchAdviceXml = (girdi: IrsaliyeGirdi): IrsaliyeUretimSonucu => {
