...
export type FaturaSenaryo =
  ...
  | "YATIRIMTESVIK" | "KAMU"
  // 2. tur (20.09.2026): ICE portalindeki diğer senaryolar. ProfileID değerleri GİB UBL-TR
  // kılavuzuna göre yazıldı; her biri gönderimden önce ICE doğrulamasından geçer.
...
export type FaturaTipi =
  ...
  | "TEKNOLOJIDESTEK" | "TEVKIFATIADE" | "SGK"
  | "HALTIPISATIS" | "HALTIPIKOMISYONCU"
  | "YTBSATIS" | "YTBISTISNA" | "YTBIADE";

/** Yatırım teşvik belgesi (YTB No / Tarihi) isteyen tipler. */
export const ytbTipiMi = (t: FaturaTipi): boolean => t === "YTBSATIS" || t === "YTBISTISNA" || t === "YTBIADE";

/** İade niteliğindeki tipler: dayanak fatura (`cac:BillingReference`) zorunludur. */
export const iadeTipiMi = (t: FaturaTipi): boolean => t === "IADE" || t === "TEVKIFATIADE" || t === "YTBIADE";
...
export interface UblTaraf {
  ...
  soyad?: string;
  vergiDairesi?: string;
  /** Mahalle / cadde / sokak — UBL `cbc:StreetName` */
  adres?: string;
  /** Bina adı / sitesi — `cbc:BuildingName` */
  binaAdi?: string;
  /** Bina (dış kapı) no — `cbc:BuildingNumber` */
  binaNo?: string;
  /** Kapı / daire no — `cbc:Room` */
  kapiNo?: string;
...
export interface UblFaturaSatiri {
  ...
  ad: string;
  aciklama?: string;
  /** Satıcının kendi mal/hizmet kodu — `cac:SellersItemIdentification` */
  hizmetKodu?: string;
  /** Satır notu — `cbc:Note` */
  not?: string;
  miktar: number;
  ...
  iskontoOrani?: number;
  /**
   * İskonto tutarı doğrudan girildiyse oran yerine bu kullanılır (ICE formunda "İskonto Tutarı" kolonu).
   * Oran, tutardan geri hesaplanıp `MultiplierFactorNumeric` olarak yazılır.
   */
  ...
  /**
   * Senaryoya özel satır alanları (ICE formundaki ek kolonlar). GİB'in her biri için beklediği özel yapı
   * doğrulanmadığından `cac:AdditionalItemProperty` (ad/değer) olarak yazılırlar; GTİP ise
   * `cac:CommodityClassification` ile bildirilir. ICE doğrulaması reddederse belge kesilmez.
   */
  gtip?: string;
  teslimSarti?: string;
  kapCinsi?: string;
...
export interface UblFaturaGirdi {
  ...
  /** Sipariş bilgisi — `cac:OrderReference` */
  siparis?: { no: string; tarih?: string };
  /** İrsaliye bilgileri — `cac:DespatchDocumentReference` (birden çok olabilir) */
  irsaliyeler?: { no: string; tarih?: string }[];
  /**
   * ÖKC (ödeme kaydedici cihaz) bilgisi — `cac:AdditionalDocumentReference`.
   * Her alan ayrı bir referans olarak, `cbc:DocumentType` içinde alan adıyla yazılır.
   */
  okc?: { fisNo?: string; fisTipi?: string; fisTarihi?: string; fisSaati?: string; okcNo?: string; zNo?: string };
  /** EK belge listesi — `cac:AdditionalDocumentReference` */
  ekBelgeler?: { no: string; tarih?: string; ad?: string; tur?: string; turKodu?: string }[];
  /** IBAN bilgisi — `cac:PaymentMeans` / `cac:PayeeFinancialAccount` */
  iban?: { iban: string; paraBirimi?: string };
  /** İhracat: yurt dışındaki alıcı — `cac:BuyerCustomerParty` + `cac:Delivery` */
  ihracat?: {
    firmaUnvani?: string; vkn?: string; ulke?: string; sehir?: string; ilce?: string;
    teslimSarti?: string; gonderimSekli?: string;
  };
  /** Yolcu beraberi eşya: turist ve iade hesabı */
  turist?: {
    ad?: string; soyad?: string; ulke?: string; uyruk?: string; sehir?: string; ilce?: string;
    pasaportNo?: string; pasaportTarihi?: string;
    bankaAdi?: string; subeAdi?: string; hesapNo?: string; hesapParaBirimi?: string; odemeNotu?: string;
  };
  ...
  /** IDIS sevkiyat numarası */
  idisSevkiyatNo?: string;
  /** Yatırım teşvik belgesi no / tarihi */
  ytb?: { no?: string; tarih?: string };
  /** Belge seviyesinde KDV istisna / ihraç kayıtlı muafiyet sebebi */
  muafiyetSebebi?: string;
  /** Hal tipi komisyoncu masrafları — `cac:AllowanceCharge` (masraf) */
  halMasraflari?: { ad: string; tutar: number; kdvOrani?: number }[];
  /** e-Arşiv: belge tipi ve gönderim şekli */
...
export const dogrulaGirdi = (girdi: UblFaturaGirdi): void => {
  ...
  (girdi.halMasraflari || []).forEach((m, i) => {
    if (!m.ad?.trim()) throw ApiError.badRequest(`${i + 1}. masraf satırında açıklama zorunludur.`);
    if (!Number.isFinite(m.tutar) || m.tutar < 0) throw ApiError.badRequest(`${i + 1}. masraf satırında tutar sıfır ya da pozitif olmalıdır.`);
  });

  const tarihGecerli = (t?: string) => !t || /^\d{4}-\d{2}-\d{2}$/.test(t);
  if (!tarihGecerli(girdi.siparis?.tarih)) throw ApiError.badRequest("Sipariş tarihi YYYY-AA-GG biçiminde olmalıdır.");
  if (!tarihGecerli(girdi.ytb?.tarih)) throw ApiError.badRequest("YTB tarihi YYYY-AA-GG biçiminde olmalıdır.");
  if (!tarihGecerli(girdi.turist?.pasaportTarihi)) throw ApiError.badRequest("Pasaport tarihi YYYY-AA-GG biçiminde olmalıdır.");
  (girdi.irsaliyeler || []).forEach((r, i) => {
    if (!r.no?.trim()) throw ApiError.badRequest(`${i + 1}. irsaliye satırında irsaliye numarası zorunludur.`);
    if (!tarihGecerli(r.tarih)) throw ApiError.badRequest(`${i + 1}. irsaliye satırının tarihi YYYY-AA-GG biçiminde olmalıdır.`);
  });
  (girdi.ekBelgeler || []).forEach((r, i) => {
    if (!r.no?.trim()) throw ApiError.badRequest(`${i + 1}. ek belge satırında belge numarası zorunludur.`);
    if (!tarihGecerli(r.tarih)) throw ApiError.badRequest(`${i + 1}. ek belge satırının tarihi YYYY-AA-GG biçiminde olmalıdır.`);
  });
  if (!tarihGecerli(girdi.okc?.fisTarihi)) throw ApiError.badRequest("ÖKC fiş tarihi YYYY-AA-GG biçiminde olmalıdır.");
 ...
 */
const belgeReferansXml = (
  etiketAdi: string,
  r: { no: string; tarih?: string; tur?: string; turKodu?: string; ad?: string }
): string =>
  `<${etiketAdi}>` +
...
const okcXml = (okc: UblFaturaGirdi["okc"], tarih: string): string => {
  if (!okc) return "";
  const alanlar: [string, string | undefined][] = [
  ...
  return alanlar
    ...
    .map(([kod, deger]) =>
      belgeReferansXml("cac:AdditionalDocumentReference", { no: deger!.trim(), tarih: okc.fisTarihi || tarih, tur: kod })
    )
    .join("");
};

/** IBAN — `cac:PaymentMeans`. PaymentMeansCode zorunludur; UBL-TR'de banka havalesi için ZZZ kullanılır. */
const paymentMeansXml = (iban: UblFaturaGirdi["iban"], belgeParaBirimi: string): string => {
...
const itemPropertyXml = (ad: string, deger?: string | number): string =>
  deger === undefined || deger === null || String(deger).trim() === ""
    ? ""
...
const satirEkAlanlariXml = (satir: UblFaturaSatiri): string =>
  (satir.gtip?.trim()
 ...
 */
const ihracatAliciXml = (g: UblFaturaGirdi): string => {
  const i = g.ihracat;
  if (!i?.firmaUnvani?.trim()) return "";
...
const turistXml = (g: UblFaturaGirdi): string => {
  const t = g.turist;
  if (!t?.pasaportNo?.trim() && !t?.ad?.trim()) return "";
  return `<cac:BuyerCustomerParty><cac:Party>` +
    `<cac:PostalAddress>` +
...
/** Yolcu beraberi eşya: aracı kurum — `cac:TaxRepresentativeParty`. */
const araciKurumXml = (g: UblFaturaGirdi): string => {
  const a = g.araciKurum;
  if (!a?.vknTckn?.trim() && !a?.unvan?.trim()) return "";
...
const turistHesabiXml = (g: UblFaturaGirdi): string => {
  const t = g.turist;
  if (!t?.hesapNo?.trim()) return "";
  return `<cac:PaymentMeans>` +
    `<cbc:PaymentMeansCode>ZZZ</cbc:PaymentMeansCode>` +
...
const masraflarXml = (g: UblFaturaGirdi, paraBirimi: string): string =>
  (g.halMasraflari || [])
    .filter((m) => m.ad?.trim())
    .map((m) =>
      `<cac:AllowanceCharge>` +
