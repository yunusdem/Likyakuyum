...
import { IceBelgeStatu, IceGonderimSonucu, toArray } from "./ice.efatura.js";
import { ApiError } from "../../utils/ApiError.js";

export interface IceEarsivKaydi {
  UUID?: string;
  ID?: string;
  STATUS_CODE?: string | number;
  STATUS_DESCRIPTION?: string;
  HEADER?: {
    CUSTOMER?: string; CUSTOMER_TITLE?: string; SUPPLIER?: string; SUPPLIER_TITLE?: string;
    CDATE?: string; ISSUE_DATE?: string; PAYABLE_AMOUNT?: unknown; PROFILEID?: string;
  };
}

/** Yalnız başlık okur. WSDL'de sayfa/ofset veya DIRECTION alanı yoktur. */
export const getEArchive = async (config: IceConnectionConfig, filtre: {
  baslangic: Date; bitis: Date; limit: number;
}): Promise<IceEarsivKaydi[]> => {
  const { data } = await callWithSession<{ EArchive?: IceEarsivKaydi | IceEarsivKaydi[] }>(config, {
    method: "GetEArchive",
  ...
  if (data == null) throw ApiError.unprocessable("ICE arşiv yanıtı alınamadı.");
  if (typeof data !== "object" && data !== "") throw ApiError.unprocessable("ICE arşiv yanıtının biçimi geçersiz.");
  return toArray(data && typeof data === "object" ? data.EArchive : undefined)
    .filter((r): r is IceEarsivKaydi => !!r && typeof r === "object");
};

export const setEArchiveStatus = async (config: IceConnectionConfig, uuid: string, statu: IceBelgeStatu): Promise<boolean> => {
...
export interface IceEarsivIptalSonucu {
 ...
 */
export const sendEarsivIptal = async (
  config: IceConnectionConfig,
  faturaNo: string,
  iptalTarihi: Date
): Promise<IceEarsivIptalSonucu> => {
  const { data } = await callWithSession<IceEarsivIptalSonucu>(config, {
...
export interface IcePreviewSonucu {
  success?: boolean | string;
  /** base64 PDF */
  response_message?: string;
}
...
export type EmailBelgeTuru = (typeof EMAIL_BELGE_TURLERI)[number];

export interface EmailAlicisi {
  /** Alıcı adı — boş bırakılabilir */
  unvan?: string;
  eposta: string;
}

export interface IceEmailAliciSonucu {
  Title?: string;
  Email?: string;
...
export const sendDocumentEmail = async (
  ...
  belgeler: {
    belgeNo?: string;
    uuid: string;
    belgeTuru: EmailBelgeTuru;
    alicilar: EmailAlicisi[];
  }[]
): Promise<IceEmailAliciSonucu[]> => {
  ...
  const belgeCevaplari = toArray<any>(data?.DocumentEmailResponseList?.DocumentEmailResponse);
  const sonuclar: IceEmailAliciSonucu[] = [];
  for (const belge of belgeCevaplari) {
    for (const alici of toArray<IceEmailAliciSonucu>(belge?.EmailResponseList?.EmailRecipientResponse)) {
      sonuclar.push(alici);
    }
  }
  return sonuclar;
...
export interface IceEarsivRaporStatu {
  ETTN?: string;
  Raporlandi?: boolean | string;
  Rapor_ETTN?: string;
  Gelis_Tarihi?: string;
  Imza_Tarihi?: string;
  Rapor_Tarihi?: string;
  Rapor_Durum_Kodu?: string;
  Rapor_Durum_Aciklama?: string;
}
 ...
 */
export const getEarsivRaporStatu = async (
  config: IceConnectionConfig,
  ettnler: string[]
): Promise<IceEarsivRaporStatu[]> => {
  const { data } = await callWithSession<any>(config, {
  ...
  return toArray<IceEarsivRaporStatu>(
    data?.Earsiv_Rapor_Status?.Earsiv_Rapor_Statu ?? data?.Earsiv_Rapor_Statu
  );
};

export interface IceEmailStatu {
  ETTN?: string;
  statu?: string;
 ...
 */
export const getEarsivMailStatu = async (
  config: IceConnectionConfig,
  ettnler: string[]
): Promise<IceEmailStatu[]> => {
  const { data } = await callWithSession<any>(config, {
  ...
  return toArray<IceEmailStatu>(data?.email_status?.Email_Statu ?? data?.Email_Statu);
};
