...
export class MasakService {
  ...
  private static listeKodGecerliMi(kod: unknown): kod is MasakListeKod {
    return typeof kod === "string" && (MASAK_LISTE_KODLARI as string[]).includes(kod);
  }
  ...
  public static async getDurum(dbContext?: DbContext): Promise<MasakListeDurumModel[]> {
    ...
    return durum.map((d) => ({
      ...d,
      listeAdi: d.listeAdi || MASAK_KAYNAKLAR[d.listeKod as MasakListeKod]?.listeAdi || null,
      kaynakUrl:
        d.kaynakUrl || MASAK_KAYNAKLAR[d.listeKod as MasakListeKod]?.varsayilanUrl || null,
    }));
  }
  ...
  public static async guncelle(
    ...
    for (const k of istenen) {
      if (!MasakService.listeKodGecerliMi(k?.listeKod)) {
        throw ApiError.badRequest(
          `Geçersiz liste kodu: ${String(k?.listeKod)}. Geçerli değerler: A, B, C, 3AB`
        );
      }
      ...
      for (const girdi of istenen) {
        ...
        try {
          ...
          const yazilan = await MasakSqlRepository.replaceListe(
            listeKod,
            ayristirma.kayitlar,
            { listeAdi: tanim.listeAdi, kaynakUrl: url, kaynakHash: dosya.hash },
            dbContext
          );
