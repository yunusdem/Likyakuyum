...
export const KASA_SORGULARI: Record<string, Sorgu> = {
  ...
  async KASHAR1(pool, p, t) {
    ...
    return sinirla(satirlar, t, `${aralikOzeti(p)}${ozetEk(p)}${adetOzeti(p.hesapIdler, "hesap")}${p.kasaTipi === 0 ? " · Giriş" : p.kasaTipi === 1 ? " · Çıkış" : ""}`,
      "Toplamlar farklı para birimlerini birlikte içerebilir; para bazında toplam için sıralamayı \"Para\" seçin. KDV tutarı TL'dir.");
  },
  ...
  async HESEKS1(pool, p, t) {
    ...
    const gruplar = new Map<string, { anahtar: string; devir: number; ortak: Record<string, any>; hareketler: any[] }>();
    for (const r of res.recordset) {
      const k = `${r.hesapKod}|${r.paraKod}`, meblag = Number(r.meblag) || 0, giris = Number(r.tip) === 0;
      if (!gruplar.has(k)) gruplar.set(k, { anahtar: k, devir: 0, hareketler: [], ortak: { grupAnahtar: k, grupBaslik: `${r.hesapKod} — ${r.hesapAd} · ${r.paraKod}`, hesapKod: r.hesapKod, hesapAd: r.hesapAd, paraKod: r.paraKod } });
      const g = gruplar.get(k)!;
      if (r.onceki) g.devir += giris ? meblag : -meblag;
      else g.hareketler.push({ ...r, giris: giris ? meblag : 0, cikis: giris ? 0 : meblag, kdv: Number(r.kdv) || 0 });
    }
    ...
    return sinirla(satirlar, t, `${aralikOzeti(p)}${adetOzeti(p.hesapIdler, "hesap")}${ozetEk(p)}`,
      "Her hesap ve para birimi ayrı gruplanır; ilk satır başlangıç tarihinden önceki devirdir. Bakiye = girişler − çıkışlar (KDV hariç; KDV TL kasasına ayrıca yansır).");
  },
  ...
  async HESBAK1(pool, p, t) {
    ...
    return sinirla(satirlar, t, `${tarihTr(bas)} – ${tarihTr(bit)}${adetOzeti(p.hesapIdler, "hesap") || " · Tüm hesaplar"}${ozetEk(p)} · ${kur.aciklama}`,
      `Giriş ve çıkış seçilen tarih aralığındaki hareketlerin toplamıdır; bakiye bitiş tarihi itibarıyla tüm hareketlerden hesaplanır (girişler − çıkışlar, KDV hariç). Hareketi olmayan hesaplar da listelenir. ${kur.aciklama}.`);
  },
};
