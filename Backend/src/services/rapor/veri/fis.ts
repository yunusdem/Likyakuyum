...
export const FIS_SORGULARI: Record<string, Sorgu> = {
  ...
  async FISLIS1(pool, p, t) {
    ...
    const satirlar = res.recordset.map((r: any) => { const ilk = !gorulen.has(Number(r.fisId)); gorulen.add(Number(r.fisId));
    ...
    const ozet = paraOzeti(satirlar.map((x: any) => ({ fisId: x.fisId, paraKod: x.paraKod, tip: x.tip, miktar: x.miktar, tutar: x.tutar, komisyon: x.komisyon, bmv: x.bmv, kmv: x.kmv, kdv: x.kdv })))
      .map(o => ({ ...o, satirAdedi: satirlar.filter((x: any) => x.paraKod === o.paraKod && x.tip === o.tip).length }));
    return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p) || " · Tüm vezneler"}${adetOzeti(p.istatistikIdler, "istatistik")} · ${gorulen.size} fiş, ${satirlar.length} satır`,
      "Her satır bir fiş satırıdır; gün bazında gruplanır. Lira = satır tutarı. Toplam tutar ve ödeme tutarı fişin ilk satırında gösterilir. Para seçilirse yalnızca o paranın satırları listelenir. İptal fişler hariçtir.",
      ozet);
  },
  ...
  async ISTRAP1(pool, p, t) {
    ...
    const ham = res.recordset.map((r: any) => ({ ...r, giris: Number(r.tipKod) === 0, miktar: Number(r.miktar) || 0, kur: Number(r.kur) || 0, tutar: Number(r.tutar) || 0,
    ...
    const topla = (anahtar: (x: any) => string, ortak: (x: any) => Record<string, any>) => { const m = new Map<string, any>();
      for (const x of ham) { const k = anahtar(x); if (!m.has(k)) m.set(k, { ...ortak(x), gF: new Set<number>(), cF: new Set<number>(), girisMiktar: 0, girisTutar: 0, cikisMiktar: 0, cikisTutar: 0, satirAdedi: 0, komisyon: 0, bmv: 0, kmv: 0, kdv: 0 });
        const o = m.get(k); o.satirAdedi++; (x.giris ? o.gF : o.cF).add(x.fisId);
        if (x.giris) { o.girisMiktar += x.miktar; o.girisTutar += x.tutar; } else { o.cikisMiktar += x.miktar; o.cikisTutar += x.tutar; }
        o.komisyon += x.komisyon; o.bmv += x.bmv; o.kmv += x.kmv; o.kdv += x.kdv; }
    ...
    const satirlar = detay ? ham.map((x: any) => ({ ...x, tip: x.giris ? "A" : "S" }))
    ...
    if (ozet.length > 1) { const g = topla(() => "*", () => ({ paraKod: "GENEL" })); ozet.push({ ...g[0], girisMiktar: 0, cikisMiktar: 0 }); }
    return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p) || " · Tüm vezneler"}${adetOzeti(p.istatistikIdler, "istatistik")} · ${detay ? "Detaylı" : "Toplam"}`,
      "Fişler istatistik koduna göre gruplanır. Giriş = alış fişleri, Çıkış = satış fişleri. Miktar toplamları farklı para birimlerini birlikte içerebilir; para bazında toplam rapor sonundaki tablodadır (GENEL satırında miktar toplanmaz). İptal fişler hariçtir.",
      ozet);
  },
  ...
  async ISTKMV1(pool, p, t) {
    ...
    const satirlar = res.recordset.map((r: any) => { const tutar = Number(r.tutar) || 0, kmv = Number(r.kmv) || 0;
    ...
    for (const x of satirlar) { if (!x.kmv && !x.kmvOrani) continue; const k = `${x.istatistikKod}|${x.paraKod}|${x.kmvOrani}`;
      if (!oz.has(k)) oz.set(k, { istatistik: x.istatistikBaslik, paraKod: x.paraKod, kmvOrani: x.kmvOrani, fisler: new Set<number>(), miktar: 0, matrah: 0, kmv: 0 });
      const o = oz.get(k); o.fisler.add(x.fisId); o.miktar += x.miktar; o.matrah += x.tutar; o.kmv += x.kmv; }
    const ozet = [...oz.values()].map(({ fisler, ...o }) => ({ ...o, adet: fisler.size }));
    return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p) || " · Alış + Satış"}${ozetEk(p) || " · Tüm vezneler"}${adetOzeti(p.istatistikIdler, "istatistik")}`,
      "Her satır bir fiş satırıdır. Tutar = satır tutarı (KMV hariç; bu sistemde KMV kura dahil değildir), Fiş toplamı = tutar + KMV. Kambiyo muameleleri vergisi yalnızca satış fişlerinde doğar; rapor sonundaki özet yalnızca KMV'li satırları (istatistik × para × KMV oranı) toplar. İptal fişler hariçtir.",
      ozet);
  },
  ...
  async VERNUM1(pool, p, t) {
    ...
    try { const tn = (await pool.request().query(`SELECT TOP 1 ISNULL(T.DOVIZ_VERGI_SINIRI,0) sinir, RTRIM(ISNULL(SP.KOD,'USD')) para FROM dbo.TODVZ_TANIM T LEFT JOIN dbo.TODVZ_PARA SP ON SP.PARA_ID=T.DOVIZ_VERGI_SINIRI_PARA_ID`)).recordset[0];
      sinir = Number(tn?.sinir) || 0; sinirPara = String(tn?.para || "USD"); } catch { /* tanım okunamadıysa sınır uygulanmaz */ }
    ...
    const ham = res.recordset.map((r: any) => { const miktar = Number(r.miktar) || 0, tutar = Number(r.tutar) || 0, usdKuru = Number(r.usdKuru) || 0; const no = String(r.kimlikNo || "");
      ...
      .filter((s: any) => !sinirUstu || s.usdKarsiligi >= sinir);
    ...
    const satirlar = ham.map((s: any, i: number) => ({ ...s, _i: i })).sort((a: any, b: any) => (p.siralama === "tutar" ? (toplam.get(b.grupAnahtar)! - toplam.get(a.grupAnahtar)!) : p.siralama === "unvan" ? (unvan.get(a.grupAnahtar) || "").localeCompare(unvan.get(b.grupAnahtar) || "", "tr") : 0)
    ...
    const oz = new Map<string, any>();
    for (const s of satirlar) { const k = `${s.grupAnahtar}|${s.tipKod}|${s.paraKod}`;
      if (!oz.has(k)) oz.set(k, { kimlik: s.grupBaslik, tip: s.tip, paraKod: s.paraKod, fisler: new Set<number>(), miktar: 0, tutar: 0, usdKarsiligi: 0, komisyon: 0, bmv: 0, kmv: 0 });
      const o = oz.get(k); o.fisler.add(s.fisId); o.miktar += s.miktar; o.tutar += s.tutar; o.usdKarsiligi += s.usdKarsiligi; o.komisyon += s.komisyon; o.bmv += s.bmv; o.kmv += s.kmv; }
    ...
    return sinirla(satirlar, t, `${aralikOzeti(p)}${tipOzeti(p)}${ozetEk(p) || " · Tüm vezneler"}${p.arama?.trim() ? ` · Arama: "${p.arama.trim()}"` : ""}${sinirUstu ? ` · +${sinirMetni}` : " · Tüm işlemler"}`,
      `${sinirUstu ? `Yalnızca USD karşılığı firma tanımındaki döviz vergi sınırına (${sinirMetni}) ulaşan fiş satırları listelenir. ` : ""}USD karşılığı = satır tutarı ÷ fişin USD kuru; USD satırlarında miktarın kendisidir. Fişte USD kuru kayıtlı değilse sırayla fişteki USD satırının kuru, yoksa fiş tarihindeki kur tablosunun USD kuru (alışta alış, satışta satış) kullanılır; hiçbiri yoksa karşılık 0 görünür. Yalnızca döviz fişleri kapsanır; iptal fişler hariçtir.`,
      ozetSatirlar);
  },
};
