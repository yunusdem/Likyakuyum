...
export const CARI2_SORGULARI: Record<string, Sorgu> = {
  ...
  async POSEKS1(pool, p, t) {
    ...
    const gruplar = new Map<string, { ortak: Record<string, any>; devirBorc: number; devirAlacak: number; hareketler: any[] }>();
    for (const r of res.recordset) {
      ...
      if (!gruplar.has(k)) gruplar.set(k, { devirBorc: 0, devirAlacak: 0, hareketler: [], ortak: { grupAnahtar: k, grupBaslik: `CİHAZ: ${pos} · ${r.paraKod}`, posCihazi: Number(r.posId) ? (r.cihazKod || String(r.posId)) : "-", paraKod: r.paraKod } });
      const g = gruplar.get(k)!;
      if (r.onceki) { if (borc) g.devirBorc += m; else g.devirAlacak += m; }
      else g.hareketler.push({ ...r, fisNo: String(r.fisNo ?? ""), borc: borc ? m : 0, alacak: borc ? 0 : m });
    }
    const satirlar: Record<string, any>[] = [], sonBakiyeler: Record<string, any>[] = [];
    for (const g of gruplar.values()) {
      let bakiye = g.devirBorc - g.devirAlacak, borc = g.devirBorc, alacak = g.devirAlacak;
      if (g.devirBorc || g.devirAlacak) satirlar.push({ ...g.ortak, fisNo: "", tarih: p.baslangic, cariKod: "", cariAd: "", aciklama: "POS Devir", vezneKod: "", borc: g.devirBorc, alacak: g.devirAlacak, bakiye, devirSatiri: true });
      for (const h of g.hareketler) { bakiye += h.borc - h.alacak; borc += h.borc; alacak += h.alacak; satirlar.push({ ...g.ortak, ...h, bakiye }); }
      if (g.hareketler.length || g.devirBorc || g.devirAlacak) sonBakiyeler.push({ cihaz: g.ortak.grupBaslik, borc, alacak, sonBakiye: bakiye });
    }
    return sinirla(satirlar, t, `${aralikOzeti(p)}${ozetEk(p) || " · Tüm cariler"}`,
      "Yalnızca hareket tipi \"POS / Kredi Kartı\" olan cari hareketler listelenir; POS cihazı ve para birimi bazında gruplanır. Borç = carinin borçlandığı, Alacak = cariden POS ile tahsil edilen tutar; bakiye = borç − alacak (yürüyen). Başlangıç tarihinden önceki hareketler \"POS Devir\" satırında toplanır ve grup toplamına dahildir.",
      sonBakiyeler);
  },
