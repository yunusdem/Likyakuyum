...
import { type RaporParametreler, aralikOzeti, filtreler, kurCoz, ozetEk, paraKumesi, sinirla } from "../raporOrtak.js";
...
export const VEZNE_SORGULARI: Record<string, Sorgu> = {
  ...
  async VEZANL1(pool, p, t) {
    ...
    return sinirla(satirlar, t, `Anlık bakiye${ozetEk(p) || " · Tüm vezneler"} · ${kur.aciklama}`,
  ...
  async KURKON2(pool, p, t) {
    ...
    const satirlar = res.recordset.map((r: any) => { const am = Number(r.alisMiktar) || 0, at = Number(r.alisTutar) || 0, sm = Number(r.satisMiktar) || 0, st = Number(r.satisTutar) || 0;
    ...
    return sinirla(satirlar, t, `${aralikOzeti(p)}${ozetEk(p) || " · Tüm vezneler"} · ${ayrinti ? "vezne ve kullanıcı ayrıntılı" : "gün × para"}`,
      "Ortalama kur = tutar / miktar. Makas = ortalama satış kuru − ortalama alış kuru (aynı satırda hem alış hem satış varsa). TL satırlarında kur 1'dir. İptal fişler hariçtir; cari dekontlar kapsam dışıdır.");
  },
