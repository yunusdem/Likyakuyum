...
const rakamlar = (v: unknown): string => kirp(v).replace(/\D/g, "");

export const luhnGecerliMi = (kartNo: string): boolean => {
  if (!/^\d{13,19}$/.test(kartNo)) return false;
  let toplam = 0;
...
export const kartsizMesaj = (mesaj: unknown): string => kirp(mesaj).replace(/\d[\d -]{11,22}\d/g, (m) => (m.replace(/\D/g, "").length >= 13 ? "[kart no gizlendi]" : m));

export const kartMaskele = (kartNo: string): string => `${kartNo.slice(0, 6)}${"*".repeat(Math.max(kartNo.length - 10, 0))}${kartNo.slice(-4)}`;

/** Vomsis transaction/find yanıtı başarılı bir çekim mi? */
export const odemeBasariliMi = (d: any): boolean => {
  if (!d) return false;
  const durum = kirp(d.status).toLocaleLowerCase("tr");
  if (durum.includes("başarısız") || durum.includes("basarisiz") || durum.includes("fail") || durum.includes("hata")) return false;
...
const donusAdresi = (apiAdresi: unknown, canli: boolean): string => {
  let url: URL;
  try {
  ...
  if (!["http:", "https:"].includes(url.protocol)) throw ApiError.badRequest("Dönüş adresi geçersiz.");
  if (canli && url.protocol !== "https:") throw ApiError.badRequest("Canlı modda kartla ödeme yalnızca HTTPS üzerinden yapılabilir.");
...
export class EBankaVposOdemeService {
  public static async taksitTablosu(dbContext?: DbContext) {
  ...
  public static async binSorgula(bin: unknown, dbContext?: DbContext) {
  ...
  public static async odemeBaslat(girdi: Record<string, any>, istemciIp: string, kullaniciId?: number, dbContext?: DbContext) {
    ...
    if (!adSoyad) throw ApiError.badRequest("Kart üzerindeki ad soyad zorunludur.");
    // Test modunda uydurma numarayla denenebilsin (Vomsis dokümanındaki örnek kart da Luhn'dan geçmiyor)
    if (!/^\d{13,19}$/.test(kartNo) || (mod === "canli" && !luhnGecerliMi(kartNo))) throw ApiError.badRequest("Kart numarası geçersiz.");
    if (!/^(0[1-9]|1[0-2])$/.test(ay) || !/^20\d{2}$/.test(yil)) throw ApiError.badRequest("Son kullanma tarihi geçersiz.");
    const simdi = new Date();
    if (Number(yil) < simdi.getFullYear() || (Number(yil) === simdi.getFullYear() && Number(ay) < simdi.getMonth() + 1)) throw ApiError.badRequest("Kartın son kullanma tarihi geçmiş.");
    if (!/^\d{3,4}$/.test(cvc)) throw ApiError.badRequest("Güvenlik kodu (CVC) geçersiz.");
    ...
    let yanit: any;
    try {
      yanit = await VomsisClient.istek(
        "vpos",
        "/payment",
        {
          metod: "POST",
    ...
    let alanlar: { ad: string; deger: string }[] = [];
    if (Array.isArray(d.inputs)) alanlar = d.inputs.map((x: any) => ({ ad: kirp(x?.name ?? x?.key), deger: kirp(x?.value) })).filter((x: { ad: string }) => x.ad);
   ...
   */
  public static async sonuc(referansNo: string, kullaniciId?: number, dbContext?: DbContext) {
    const islem = await EBankaVposSqlRepository.islemGetir(referansNo, dbContext);
    if (!islem) throw ApiError.notFound("İşlem bulunamadı.");
    // Link ödemesinin tahsilat fişi linke kesilir (Ödeme Linkleri > Durumları Güncelle); burada ikinci bir fiş kesilmesin
    if (islem.linkUid) throw ApiError.badRequest("Bu işlem bir ödeme linkinden geldi; tahsilat fişi Ödeme Linkleri ekranından kesilir.");
    if (!islem.cariKartId) throw ApiError.badRequest("Bu işlem bu programdan başlatılmamış; cariye bağlı değil.");
    ...
    if (islem.tur === "bekliyor" || islem.durumKodu === null) {
      ...
      if (!d || (!kirp(d.status) && !kirp(d.errorCode))) return { durum: "bekliyor" as const, mesaj: "3D Secure doğrulaması bekleniyor.", islem };
      ...
      await EBankaVposSqlRepository.odemeSonucuYaz(
        referansNo,
        { basarili, hataKodu: kirp(d.errorCode) || null, hataMesaji: kirp(d.errorMessage) || (basarili ? null : kirp(d.status) || null), kartBanka: kirp(d.creditCardBank) || null, posAdi: kirp(d.posName) || null },
        dbContext
      );
      if (!basarili) {
        return { durum: "basarisiz" as const, mesaj: kirp(d.errorMessage) || kirp(d.status) || "Ödeme başarısız.", islem: await EBankaVposSqlRepository.islemGetir(referansNo, dbContext) };
      }
    } else if (islem.durumKodu !== 1) {
      return { durum: "basarisiz" as const, mesaj: islem.hataMesaji || "Ödeme başarısız.", islem };
    }
    ...
    if (guncel.bankaHareketId && guncel.bankaHareketId > 0) {
      ...
      if ("neden" in on) {
        ...
        try {
          const fisId = await EBankaVposService.fisYaz(
            { islemTipi: 0, bankaId: on.bankaId, cariKartId: guncel.cariKartId, paraId: on.paraId, kur: on.kur, tutar: guncel.tutar, gun, belgeNo: referansNo, aciklama: `Sanal POS kartla ödeme: ${guncel.aciklama || referansNo}` },
            kullaniciId,
            dbContext
          );
    ...
    return { durum: "basarili" as const, mesaj: muhasebe, islem: await EBankaVposSqlRepository.islemGetir(referansNo, dbContext) };
  }
}
...
export const DONUS_SAYFASI =
  `<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Ödeme</title></head>` +
