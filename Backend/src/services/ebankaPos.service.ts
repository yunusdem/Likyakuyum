...
import { EBankaPosSqlRepository, PosFiltre, VomsisPosHareket, VomsisTerminal } from "../models/ebankaPosSql.repository.js";
...
const firmaAnahtari = (c?: DbContext) => `pos|${(c?.dbServer || "").toLowerCase()}|${(c?.dbName || "").toLowerCase()}`;
...
export const dilimle = (baslangic: string, bitis: string, gun = VOMSIS_EN_UZUN_ARALIK_GUN): { bas: number; bit: number }[] => {
  const son = Date.parse(`${bitis}T00:00:00Z`);
  const dilimler: { bas: number; bit: number }[] = [];
...
export class EBankaPosService {
  public static async esitle(girdi: { baslangic?: string; bitis?: string }, kullaniciId?: number, dbContext?: DbContext) {
    ...
    if (calisanlar.has(anahtar)) throw ApiError.conflict("Şu anda başka bir POS eşitlemesi sürüyor. Bitmesini bekleyin.");
    calisanlar.add(anahtar);

    let yeni = 0;
    let guncellenen = 0;
    try {
      const terminaller = dizi<VomsisTerminal>((await VomsisClient.istek("banka", "/pos-rapor/stations", {}, dbContext))?.data);
      ...
      await EBankaSqlRepository.logYaz(
        { islem: "pos-esitleme", mod, basarili: true, adet: yeni, mesaj: `${baslangic} – ${bitis}: ${terminaller.length} terminal, ${yeni} yeni / ${guncellenen} güncellenen hareket`, kullaniciId },
        dbContext
      );
      return { mod, baslangic, bitis, terminalAdedi: terminaller.length, yeniHareket: yeni, guncellenenHareket: guncellenen };
    } catch (err: any) {
      await EBankaSqlRepository.logYaz(
        { islem: "pos-esitleme", mod, basarili: false, adet: yeni, mesaj: `${baslangic} – ${bitis}: ${err?.message} (hatadan önce ${yeni} yeni hareket yazıldı)`, kullaniciId },
        dbContext
      );
      throw err;
    } finally {
  ...
  public static async hareketleriListele(filtre: PosFiltre, dbContext?: DbContext) {
  ...
  public static async excel(filtre: PosFiltre, dbContext?: DbContext): Promise<Buffer> {
    ...
    const para = "#,##0.00";
    const baslikYap = (sayfa: ExcelJS.Worksheet) => {
    ...
    for (const g of veri.gunluk) ozet.addRow({ tarih: g.islemTarihi, banka: g.bankaAdi, terminal: g.terminalAdi, doviz: g.doviz, adet: g.adet, brut: g.brut, komisyon: g.komisyon, net: g.net, valor: g.valor });
  ...
  public static async fisKes(
    girdi: { vomsisIdler?: unknown; bankaId?: number; cariKartId?: number | null; tarih?: string; kur?: number | null },
    kullaniciId?: number,
    dbContext?: DbContext
  ): Promise<{ bankaHareketId: number; net: number; adet: number }> {
    ...
    if (!idler.length) throw ApiError.badRequest("POS hareketi seçilmedi.");
    const bankaId = Number(girdi.bankaId);
    if (!(bankaId > 0)) throw ApiError.badRequest("Banka hesabı seçilmelidir.");
    await BankaService.getBankaById(bankaId, dbContext); // yoksa 404; mevcut fiş kaydı olmayan kart için sessizce yeni kart açıyor
    if (girdi.tarih !== undefined && !gunMu(girdi.tarih)) throw ApiError.badRequest("Fiş tarihi geçersiz.");

    const satirlar = await EBankaPosSqlRepository.secilenleriGetir(idler, dbContext);
    if (satirlar.length !== idler.length) throw ApiError.badRequest("Seçilen hareketlerin bir kısmı bulunamadı.");
    if (satirlar.some((s) => s.bankaHareketId !== null)) throw ApiError.conflict("Seçilen hareketlerin bir kısmı için zaten banka fişi kesilmiş.");
    const dovizler = [...new Set(satirlar.map((s) => (s.doviz || "TL").toUpperCase()))];
    if (dovizler.length > 1) throw ApiError.badRequest("Farklı döviz cinsindeki hareketler tek fişte toplanamaz.");

    const net = Math.round(satirlar.reduce((t, s) => t + s.net, 0) * 100) / 100;
    if (!(net > 0)) throw ApiError.badRequest("Seçilen hareketlerin net toplamı sıfır ya da eksi.");

    const para = await EBankaAktarimSqlRepository.paraSozlugu(dbContext);
    const paraId = para.kodlar.get(dovizler[0]);
    if (!paraId) throw ApiError.badRequest(`"${dovizler[0]}" döviz cinsi para tanımlarında yok.`);
    ...
    if (!(await EBankaPosSqlRepository.talepEt(idler, dbContext))) throw ApiError.conflict("Seçilen hareketlerin bir kısmı için zaten banka fişi kesilmiş.");
    try {
